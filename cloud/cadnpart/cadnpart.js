...
function getNamLan(key, lang) {
	for(var i=0; i< lanVal.length; i++) {
		var k = lanVal[i]['key'];
		if(k==key) {
			return lanVal[i][lang];
		}
	}
	return 'unknown';
}
...
//        data:data,
    ...
    var makerStore = Ext.create('B2bLounge.store.MakerStore', {lang: vLANG});
...
//                    '<span>{type}({tel_no} Review{[values.reviews == 1 ? "" : "s"]})</span>',
	...
	var objWest = null;
		...
		var addMakerAction =	 Ext.create('Ext.Action', {
			iconCls:'add',
		    text: CMD_ADD,
			disabled: false,
		    handler: function(widget, event) {
		...
		var removeMakerAction = Ext.create('Ext.Action', {
			itemId: 'removeButton',
		    iconCls: 'remove',
		    text: CMD_DELETE,
		    disabled: true,
...
//                	    //Tree를 가린다.
//                	    //Ext.getCmp('west-panel-tree').collapse();
//                	    var w = Ext.getCmp('west-panel-tree');
//                	    console_log(w);
//                	    w.expand();
...
//          							    				companyStore.load(function() {});
		...
		objCenter = {
            ...
            activeTab: 0,
            tabPosition: 'top',
            items: [ productPanel, cadlibPanel, companyPanel, peoplePanel
            /*, {
                //contentEl: 'center2',
                title: '사용안내',
                html: '다음과 같이 사용하세여.'
...
//					    	rec.get('leaf'); // 이렇게 데이터 가져올 수 있음
			 ...
			 listeners: {
	           ...
	           dockedItems: [
	                         
					{
						    dock: 'top',
						    xtype: 'toolbar',
						    items: [addMakerAction,  '-', removeMakerAction,
						            '->', { xtype: 'button', text: 'Owner', pressed: false, toggleGroup: 'togTreeSort',tooltip: 'Sort by Owner' }, {xtype: 'button', text: 'Name',  pressed: true, toggleGroup: 'togTreeSort', tooltip: 'Sort by Name'  }
						            ]
					},
	               ...
	               items: [
	                	                    {
	         			                    	...
	         			                    	listeners : {
	         				    	            		specialkey : function(field, e) {
	         				    	            		if (e.getKey() == Ext.EventObject.ENTER) {
	         				    	            			srchTreeHandler (treepanelFolder, makerStore, 'srchMaker_name', 'maker_name', true);
	         				    	            			//srchSingleHandler ('srchName', 'srchMaker_name', true);
	         				    	            		}
	         				    	            	}
	         				                	},
	         			                        trigger1Cls: Ext.baseCSSPrefix + 'form-clear-trigger',
	    ...
	    objWest = new Ext.TabPanel({
	        ...
	        activeTab: 0,
	        tabPosition: 'top',
	        items: [ treepanelUnspsc , treepanelFolder, treepanelBizcategory]
	    });
...
function getLoginedToolbar(objLang) {
	...
	items.push({ 
	 	xtype : "component",
	 	html : '<span style="margin-left: 120px; font-size:15px; font-weight:bold;font-color:#333333"><i>CADnPART :: Joy for Engineers !!!</i></span>'
	});
