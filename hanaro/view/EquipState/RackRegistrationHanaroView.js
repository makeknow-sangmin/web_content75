Ext.define('Hanaro.view.equipState.RackRegistrationHanaroView', {
			...
			buttons: [{
				...
				handler: function () {
					...
					if (selections == null || selections.length == 0) {
						Ext.Msg.alert('확인', '선택한 랙이 없습니다.');
						return;
					} else {
						var rec = selections[0];
						var class_code = rec.get('class_code');
						var spec_need_flag = rec.get('spec_need_flag'); //층
						var class_type = rec.get('class_type'); //호
						if(spec_need_flag==null || spec_need_flag=='') {
							Ext.Msg.alert('확인', '층값이 지정되지 않았습니다.');
							return;
						}
						if(class_type==null || class_type=='') {
							Ext.Msg.alert('확인', '호값이 지정되지 않았습니다.');
							return;
						}
						var form = gu.getCmp('formPanel').getForm();

						if ( form.isValid() == false) {
							Ext.Msg.alert('확인', '유효한 form이 아닙니다.');
							return;
						}

						var each = "N";//개별바코드
						var val = form.getValues(false);
						var print_qty = Number(val['print_qty']);
						var cols_qty = Number(val['cols_qty']);
						var ho = Number(class_type);
						var floor = Number(spec_need_flag);
						...
						for(var i=0 ; i< ho; i++) {
							for(var j=0 ; j< floor; j++) {
								...
								var strFloor = "" + (j+1);
								if(strFloor.length<2) {
									strFloor = "0" + strFloor;
								}

								if(print_qty<2) {
									var barcode = class_code + "-" + strFloor + "-" + strHo;
									barcodeList.push(barcode);
									descList.push(barcode);
								} else {
									for(var k=0; k<print_qty; k++) {
										var strSerial = "" + (k+1);
										if(strSerial.length<2) {
											strSerial = "0" + strSerial;
										}
										var barcode = class_code + "-" + strFloor + "-" + strHo + "-" + strSerial;
										barcodeList.push(barcode);
										descList.push(barcode);
									}
								}
						...
						Ext.Ajax.request({
							url: CONTEXT_PATH + '/pdf.do?method=printBarcode',
							params: {
