 ...
 * 하나로 BOM 구성
 */
Ext.define('Hanaro.view.produceMgmt.DesignBomHanaroView2', {
...
//                                    title: panelSRO1186 + ' | ' + panelSRO1187 + ' | ' + panelSRO1188 + ' | 통화', //panelSRO1174,
        ...
        var buttonToolbar = Ext.create('widget.toolbar', {
    		...
    		items: [
					{
						id: gu.id('target-routeTitlename')/*DBM7*/,
					    xtype:'component',
					    html: "BOM List를 선택하세요.",
					    width: 370,
					    style: ''
...
// id: gu.id('childCount')/*DBM7*/,
// html: ''
// }
    		        ]
    	});
        
		this.createGrid([ buttonToolbar, this.addSearchField('pj_name') ], {width: '60%'});
	            ...
	            items: [
	 	         {
			 	   	 ...
			 	   	 handler: function() {
		                     ...
		                     fn: function(btn) {
		                         ...
		                         if(result=='yes') {
		                        	 var o = Ext.getCmp(gu.id('bom_content'));
	 	         ...
	 	         },'-',
	 	        {
			 	   	 ...
			 	   	 text: '사전검증',
			 	   	 handler: function() {
			 	   		 var bom_content = Ext.getCmp(gu.id('bom_content'));
		             ...
		             handler: function() {
			 	   		...
			 	   		Ext.getCmp(gu.id('bom_content')).setLoading(true);
					  	...
					  	Ext.Ajax.request({
							...
							success : function(result, request) { 
		        				...
		        				Ext.getCmp(gu.id('bom_content')).setLoading(false);
							},
							failure: extjsUtil.failureMessage
						});
    ...
    createWest: function() {
    			...
    			var bHeight = 250;
    			        ...
    			        var win = Ext.create('ModalWindow', {
    			            ...
    			            buttons: [{
    			                text: CMD_OK,
    			            	handler: function(){
    			                    var form = Ext.getCmp(gu.id('modformPanel')).getForm();
    			                    if(form.isValid())
    			                    {
    			                	var val = form.getValues(false);
		...
		this.redrawAssyAction = Ext.create('Ext.Action', {
            iconCls: 'af-search',
            text: '검색',
            tooltip: '조건 검색',
            handler: function () {
				...
				var search_itemcode = gm.me().search_mjcm01kr;

				console_logs("bom구성 검색어 >>>", search_itemcode);

				gm.me().productStore.getProxy().setExtraParam('bom_flag', 'T');
				gm.me().productStore.getProxy().setExtraParam('sg_code', 'BOM');
				gm.me().productStore.getProxy().setExtraParam('parent', '-1');
				gm.me().productStore.getProxy().setExtraParam('parent_uid', '-1');

				//gMain.selPanel.productStore.getProxy().setExtraParam('name', '%'+search_itemcode+'%');
				gMain.selPanel.productStore.getProxy().setExtraParam('item_code', '%'+search_itemcode+'%');

            }
		});
		
		this.addCart = Ext.create('Ext.Action', {
            xtype: 'button',
            iconCls: 'fa-cart-arrow-down_14_0_5395c4_none',
            text: '구매카트 담기 ',
            tooltip: '구매를 위한 마이카트 담기',
            // disabled: true,
            handler: function (widget, event, selection) {
				...
				//var selections = gridMycart.getSelectionModel().getSelection();
					...
					console_logs('selections 3333', selections);
...
//                            },
		...
		var myFormPanel = Ext.create('Ext.form.Panel', {
	        ...
	        items: [{
				id: gu.id('search_information')/*DBM7*/,
	        ...
	        {	
				...
				listeners : {
		          ...
		          trigger1Cls: Ext.baseCSSPrefix + 'form-search-trigger', 'onTrigger1Click': function() {
		        	  ...
		        	  var val = Ext.getCmp(gu.id('item_code')).getValue();
		        	  if(val!=null && val!='') {
 	               ...
 	               listeners: {
     	                    select: function (combo, record) {
     	                    	...
     	                    	console_log('systemCode : ' + systemCode 
     	                    			+ ', codeNameEn=' + codeNameEn
     	                    			+ ', codeName=' + codeName	);
     	                    	Ext.getCmp(gu.id('standard_flag')).setValue(systemCode);
     	                    	
     	                    	gm.me().getPl_no(systemCode);
// var prefix = systemCode;
     	                    	var oClass_code1 = Ext.getCmp(gu.id('class_code1'));
							 	if(systemCode=='S') {
		...
		,{
            ...
            items: [
		        {
                    xtype:'button',
                    id: gu.id('ok_btn_id')/*DBM7*/,
                    text: CMD_OK,
		            handler: function() {
		            	
		            	var item_code = Ext.getCmp(gu.id('item_code')).getValue();
			    		if(item_code==null || item_code.length==0) {
			    			item_code = this.selectedPjCode + this.selectedAssyCode + Ext.getCmp(gu.id('pl_no')).getValue();
			    			Ext.getCmp(gu.id('item_code')).setValue(item_code);
			    		}
		            	...
		            	var isUpdateSpec = Ext.getCmp(gu.id('isUpdateSpec')/*DBM7*/).getValue();
		            	var specification = Ext.getCmp(gu.id('specification')).getValue();
		            	var unique_id = Ext.getCmp(gu.id('unique_id')).getValue();
		            	var standard_flag = Ext.getCmp(gu.id('standard_flag')).getValue();
			            		...
			            		Ext.Ajax.request({
			            			...
			            			success : function(result, request) {
			            				var jsonData = Ext.decode(result.responseText);
			             				...
			             				var exist = Ext.getCmp(gu.id('unique_id')).getValue();
    	...
    	this.productStore = /*
							 ...
							 * }); //
							 */  Ext.create('Mplm.store.ProductStoreHanaro');
    	
    	this.productStore.getProxy().setExtraParam('bom_flag', 'T');
		this.productStore.getProxy().setExtraParam('sg_code', 'BOM');
		this.productStore.getProxy().setExtraParam('parent', '-1');
		this.productStore.getProxy().setExtraParam('parent_uid', '-1');
    	...
    	var productItems = [
			...
			change : function(fieldObj, e) { 
				console_logs(" eeee 출력 ", e);
				gm.me().search_mjcm01kr= e;
				console_logs(" gm.me().search_mjcm01kr 검색어 출력 ", gm.me().search_mjcm01kr);
		 ...
		 this.productGrid =
             ...
             Ext.create('Rfx.view.grid.StandardProductHanaro', {
        ...
        this.productGrid.getSelectionModel().on({
        	selectionchange: function(sm, selections) {
        		...
        		if(selections!=null && selections.length > 0){
					 ...
					 gm.me().selectedChild = rec.get('child');
					 gm.me().selectedAssyUid = rec.get('unique_uid');
					 ...
					 var routeTitlename = '[' + gm.me().itemCode  + '] ' + rec.get('item_name');

					 Ext.getCmp(gu.id('target-routeTitlename')).update('<b>'+routeTitlename+'</b>'); 
    ...
    INIT_TABLE_HEAD: function(){
    	...
    	setChildQuan : function (n) {
    		var o = Ext.getCmp(gu.id('childCount')/*DBM7*/);
    		if(o!=null) {
    			o.update(''+ n);	
    		}
    	},
    	...
    	setAssyQuan : function(n) {
    		var o = Ext.getCmp(gu.id('assy_quan')/*DBM7*/);
    		if(o!=null) {
    			o.update(''+ n);	
    		}
    	},
    	setProjectQuan : function(n) {
    		var o = Ext.getCmp(gu.id('pj_quan')/*DBM7*/);
    		if(o!=null) {
    			o.update(''+ n);	
    		}
    	},
    	...
    	setMaking_quan : function(n) {
    		var o = Ext.getCmp(gu.id('making_quan')/*DBM7*/);
    		if(o!=null) {
    			o.update(''+ n);	
    		}
    	...
    	selectAssy: function(routeTitlename, depth) {
    		...
    		Ext.getCmp(gu.id('addPartForm')/*DBM7*/).enable();
    		Ext.getCmp(gu.id('target-routeTitlename')).update('<b>'+routeTitlename+'</b>'); 
    		if(depth==1) {
    	...
    	unselectAssy : function() {
    		...
    		Ext.getCmp(gu.id('bom_content')).setValue(initTableInfo);
    		Ext.getCmp(gu.id('addPartForm')/*DBM7*/).disable();
    		Ext.getCmp(gu.id('target-routeTitlename')/*DBM7*/).update('Assembly를 선택하세요.'); 
    	},
    	 	...
    	 	if(systemCode=='S') {
    		   ...
    		   Ext.Ajax.request({
    			...
    			success : function(result, request) {   
    				...
    				Ext.getCmp(gu.id('pl_no')).setValue(str);
    	...
    	resetAction : Ext.create('Ext.Action', {
    		 ...
    		 handler: function(widget, event) {
    			 resetPartForm();
    			 Ext.getCmp(gu.id('addPartForm')/*DBM7*/).getForm().reset();
    		    	    ...
    		    	    w = Ext.create('ModalWindow', {
    			            ...
    			            buttons: [{
    			            	...
    			            	handler: function(btn){
    			            		var form = Ext.getCmp(gu.id('formPanelSelect')/*DBM7*/).getForm();
    		            			var val = form.getValues(false);
    		            		    var selections = gridMycart.getSelectionModel().getSelection();
    		            		    if (selections) {
    		            		        	var uids = [];
    		...
    		Ext.Ajax.request({
    			...
    			success : function(result) {
    			   ...
    			   console_logs('o>>>>>>', o);
    				...
    				Ext.getCmp(gu.id('unique_id')).setValue( o['unique_id']);
    				Ext.getCmp(gu.id('unique_uid')).setValue( o['unique_uid']);
    				Ext.getCmp(gu.id('item_code')).setValue( o['item_code']);
    				...
    				Ext.getCmp(gu.id('standard_flag')).setValue(standard_flag);
    				// Ext.getCmp(gu.id('standard_flag_disp')).select(getPosStandard(standard_flag));
    				Ext.getCmp(gu.id('model_no')).setValue( o['model_no']);	
    				Ext.getCmp(gu.id('description')).setValue( o['description']);
    				
    				Ext.getCmp(gu.id('comment')).setValue( o['comment']);
    				Ext.getCmp(gu.id('maker_name')).setValue( o['maker_name']);
    				Ext.getCmp(gu.id('bm_quan')).setValue('1');
    				Ext.getCmp(gu.id('unit_code')).setValue( o['unit_code']);
    				Ext.getCmp(gu.id('sales_price')).setValue( o['sales_price']);
    				...
    				Ext.getCmp(gu.id('currency')).setValue(currency);
    	...
    	setPartForm: function(record) {
    		...
    		Ext.getCmp(gu.id('unique_id')).setValue( record.get('unique_id'));
    		Ext.getCmp(gu.id('unique_uid')).setValue( record.get('unique_uid'));
    		Ext.getCmp(gu.id('item_code')).setValue( record.get('item_code'));

    		Ext.getCmp(gu.id('item_name')).setValue( record.get('item_name'));
    		Ext.getCmp(gu.id('specification')).setValue( record.get('specification'));
    		
    		var standard_flag =  record.get('standard_flag');
    		Ext.getCmp(gu.id('standard_flag')).setValue(standard_flag);
    		
    		Ext.getCmp(gu.id('standard_flag_disp')).select(getPosStandard(standard_flag));
    		Ext.getCmp(gu.id('model_no')).setValue( record.get('model_no'));	
    		Ext.getCmp(gu.id('description')).setValue( record.get('description'));
    		Ext.getCmp(gu.id('pl_no')).setValue( record.get('pl_no'));
    		Ext.getCmp(gu.id('comment')).setValue( record.get('comment'));
    		Ext.getCmp(gu.id('maker_name')).setValue( record.get('maker_name'));
    		Ext.getCmp(gu.id('bm_quan')).setValue( record.get('bm_quan'));
    		Ext.getCmp(gu.id('unit_code')).setValue( record.get('unit_code'));
    		Ext.getCmp(gu.id('sales_price')).setValue( record.get('sales_price'));
    	...
    	resetPartForm: function() {
    		...
    		Ext.getCmp(gu.id('unique_id')).setValue( '');
    		Ext.getCmp(gu.id('unique_uid')).setValue( '');
    		Ext.getCmp(gu.id('item_code')).setValue( '');
    		Ext.getCmp(gu.id('item_name')).setValue( '');
    		Ext.getCmp(gu.id('specification')).setValue('');
    		Ext.getCmp(gu.id('standard_flag')).setValue('');
    		Ext.getCmp(gu.id('standard_flag_disp')).setValue('');

    		Ext.getCmp(gu.id('model_no')).setValue('');
    		Ext.getCmp(gu.id('description')).setValue('');
    		Ext.getCmp(gu.id('pl_no')).setValue('');
    		Ext.getCmp(gu.id('comment')).setValue('');
    		Ext.getCmp(gu.id('maker_name')).setValue('');
    		Ext.getCmp(gu.id('bm_quan')).setValue('1');
    		Ext.getCmp(gu.id('unit_code')).setValue('');
    		Ext.getCmp(gu.id('sales_price')).setValue( '0');

    		Ext.getCmp(gu.id('currency')).setValue('KRW');
    		Ext.getCmp(gu.id('unit_code')).setValue('PC');
    		this.readOnlyPartForm(false);
    	},
    	...
    	unselectForm: function(){
    		...
    		var cur_val = Ext.getCmp(gu.id('specification')).getValue();
    		var cur_standard_flag = Ext.getCmp(gu.id('standard_flag')).getValue();
    		
    		if(cur_standard_flag!='O') {
    			Ext.getCmp(gu.id('specification')).setValue(cur_val + ' ' + this.CHECK_DUP);		
    		}
    		
    		Ext.getCmp(gu.id('currency')).setValue('KRW');
    		
    		this.getPl_no(Ext.getCmp(gu.id('standard_flag')).getValue());
    		this.readOnlyPartForm(false);
    	},
    	...
    	readOnlyPartForm :function(b) {
    		...
    		this.setReadOnlyName(gu.id('model_no')/*DBM7*/, b);
    		this.setReadOnlyName(gu.id('description')/*DBM7*/, b);
    		// this.setReadOnlyName('pl_no', b);
    		this.setReadOnlyName(gu.id('comment')/*DBM7*/, b);
    		this.setReadOnlyName(gu.id('maker_name')/*DBM7*/, b);

    		this.setReadOnlyName(gu.id('currency')/*DBM7*/, b);
    		this.setReadOnlyName(gu.id('unit_code')/*DBM7*/, b);
    		
    		Ext.getCmp(gu.id('search_information')/*DBM7*/).setValue('');
    		
    	},

    	addNewAction: function() {

    		var form = Ext.getCmp(gu.id('addPartForm')/*DBM7*/).getForm();
    	    if(form.isValid()) {
    	    	var val = form.getValues(false);
...
// gm.me().reSelect();
    	...
    	registPartFc: function(params) {
			console_logs('registPartFc params', params);
			params.level = params.reserved_integer1;
			params.assytop_uid = params.parent_uid;

        	Ext.Ajax.request({
    			url: CONTEXT_PATH + '/purchase/material.do?method=addChildBOMTreeForce',
    			params: params,
        		success : function(result, request) {   
