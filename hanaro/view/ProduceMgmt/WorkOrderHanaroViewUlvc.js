Ext.define('Hanaro.view.produceMgmt.WorkOrderHanaroViewUlvc', {
...
// 		});
        ...
        if(	useMultitoolbar == true ) {
         ...
         this.createStore('Rfx.model.HanaroWorkOrderUlvc', [{
...
//        case "SHNH01KR":
        ...
        this.denyWorkOrder = Ext.create('Ext.Action', {
            ...
            handler: function() {
                Ext.MessageBox.show({
                    title:CMD_OK,
                    msg: '작업지시 취소는 한 개씩 만 가능합니다.<br>작업지시를 취소하시겠습니까?.',
...
//
    ...
    denyWorkOrderFc: function() {
        ...
        var rtgastUid = this.vSELECTED_RECORD.get('rtgast_uid');

        Ext.Ajax.request({
            url: CONTEXT_PATH + '/index/process.do?method=denyWorkOrderHanaroUlvc',
            params:{
                rtgastUid: rtgastUid
            },
...
*/
                    ...
                    for (var i = 0; i < selections.length; i++) {
                        var rec = selections[i];
                        console_logs('rec', rec);
                        var uid = rec.get('unique_id');  //assymap unique_id
                        var buyer_pj_code = rec.get('buyer_pj_code');
                        ...
                        var date = reserved_timestamp1 + ' <-> ' + reserved_timestamp3;
                        uniqueIdArr.push(uid);
                        pjcode_arr.push(pl_no);
                        lot_no_arr.push(buyer_pj_code + ':' + lot_no);
                        date_arr.push(date);
                        item_name_arr.push(item_name);
                    }

                    console_logs('uniqueIdArr', uniqueIdArr);
                    console_logs('pjcode_arr', pjcode_arr);
                    console_logs('lot_no_arr', lot_no_arr);
                    console_logs('date_arr', date_arr);
                    console_logs('item_name_arr', item_name_arr);

                    var form = gu.getCmp('formPanel').getForm();

                    form.submit({
                        url: CONTEXT_PATH + '/sales/productStock.do?method=printBarcodeUlvac',
                        params: {
                            assymapUids: uniqueIdArr,
                            pjcode_arr: pjcode_arr,
                            lot_no_arr : lot_no_arr,
                            date_arr : date_arr,
                            item_name_arr: item_name_arr
                        },
                        success: function (val, action) {
                            prWin.close();
