//자재 관리
Ext.define('Hanaro.view.designPlan.TopProducMgmtHanaroView', {
    extend: 'Hanaro.base.HanaroBaseView',
    xtype: 'top-product-mgmt-view',
    initComponent: function(){
       ...
       this.buttonToolbar.insert(4, this.myRemoveAction);
        ...
        this.createStore('Rfx2.model.company.hanaro.ProductTopMgmtHanaro', [{
                property: 'unique_id',
                direction: 'DESC'
            }],
		...
		this.store.getProxy().setExtraParam('sg_code', 'PROD');
...
//  				 }
        ...
        //카트담기, 세션
        //console_logs(" KSCM01KR 버튼 >>>>" , vCompanyReserved4);

        if(vCompanyReserved4 =='KSCM01KR') {

            this.selectCartabutton = Ext.create('Ext.Action', {
                iconCls: 'barcode',
                text: '수주 제품 선택',
                tooltip: '선택 제품을 담습니다.',
                disabled: true,
       ...
       // };
        ...
        if(vCompanyReserved4 =='KSCM01KR'){
            buttonToolbar.insert(6, this.selectCartabutton);
            buttonToolbar.insert(7, this.add_Cart);
        }
        ...
        this.setGridOnCallback(function(selections) {
            ...
            if (selections.length) {
                ...
                gm.me().selectCartabutton.enable();
                ...
                gm.me().selectCartabutton.disable();
    ...
    //                 //item_codess.push(vCompanyReserved4);
            
    //                 for (var i = 0; i < selections.length; i++) {
    //                     var selectarr = selections[i];
    //                     var itemCode = selectarr.get("item_code");
    //                     var srcahd_uid = selectarr.get("srcahd_uid");
    //                     console_logs("session 세션 하나씩담기 >>> " , itemCode);
    //                     console_logs("srcahd_uid 세션 하나씩담기 >>> " , srcahd_uid);
    ...
    //             Ext.util.Cookies.clear('product_manage', item_codess);
    ...
    //                         expires : new Date(Ext.Date.now() + (1000*60*60*24)) //  days

    //                     }));
    //                 Ext.state.Manager.set('product_values', item_codess);
    //     var test11 = Ext.state.Manager.set('test', item_codess);
    //     console_logs('>> test11 리스트  >>>>  ', test11);

    //                 Ext.state.Manager.set('test', item_codess);

    //                 var aa =  Ext.state.Manager.get("product_values");
    //                 //var select1 =  Ext.state.Manager.get("item_codeOne");
    //                 console_logs( " item_codess 제품관리조립 셀렉션 >>>" , aa);

    //                 console_logs( " 출력되면 안되는 manage >>>" , Ext.state.Manager.get("product_manage"));
    //                 //console_logs( " item_codeOne 1개 셀렉트 >>>" , select1);
    ...
    // clearSession : function (form) {
    //     addSCartKscm : Ext.MessageBox.show({
    //         title:'제품 담기',
    //         msg: '고르신 제품 세션을 리셋하시겠습니까?', 
    //         buttons: Ext.MessageBox.YESNO,
    //         fn: function(btn) {
    //             if(btn=='yes') {
    ...
    deleteConfirm: function(result) {
        if(result=='yes') {
            if(gm.me().selectedRecord == null) {
                ...
                console_logs('gm.me().selectedRecord', gm.me().selectedRecord);
                var unique_id = gm.me().selectedRecord.get('unique_id');
                Ext.Ajax.request({
                        url: CONTEXT_PATH + '/sales/product.do?method=destroy',
                        params:{
                            unique_id : [unique_id]
                        },
                        success : function(result, request) {   
