//수주관리 메뉴
Ext.define('Hanaro.view.salesDelivery.RecvPoHanaroView', {
...
//            text: '품목코드',
//            name: 'product_code',
//            storeClass: 'Rfx2.store.ProductStore',
//            emptyText:'제품을 선택하세요.',
			...
			handler: function() {
				...
				if (selections) {
					...
					for(var i=0; i< selections.length; i++) {
						...
						if(reserved_varchar6=='' || reserved_varchar6=='N') {
							arr.push(pj_code);
...
//	                       						result = result.substring(0,6)+'-'+result.substring(6,9);
    ...
    printBarcode: function () {
        ...
        for (var i = 0; i < selections.length; i++) {
            var rec = selections[i];
			var uid = rec.get('product_uid');  //Srcahd unique_id
			pj_code = pj_code + rec.get('pj_code');
