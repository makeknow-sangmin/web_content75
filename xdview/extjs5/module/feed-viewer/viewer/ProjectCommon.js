...
function arrangeColorOrg(series) {
	...
	function changeProjectTab(o, code) {
    	     mainTabs.setActiveTab(code);
    }
	...
	function Main() {
			...
			menuArr.push(

			    {
			        ...
			        url: {  name: '제품별현황',
			        		xtype: 'projectProduct',
			        		code: 'project-product'
			    }}, { title: '조직별현황',
			        url: {  name: '조직별현황',
			        		xtype: 'projectOrg',
			        		code: 'project-org'
...
//			    var tokenDelimiter = ':';
			    ...
			    function onTabChange(tabPanel, tab) {
			    	window.location.hash = '#'+ tab.itemId;
				        ...
				        case '차종별현황':
				        	...
				        	break;
				        case '기술별현황':
				        	if(selectedT1==null || AUTO_REFRESH) {
				        		redrawTechChart1();
				        	}
				        	break;
				        case '팀별현황':
	        ...
	        for(var i=0; i<menuArr.length; i++ ) {
	        	...
	        	var active = ( url['code']==token )? true : false;
	        	if(active==true) {
	        			var menu = 'menu' + (i+1);
						var menuOn = menu+'on';
	        	...
	        	mainTabs.addFeed(menuArr[i]['title'], menuArr[i]['url'], active);
