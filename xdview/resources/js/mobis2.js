...
function init_tabs(num) {
	if ( num == ""){
   	...
   	$('.TapArea').each(function() {
   			...
   			tabBtn.eq(num).click(); //탭 첫번째 컨텐츠를 보여줌
  	});
}
...
function top_menu(){
 ...
 //$(".tab_content").hide();
    ...
    //$('div#tab_content03').show(); //컨텐츠 지정해서 보여줌
    //$('.tabs a[href=#tab_content03]').closest('li').addClass('tap-on');
    //$('.TapArea').each(function() {
    //    $(this).find('div.tab_content:first').show(); //탭 첫번째 컨텐츠를 보여줌
    //    $('ul.tabs li:first').addClass('tap-on'); //탭 첫번째 메뉴 선택 스타일 지정
        //$('div#tab_content03').show(); //컨텐츠 지정해서 보여줌
        //$('.tabs a[href=#tab_content03]').closest('li').addClass('tap-on');
    //});
    //$('ul.tabs li').click(function() { //탭 클릭
     //   if (!$(this).hasClass('tap-on')) {
     //       $(this).addClass('tap-on').siblings('li').removeClass('tap-on'); //선택된 탭을 제외하고 나머지 class를 지움
     //       $($(this).find('a').attr('href')).show().siblings('div.tab_content').hide(); //선택된 컨텐츠를 제외하고 나머지 컨텐츠들을 숨김
      //  }
