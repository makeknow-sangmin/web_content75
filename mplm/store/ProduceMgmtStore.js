
Ext.define('Mplm.store.ProduceMgmtStore', {
    extend: 'Ext.data.Store',

    initComponent: function (params) {

        Ext.apply(this, {
            //검색조건에 필요한 조건값 여기에 추가
            wa_code: params.wa_code

        });

    },
    fields: [
        {name: 'unique_id', type: "string"}
        , {name: 'id', type: "int"}
        , {name: 'unique_id_long', type: "int"}
        , {name: 'wa_code', type: "string"}
        , {name: 'wa_name', type: "string"}
        , {name: 'nation_code', type: "string"}
        , {name: 'company_name', type: "string"}
    ],
    sorters: [{
        property: 'wa_name',
        direction: 'ASC'
    }],
    pageSize: vCompanyReserved4 == 'BIOT01KR' ? 1000 : 100,
    proxy: {
        type: 'ajax',
        url: CONTEXT_PATH + '/production/schdule.do?method=readProduceRequestVersion&status=CR',
        reader: {
            type: 'json',
            root: 'datas',
            totalProperty: 'count',
            successProperty: 'success'
        }
        , autoLoad: false
    },
    listeners: {
        load: function (store, records, successful, operation, options) {

            if (this.hasNull) {
                var blank = {
                    unique_id: '-1',
                    wa_code: '',
                    wa_name: '* 전체 *',
                    nation_code: '-'
                };

                this.add(blank);
            }

        },
        beforeload: function () {

            if (this.wa_code != null) {
                this.getProxy().setExtraParam('wa_code', this.wa_code);
            }

        }//endofbeforeload
    }//endoflistener
});