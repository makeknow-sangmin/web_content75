Ext.define('Rfx2.view.executiveInfo.ReportTemplateDefView', {
    extend: 'Rfx2.base.BaseView',
    xtype: 'report-template-def-view',
    temper: 1,
    initComponent: function () {


        //모델을 통한 스토어 생성
        this.createStore('Rfx.model.ReportTemplateDef', [{
                property: 'unique_id',
                direction: 'ASC'
            }],
            gm.pageSize
            ...
            items: [
                {
                    ...
                    items: [{
                        ...
                        fieldLabel: '구분',
                        items: [
                            {boxLabel: 'Pd함량', name: 'radio1', inputValue: 'PD', checked: true},
        ...
        this.grid.getSelectionModel().on({
            selectionchange: function(sm, selections) {
                ...
                gm.me().grid.getStore().getProxy().setExtraParam('unique_ids', null);
                ...
                if(selections.length > 0) {
                    ...
                    gm.me().grid.getStore().getProxy().setExtraParam('unique_ids', unique_ids);
                    ...
                    switch(rec.get('type')) {
                        ...
                        case 'INSB':
                            for(var i = 0; i < selections.length; i++) {
                                ...
                                for(var j = 2; j < 30; j++) {
                                    var value2 = selections[i].get(j < 10 ? 'v00' + j : 'v0' + j);
                                    if(value2 !== null) {
                                        ...
                                        for(var k = 0; k < columnNames.length; k++) {
                                            if(columnName2 == columnNames[k]['text']) {
                                                isDuplicate = true;
                                                break;
                                            }
                                        }
                            ...
                            for(var i = 0; i < selections.length; i++) {
                                ...
                                objk['lotNo'] = selections[i].get('lotNo');
                                for(var j = 2; j < 30; j++) {
                                    var value2 = selections[i].get(j < 10 ? 'v00' + j : 'v0' + j);
                                    if(value2 !== null) {
                                        var columnSplit2 = value2.split(":");
