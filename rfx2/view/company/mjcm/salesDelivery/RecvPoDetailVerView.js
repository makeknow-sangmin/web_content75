Ext.define('Rfx2.view.company.mjcm.salesDelivery.RecvPoDetailVerView', {
...
//objv['item_incoterms'] = item.get('item_incoterms');
                            ...
                            if (btn == "no") {
                                ...
                                if (form.isValid()) {
                                    ...
                                    for (var j = 0; j < storeData.data.items.length; j++) {
                                        ...
                                        objv['srcahd_uid'] = item.get('srcahd_uid');
                                        objv['reserved_varcharg'] = item.get('reserved_varcharg');
                                        objv['sales_price'] = item.get('dc_price');
                                        objv['reserved_varchar8'] = item.get('reserved_varchar8');
                                        objv['reserved_varchar9'] = item.get('reserved_varchar9');
                                        objv['delivery_plan'] = item.get('delivery_plan');
                                        objv['sales_amount'] = item.get('sales_amount');
        ...
        this.gridContractCompany = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                {
                    text: this.getMC('msg_order_grid_quan_desc', '수량'), width: 95, style: 'text-align:center', dataIndex: 'po_qty', sortable: false, align: 'right',
                    editor: 'numberfield',
                    renderer: function (value, context, tmeta) {
                        if (context.field == 'sales_amount') {
                            context.record.set('sales_amount', Ext.util.Format.number(value, '0,00/i'));
                        ...
                        handler: function (btn) {
                            ...
                            if (btn == "no") {
                                ...
                                for (var i = 0; i < items.length; i++) {
                                    ...
                                    var delivery_plan = present_date.setDate(present_date.getDate() + 1);
                                    var delivery_plan_parse = new Date(delivery_plan);
                                    var sales_price = item.get('dc_price');
        ...
        var action = Ext.create('Ext.Action', {
            ...
            handler: function (widget, event) {
                ...
                if (gu.getCmp('order_com_unique').getValue() == null) {
                 ...
                 //고객사 코드 입력
                if(gu.getCmp('order_com_unique').getValue().length >0){
                    gm.me().searchDetailStoreOnlySrcMap.getProxy().setExtraParam('combst_uid',gu.getCmp('order_com_unique').getValue());
                }
                ...
                var searchItemGrid = Ext.create('Ext.grid.Panel', {
                    ...
                    columns: [
                        ...
                        { text: gm.me().getMC('msg_product_add_search_field3', '규격'), flex: 1.8, style: 'text-align:center', dataIndex: 'specification', sortable: true },
                        { text: gm.me().getMC('msg_product_add_dia_price', '판매단가'), flex: 1, align: 'right', style: 'text-align:center', dataIndex: 'sales_price', sortable: true },
                        { text: gm.me().getMC('msg_product_add_dia_price', '할인적용단가'), flex: 1, align: 'right', style: 'text-align:center', dataIndex: 'dc_price', sortable: true },
    ...
    redrawSearchStore: function () {
        ...
        //var rec = gu.getCmp('productGrid').getSelectionModel().getSelected();
        //var rec = gm.me().grid.getSelectionModel().getSelected().items[0];
        console_logs("탑니다",  gu.getCmp('order_com_unique').getValue());
       
        //console_logs("탑니다", rec.get('order_com_unique'));
    ...
    // searchDetailStoreOnlySrcMap: Ext.create('Mplm.store.ProductDetailSearchExepOrderSrcMapStore', {}),
     searchDetailStoreOnlySrcMap: Ext.create('Mplm.store.ProductDetailSearchStore', {}),
