//수주관리 메뉴
Ext.define('Rfx2.view.company.chmr.salesDelivery.GovChangeMgmtView', {
    ...
    initComponent: function () {
        ...
        this.createStore('Rfx2.model.RecvMgmtGovChange', [{
            property: 'unique_id',
            direction: 'DESC2'
        }],
    ...
    itemdblclick: function (view, record, htmlItem, index, eventObject, opts) {
        ...
        var rec = selections[0];
        var detailStore = Ext.create('Rfx2.store.company.chmr.AccountInStore', {});
        detailStore.getProxy().setExtraParam('sloast_uid', rec.get('unique_uid'));
        detailStore.load();
        var paytype = Ext.create('Ext.data.Store', {
            fields: ['var_value', 'var_name'],
            data: [
                    ...
                    editor: {
                        ...
                        listeners: {
                            change: function (field, newValue, oldValue) {
                                console_logs('>>>>1', '>>>>>>>');
                                var store = gu.getCmp('loadForm').getStore();
                                var record = gu.getCmp('loadForm').getSelectionModel().getSelected().items[0];
                                var index = store.indexOf(record);
                                var sDate = new Date(store.getAt(index).get('request_date_str'));
                                var request_date = new Date(newValue - (sDate.getTimezoneOffset() * 60 * 1000)).toISOString().slice(0, 19).replace('T', ' ');

                                Ext.Ajax.request({
                                    url: CONTEXT_PATH + '/index/process.do?method=paycompleteInfoEdit',
                                    waitMsg: '데이터를 처리중입니다.',
                                    params: {
                    ...
                    editor: {
                        ...
                        listeners: {
                            change: function (field, newValue, oldValue) {
                                ...
                                var index = store.indexOf(record);
                                // var sDate = new Date(store.getAt(index).get('request_date_str'));
                                var request_date = new Date(newValue - (newValue.getTimezoneOffset() * 60 * 1000)).toISOString().slice(0, 19).replace('T', ' ');
                    ...
                    editor: {
                        ...
                        listeners: {
                            change: function (field, newValue, oldValue) {
                                ...
                                var index = store.indexOf(record);
                                // var sDate = new Date(store.getAt(index).get('request_date_str'));
                                // var request_date =  new Date(sDate - (sDate.getTimezoneOffset() * 60 * 1000)).toISOString().slice(0, 19).replace('T', ' ');
        ...
        var winProduct = Ext.create('ModalWindow', {
            title: '정산 입력현황<br>[고객명 : ' + rec.get('wa_name') + ' / 납품요구번호 : ' + rec.get('reserved_varchard') + ']',
            width: 200,
            height: 600,
            minWidth: 600,
            minHeight: 300,
