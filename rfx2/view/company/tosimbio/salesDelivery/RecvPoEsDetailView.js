Ext.define('Rfx2.view.company.chmr.salesDelivery.RecvPoEsDetailView', {
    ...
    initComponent: function () {
        ...
        this.addSearchField('buyer_company');
        //명령툴바 생성
        ...
        //         var rec = selections[0];
        //         if (rec.get('state') !== 'A') {
        //             gm.me().estimateConfirmByApprover(rec);
        //         } else {
        ...
        //                             console_logs('srcahd_uid', Number(item.get('srcahd_uid')));
        ...
        //                 var delivery_date = gu.getCmp('delivery_dates');
        ...
        //                                     objv['sales_amount'] = item.get('bm_quan');
                    ...
                    var productGridExtra = Ext.create('Ext.grid.Panel', {
                        ...
                        listeners: {
                            edit: function (editor, e, eOpts) {
                                ...
                                for (var j = 0; j < previous_store.length; j++) {
                                    var item = previous_store[j];
                                    console_logs('sales_amount_EDIT', Number(item.get('bm_quan')));
                                    console_logs('sales_price_EIDT', Number(item.get('sales_price')));
                                    total_quan = Number(total_quan) + Number(item.get('bm_quan'));
                                    total_price = Number(total_price) + (Number(item.get('sales_price')) * Number(item.get('bm_quan')));
                                }
                                ...
                                if (btn == "no") {
                                    ...
                                    if (form.isValid()) {
                                        ...
                                        for (var j = 0; j < storeData.data.items.length; j++) {
                                            ...
                                            objv['unit_code'] = item.get('remark');
                                            //objv['delivery_date'] = item.get('delivery_date');
                                        ...
                                        form.submit({
                                            ...
                                            url: CONTEXT_PATH + '/purchase/prch.do?method=addEstimateExtraCHMR',
                                            params: {
        ...
        this.gridContractCompany = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                { text: '품명', width: 100, style: 'text-align:center', dataIndex: 'item_name', sortable: false },
                { text: '규격', width: 200, style: 'text-align:center', dataIndex: 'concat_spec_desc', sortable: false },
                ...
                {
                    text: '수량', width: 90, style: 'text-align:center', dataIndex: 'item_quan', sortable: false, align: 'right',
                    editor: 'numberfield',
                    renderer: function (value, context, tmeta) {
                ...
                },
                {
                    ...
                    align: 'right',
                    renderer: function (value, context, tmeta) {
                        if (context.field == 'item_unit_price') {
                            context.record.set('item_unit_price', Ext.util.Format.number(value, '0,00/i'));
                        }
                        return Ext.util.Format.number(value, '0,00/i');
                    },
                },
                {
                    ...
                    align: 'right',
                    renderer: function (value, context, tmeta) {
                        if (context.field == 'estiamount') {
                            context.record.set('estiamount', Ext.util.Format.number(value, '0,00/i'));
                        }
                        return Ext.util.Format.number(value, '0,00/i');
                    },
                },
                ...
                { text: '비고', width: 150, style: 'text-align:center', dataIndex: 'item_remark', editor: { xtype: 'textfield' }, sortable: false },
            ],
            title: '상세정보',
            name: 'po',
            autoScroll: true,
        ...
        this.setGridOnCallback(function (selections) {
            if (selections.length) {
                ...
                gu.getCmp('selectedMtrl').setHtml(buyer_company);
    ...
    getPrdExtraAdd: function () {
        var action = Ext.create('Ext.Action', {
            ...
            handler: function (widget, event) {
                ...
                //DetailStoreOnlySrcMap.getProxy().setExtraParam('fix_type', 'SE');
                ...
                var searchItemGrid = Ext.create('Ext.grid.Panel', {
                    ...
                    layout: 'fit',
                    title: gm.me().getMC('msg_product_add_dia_header_title', '제품검색'),
                    plugins: {
                        ptype: 'cellediting',
                        clicksToEdit: 2,
                    },
                        ...
                        handler: function (btn) {
                            ...
                            if (btn == "no") {
                                ...
                                for (var i = 0; i < items.length; i++) {
                                    ...
                                    var unit = item.get('remark');
                                    var currency = item.get('currency');
                                    var po_date = new Date();
                                    var present_date = new Date();
                                    // var delivery_date = present_date.setMonth(present_date.getMonth() + 2);
                                    // var delivery_date_parse = new Date(delivery_date);
                                    ...
                                    store.insert(store.getCount(), new Ext.data.Record({
                                        ...
                                        // 'delivery_date': delivery_date_parse,
                                        'description': description,
                                        'bm_quan': quan,
    ...
    addPoEsWindow: function () {
        ...
        var shippingStore = Ext.create('Ext.data.Store', {
            fields: ['ship', 'ship_kr'],
            data: [
                { "ship": "자가도착도", "ship_kr": "자가 도착도" },
                { "ship": "현장 도착도", "ship_kr": "현장 도착도" },
            ]
        });

        var isTax = Ext.create('Ext.data.Store', {
            fields: ['is_tax', 'is_tax_kr'],
            data: [
                { "is_tax": "Y", "is_tax_kr": "포함" },
                { "is_tax": "N", "is_tax_kr": "별도" },
            ]
        });
        ...
        var codeYnStore = Ext.create('Mplm.store.CommonCodeStore', { parentCode: 'YN' });
        var form = Ext.create('Ext.form.Panel', {
            id: gu.id('formPanel'),
            xtype: 'form',
            frame: false,
            ...
            items: [
                {
                    ...
                    items: [
                        ...
                        },
                        {
                            ...
                            id: gu.id('reserved_number1'),
                            mode: 'local',
                            anchor: '100%',
                            width: '99%',
                            editable: true,
                            displayField: 'wa_name',
                            store: Ext.create('Mplm.store.CombstStore', { blocking_flag: 'N' }),
                            ...
                            listeners: {
                                change: function (fieldObj, e) {
                                    var buyer = gu.getCmp('reserved_number1').getValue();
                                    console_logs('>>> buyer', buyer);
                                    gu.getCmp('buyer_name').setValue(buyer);
                                    //if (e.getKey() == Ext.EventObject.ENTER) {
                                    // gm.me().redrawSearchStore();
                                    //srchSingleHandler (store, srchId, fieldObj, isWild);
                                    //}
                ...
                },
                {
                    ...
                    items: [
                        ...
                        },
                        {
                            ...
                            listConfig: {
                                getInnerTpl: function () {
                                    return '<div data-qtip="{is_tax}">{is_tax_kr}</div>';
                                }
                            },
                            width: '99%'
                        },
                    ...
                    handler: function (btn) {
                        if (btn == 'no') {
                            ...
                            if (form.isValid()) {
                                ...
                                form.submit({
                                    submitEmptyText: false,
                                    url: CONTEXT_PATH + '/purchase/prch.do?method=insertEstCHMR&copy_flag=N',
                                    waitMsg: '데이터를 저장중입니다.',
                                    params: {
                                        jsonData: jsonData
                                    },
    ...
    getPrdEstimate: function () {
        var action = Ext.create('Ext.Action', {
            ...
            handler: function (widget, event) {
                ...
                //     var delivery_date_isnull = gu.getCmp('delivery_dates').getValue();
                //     //var rec = gm.me().grid.getSelectionModel().getSelected().items[0];
                //    // console_logs("테스트3",rec);
                //     // var reserved_varcharb = rec.get('del');
                //     if (delivery_date_isnull == null) {
                //         Ext.MessageBox.alert('알림', '납기일이 입력되지 않았습니다.');
                //         return;
                //     }
                ...
                var searchItemGrid = Ext.create('Ext.grid.Panel', {
                    ...
                    columns: [
                        ...
                        { text: "규격", flex: 2, style: 'text-align:center', dataIndex: 'desc_concat_spec', sortable: true },
                    ],
                    multiSelect: true,
                    pageSize: 100,
                    width: 700,
                ...
                var saveForm = Ext.create('Ext.grid.Panel', {
                    ...
                    columns: [
                        { text: gm.me().getMC('msg_product_add_dia_model', '제품명'), flex: 1, style: 'text-align:center', dataIndex: 'item_name', sortable: true },
                        //{ text: gm.me().getMC('msg_order_grid_prd_desc', '기준모델'), flex: 1, dataIndex: 'description', style: 'text-align:center', sortable: true },
                        { text: gm.me().getMC('msg_product_add_search_field3', '규격'), flex: 2, dataIndex: 'specification', style: 'text-align:center', sortable: true },
                        {
                            text: gm.me().getMC('msg_order_grid_quan_desc', '수량'),
                            ...
                            handler: function (btn) {
                                ...
                                if (btn == "no") {
                                    ...
                                    if (store_cnt === 0) {
                                        ...
                                        for (var j = 0; j < previous_store.length; j++) {
                                            var item = previous_store[j];
                                            console_logs('sales_amount_????', Number(item.get('bm_quan')));
                                            console_logs('sales_price_????', Number(item.get('sales_price')));
                                            total_quan = Number(total_quan) + Number(item.get('bm_quan'));
                                            total_price = Number(total_price) + (Number(item.get('sales_price')) * Number(item.get('bm_quan')));
                                        }
            ...
            handler: function (widget, event) {
                ...
                var searchItemGrid = Ext.create('Ext.grid.Panel', {
                    ...
                    columns: [
                        ...
                        { text: "제품명", flex: 0.8, style: 'text-align:center', dataIndex: 'item_name', sortable: true },
    ...
    codeYnStore: Ext.create('Mplm.store.CommonCodeStore', { parentCode: 'YN' }),
    searchDetailStore: Ext.create('Mplm.store.ProductDetailSearchStore', { autoLoad: true }),
    searchDetailStoreSrcMap: Ext.create('Mplm.store.ProductPriceInfoStore', { autoLoad: true }),
