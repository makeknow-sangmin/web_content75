//자재 관리
Ext.define('Rfx2.view.company.tosimbio.designPlan.ProductMgmtView', {
    ...
    initComponent: function () {
        ...
        this.addCallback('CHECK_CODE', function (o) {
            ...
            if (code.length < 1) {
                Ext.Msg.alert('안내', '코드를 입력하시기 바랍니다', function () { });
            } else {
                Ext.Ajax.request({
        ...
        this.addCallback('GET_MID_CODE', function (o) {
            var top_level = '';
            console_logs('>>>>>>> zzzzz', 'zaawasdfasdfasd');
            top_level = Ext.getCmp('design-plan-' + gm.me().link + '-class_code');
            console_logs('>>>>> 1. ', top_level.getValue());
            var store = top_level.getStore();

            // var mid_level_code = Ext.getCmp('design-plan-' + gm.me().link + '-mid_level_code');
            // var mid_level_code = mid_level_code.getStore();

            // top_level.setValue(null);
            mid_level_code.getProxy().setExtraParam('parent_class_code', top_level.getValue());

            store.load();
            mid_level_code.load();
        });

        this.addCallback('GET_MID_CLASS_CODE', function (o) {
            console_logs('>>>>>>> zzzzz', 'aazzzzzzz');
            var sg_code = Ext.getCmp('design-plan-' + gm.me().link + '-sg_code');
            var store = sg_code.getStore();

            sg_code.setValue(null);
            var mid_level_code = Ext.getCmp('design-plan-' + gm.me().link + '-class_code');
            store.getProxy().setExtraParam('parent_class_code', mid_level_code.getValue());
            store.load();
        ...
        //버튼 추가.
        buttonToolbar.insert(4, this.productDeleteAction);
        //buttonToolbar.insert(5, this.changeUsingStatusAction);
