//설비현황
Ext.define('Rfx2.view.company.bioprotech.equipState.VcmSettingMgmtView', {
    ...
    initComponent: function () {
        ...
        this.createStore('Rfx2.model.company.bioprotech.ProductMgmtVcm', [{
                property: 'unique_id',
                direction: 'DESC'
            }],
            gm.pageSize
        ...
        /*
        Ver1. - AssyMap으로 레시피 구성
        Ver2. - BomDtl로 레시피 구성
         */
        if (VER_BOM <= 1) {
            this.bomListStore = Ext.create('Rfx2.store.company.bioprotech.BomRecipeStore', {not_pl_no: '---'});
        } else {
            this.bomListStore = Ext.create('Rfx2.store.company.bioprotech.BomRecipeVerStore', {not_pl_no: '---'});
        }
        ...
        this.saveAction = Ext.create('Ext.Action', {
            ...
            handler: function () {
                ...
                for (var i = 0; i < store.getCount(); i++) {
                    ...
                    var tableName = 'bomdtl';
                    var uniqueValue = rec.get('unique_id_long');

                    if (VER_BOM <= 1) {
                        tableName = 'assymap';
                        uniqueValue = rec.get('unique_uid');
                    }

                    gm.editAjax(tableName, 'disp_order', rec.get('recipe_order'), 'unique_id', uniqueValue, {type: ''});
                    gm.editAjax(tableName, 'reserved8', rec.get('reserved8'), 'unique_id', uniqueValue, {type: ''});
                    //gm.editAjax('assymap', 'reserved9', rec.get('reserved9'), 'unique_id', rec.get('unique_uid'), {type: ''});
                    gm.editAjax(tableName, 'reserved11', rec.get('reserved11'), 'unique_id', uniqueValue, {type: ''});
                    gm.editAjax(tableName, 'reserved12', rec.get('reserved12'), 'unique_id', uniqueValue, {type: ''});
                }
        ...
        this.gridTemplateBuffer = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                },
                {
                    text: '기준량',
                    width: 100,
                    style: 'text-align:center', /*editor: 'textfield', */
                    dataIndex: 'bm_quan',
                    sortable: false
                },
                {
        ...
        this.setGridOnCallback(function (selections) {
            if (selections.length > 0) {
                ...
                if (VER_BOM <= 1) {
                    ...
                    gm.me().bomListStore.getProxy().setExtraParam('ver_child', child);
                }
