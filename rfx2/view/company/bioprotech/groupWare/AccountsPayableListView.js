Ext.define('Rfx2.view.company.bioprotech.groupWare.AccountsPayableListView', {
    ...
    initComponent: function () {
        ...
        this.toogleBillCompleteState = Ext.create('Ext.Action', {
            ...
            handler : function () {
                //  gMain.selPanel.doRequestProduce();
                let select = gm.me().grid.getSelectionModel().getSelection();
                let isProcess = true;
                for (let i = 0; i < select.length; i++) {
                    let selects = select[i];
                    let state = selects.get('state');
                    if (state === 'A') {
                ...
                console_logs('>>>> aaaaa', isProcess);

                if (isProcess === true) {
                    Ext.MessageBox.show({
                        title  : '최종확정 / 취소',
                        msg    : '선택 항목들에 대하여 확정 또는 취소처리를 하시겠습니까?',
                        buttons: Ext.MessageBox.YESNO,
    ...
    toogleBillCompleteAction: function (result) {
        if (result == 'yes') {
            console_logs('>>>> toogleBillCompleteAction', 'OK');
            let select = gm.me().grid.getSelectionModel().getSelection();
            if (select == null || select == undefined || select.length < 1) {
                Ext.MessageBox.alert('알림', '계산서가 선택되지 않았습니다.');
                return null;
            }
            console_logs('select.length', select.length);
