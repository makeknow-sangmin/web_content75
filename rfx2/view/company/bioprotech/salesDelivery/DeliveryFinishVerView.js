//생산완료 현황
Ext.define('Rfx2.view.company.bioprotech.salesDelivery.DeliveryFinishVerView', {
    ...
    initComponent: function () {
        ...
        this.poPrdDetailStore = Ext.create('Rfx2.store.company.bioprotech.PoPrdShipmentPackingListVerStore', {});
        ...
        this.attcheContainerAction = Ext.create('Ext.Action', {
            ...
            disabled: false,
            hidden: gu.setCustomBtnHiddenProp('attcheContainerAction'),
            handler: function () {
        ...
        this.assignContainerAction = Ext.create('Ext.Action', {
            ...
            disabled: true,
            hidden: gu.setCustomBtnHiddenProp('assignContainerAction'),
            handler: function () {
                var rec = gm.me().grid.getSelectionModel().getSelection()[0];
                if (rec.get('reserved_varchar1') === 'OC') {
        ...
        this.deliveryFinishAction = Ext.create('Ext.Action', {
            ...
            hidden: gu.setCustomBtnHiddenProp('deliveryFinishAction'),
            handler: function () {
                                    ...
                                    fn: function (btn) {
                                        if (btn == "no") {
                                            ...
                                            Ext.Ajax.request({
                                                ...
                                                url: CONTEXT_PATH + '/sales/delivery.do?method=makedeliveryCompleteMultiSloast',
                                                params: {
                                                ...
                                                success: function (result, request) {
                                                    ...
                                                    gm.me().stop_msg();
                                                    if(resultText === 'true') {
                                                        Ext.MessageBox.alert('알림', '납품처리가 완료되었습니다');
                                                        gm.me().store.load();
                                                        gm.me().poPrdDetailStore.load();
                                                    } else {
                                                        Ext.MessageBox.alert('알림', '적재된 제품의 재고가 납품수량에 충족되지 않아<br>납품처리가 실행되지 않았습니다.<br>다시 확인해주세요.');
                                                        gm.me().store.load();
                                                        gm.me().poPrdDetailStore.load();
                                                    }
        ...
        this.gridContractCompany = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                {
                    text: this.getMC('msg_order_grid_prd_fam', '납품여부'),
                    width: 100,
                    style: 'text-align:center',
                    dataIndex: 'delivery_process_yn',
                    sortable: true
                },
                {
                    text: this.getMC('msg_order_grid_prd_fam', 'Pallet No'),
                    width: 100,
                    style: 'text-align:center',
                    dataIndex: 'po_no',
                    sortable: true
                ...
                {
                    text: this.getMC('msg_order_grid_prd_name', '기준모델'),
                    width: 120,
                    style: 'text-align:center',
                ...
                {
                    text: this.getMC('msg_order_grid_prd_name', 'LOT NO'),
                    width: 100,
                    style: 'text-align:center',
                    dataIndex: 'lot_no',
                    sortable: true
        ...
        buttonToolbar.insert(1, this.deliveryFinishAction);
        buttonToolbar.insert(2, this.downloadSheetAction);
        buttonToolbar.insert(3, this.attcheContainerAction);
        ...
        this.setGridOnCallback(function (selections) {
            if (selections.length) {
                ...
                if (rec.get('dl_uids').length > 0) {
                    ...
                    this.poPrdDetailStore.getProxy().setExtraParam('limit', 1000);
        ...
        this.store.getProxy().setExtraParam('pallet_load_complete', true);
