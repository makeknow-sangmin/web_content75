//수주등록된 전체 제품 메뉴
Ext.define('Rfx2.view.company.bioprotech.salesDelivery.PackageOrderView', {
    ...
    initComponent: function () {
        ...
        this.addSearchField({
            ...
            items: [
                {
                    boxLabel: gm.getMC('CMD_Closed', 'LOT마감'),
                    checked: false
                },
            ],
        });
        ...
        this.addSearchField({
            ...
            text: gm.getMC('CMD_Start_Date', '예정일'),
            labelWidth: 37,
        ...
        this.addSearchField({
            type: 'combo',
            width: 150
            , field_id: 'pcsmchn_pkg_uid'
            , emptyText: gm.getMC('CMD_Paking_Line', '포장라인')
            , store: "MachinePkgStore"
            , displayField: 'name_ko'
            , valueField: 'unique_id_long'
            , value: 'unique_id_long'
            , innerTpl: '<div data-qtip="{unique_id_long}">{name_ko} [{mchn_code}]</div>'
        });
        ...
        this.createStoreSimple({
            modelClass: 'Rfx2.model.company.bioprotech.PackageWork',
            pageSize: 100,
            sorters: [{
        ...
        this.inAction = Ext.create('Ext.Action', {
            ...
            text: gm.getMC('CMD_GR_PROCESS', '입고처리'),
            tooltip: '입고처리',
            disabled: false,
                        ...
                        buttons: [
                            {
                                ...
                                handler: function () {
                                    ...
                                    if (selection.get('pj_type') === 'A' || selection.get('pj_type') === 'AO') {
                                        ...
                                        console_logs('gu.mesTplProcessAll[gu.mesTplProcessBig[0].code].length - 1 >>', gu.mesTplProcessAll[gu.mesTplProcessBig[1].code].length - 1);
                                        var maxPcsCode = gu.mesTplProcessAll[gu.mesTplProcessBig[0].code][gu.mesTplProcessAll[gu.mesTplProcessBig[1].code].length - 1];
                                        ...
                                        if (cur_gr_quan + gr_quan > Number(outpcsQty)) {
                                            ...
                                            if (packingArr.length === 0 || boxqtyArr.length === 0 || totalqtyArr.length === 0) {
                                                ...
                                                console_logs('>>>>> kkkkkkkk', remain_qty);
                                                if (remain_qty == 0) {
                                                    Ext.Ajax.request({
        ...
        this.doProductionAction = Ext.create('Ext.Action', {
            iconCls: 'mfglabs-step_forward_14_0_5395c4_none',
            text: gm.getMC('CMD_INPUT_PERFORMANCE', '실적입력'),
            ...
            handler: function () {
                ...
                var radioValue = {
                    boxLabel: pcs_codes[1].name,
                    name: 'pcs_radio',
                    inputValue: selection.get(/*pcs_codes[i].code*/processCode + 0 + '|step_uid'),
                ...
                var process = selection.get(/*pcs_codes[i].code*/processCode + 0 + '|process_qty');
                var outpcs = selection.get(/*pcs_codes[i].code */processCode + 0 + '|outpcs_qty');
                var defaultNum = process - outpcs < 0 ? 0 : process - outpcs;
                console_logs('>>>process ???', process);
                console_logs('>>>outpcs ???', outpcs);
                ...
                if (ngr_quan > 0) {
                                ...
                                items: [
                                    {
                                        ...
                                        listeners: {
                                            change: function (field, newValue, oldValue) {
                                                ...
                                                var stepUid = selection.get(/*pcs_codes[i].code*/processCode + 0 + '|step_uid');

                                                if (stepUid === newValue.pcs_radio) {

                                                    var processQty = selection.get(/*pcs_codes[i].code*/processCode + 0 + '|process_qty');
                                                    var outpcsQty = selection.get(/*pcs_codes[i].code */processCode + 0 + '|outpcs_qty');
                    ...
                    var outpcsQty = selection.get('production_qty_sum');
                    defaultQty = processQty - outpcsQty < 0 ? 0 : processQty - outpcsQty;
        ...
        this.barcodePrintAction = Ext.create('Ext.Action', {
            iconCls: 'barcode',
            text: gm.getMC('CMD_INNERBOX_BARCODE', '이너박스바코드(생산)'),
        ...
        this.backToPrdPlan = Ext.create('Ext.Action', {
            iconCls: 'af-remove',
            text: gm.getMC('CMD_CANCEL_ORDER', '지시취소'),
                                        ...
                                        items: [
                                            {
                                                ...
                                                width: 210,
                                                store: lineStore,
                                                fieldLabel: gm.getMC('CMD_Paking_Line', '포장라인'),
                                                id: gu.id('pkg_line'),
                                                displayField: 'name_ko',
                                                valueField: 'mchn_code',
        ...
        this.completePrdAction = Ext.create('Ext.Action', {
            iconCls: 'mfglabs-step_forward_14_0_5395c4_none',
            text: gm.getMC('CMD_DECIDE_PROCESS', '완료/취소처리'),
        ...
        this.pcsworkDefectStore = Ext.create('Rfx2.store.company.bioprotech.PcsWorkDefectStore', {});
        ...
        this.workListGrid = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                {
                    text: '작업반', width: 80, style: 'text-align:center', dataIndex: 'work_type', sortable: true,
                    renderer: function (value, meta, record) {
                        switch (value) {
                            case 'day':
                                return '주간';
                ...
                {text: '입력자', width: 100, style: 'text-align:center', dataIndex: 'user_name', sortable: true},
                {
        ...
        this.defectListGrid = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                {
                    text: '작업반', width: 75, style: 'text-align:center', dataIndex: 'work_type', sortable: true,
                    renderer: function (value, meta, record) {
                        switch (value) {
                            case 'day':
                                return '주간';
                ...
                {text: '불량유형', width: 100, style: 'text-align:center', dataIndex: 'defect_name', sortable: true},
                {
                    text: '불량수량', width: 80, style: 'text-align:center', dataIndex: 'defect_total_qty', sortable: true,
                    align: 'right',
                    renderer: function (value, meta, record) {
                        return Ext.util.Format.number(value, '0,00/i');
                    }
                },
                {text: '불량코드', width: 100, style: 'text-align:center', dataIndex: 'defect_detail', sortable: true},
                {text: '등록자', width: 80, style: 'text-align:center', dataIndex: 'user_name', sortable: true},
                // { text: '공정명', width: 80, style: 'text-align:center', dataIndex: 'pcs_name', sortable: false },

                {
                    text: '등록시간', width: 130, style: 'text-align:center', dataIndex: 'change_date', sortable: true,
                    renderer: function (value, meta, record) {
                        if (value.length > 16) {
                            return value.substring(0, 16);
                        } else {
        ...
        if (this.tab_info.length == 0) {
            ...
            var tab = this.createTabGrid('Rfx2.model.company.bioprotech.PackageWork', items, 'big_pcs_code', arr, function (curTab, prevtab) {
                currentTab = curTab;
