Ext.define('Rfx2.view.company.bioprotech.salesDelivery.RecvPoDetailVerView', {
    ...
    initComponent: function () {
        ...
        this.addPoAction = Ext.create('Ext.Action', {
            ...
            handler: function () {
                ...
                var fax_no = '';
                var tel_no = '';
                console_logs('>>>>> vLANG', vLANG);
                var display_type = '';
                switch (vLANG) {
                    case 'kr':
                        display_type = 'code_name_kr';
                        break;
                    case 'en':
                        display_type = 'code_name_en';
                        break;
                    case 'zh':
                        display_type = 'code_name_zh';
                        break;
                    default:
                        display_type = 'code_name_kr';
                        break;
                ...
                gm.me().rtgapp_store.getProxy().setExtraParam('change_type', 'D');
                gm.me().rtgapp_store.getProxy().setExtraParam('app_type', 'SA');
                                    ...
                                    items: [
                                        {
                                            ...
                                            listConfig: {
                                                ...
                                                getInnerTpl: function () {
                                                    ...
                                                    return '<div data-qtip="{systemCode}">{' + display_type + '}</div>';
                                                }
                                            },
                                            listeners: {
                                                select: function (combo, record) {
...

                                        {
                                            ...
                                            listeners: {
                                                select: function (combo, record) {
                                                    console_logs('>>> address_1', record.get('address_1'));
                                                    console_logs('>>> nation_code', record.get('nation_code'));
                                                    console_logs('>>> wa_name', record.get('wa_name'));
                                                    ...
                                                    nation_code = record.get('nation_code');
                                                    address_1 = record.get('address_1');
                                                    fax_no = record.get('arap_fax_no');
                                                    tel_no = record.get('site_manager_tel_no');
                                                }// endofselect
                                            }
                                        },
                                        {
                        ...
                        handler: function (btn) {
                            if (btn == "no") {
                                ...
                                if (form.isValid()) {
                                    ...
                                    tempObj['pi_no'] = '';
                                    tempObj['ref_po'] = val['reserved_varchard'];
                                    tempObj['validity'] = '';
                                    tempObj['po_issued_by'] = '';
                                    tempObj['pi_issue_date'] = result;
                                    tempObj['issue_date'] = result;
                                    tempObj['po_recv'] = result;

                                    tempObj['company_name'] = wa_name;
                                    tempObj['country'] = nation_code;
                                    tempObj['address'] = address_1;
                                    tempObj['fax_no'] = fax_no;
                                    tempObj['tel_no'] = tel_no;
                                    tempObj['destination'] = nation_code;
                                    ...
                                    for (var j = 0; j < storeData.data.items.length; j++) {
                                        ...
                                        // Proforma Invoice 송신용.
                                        tempObj['articles_' + (tempPos > 9 ? tempPos : '0' + tempPos)] = item.get('class_name');
                                        tempObj['model_' + (tempPos > 9 ? tempPos : '0' + tempPos)] = item.get('item_name');
                                        tempObj['quantity_' + (tempPos > 9 ? tempPos : '0' + tempPos)] = item.get('sales_amount');
                                        tempObj['unit_code_' + (tempPos > 9 ? tempPos : '0' + tempPos)] = item.get('reserved_varchar9');
                                        tempObj['unit_currency_' + (tempPos > 9 ? tempPos : '0' + tempPos)] = item.get('reserved_varchar8');
                                        tempObj['sales_price_' + (tempPos > 9 ? tempPos : '0' + tempPos)] = item.get('sales_price');
                                        tempObj['unit_dollar_' + (tempPos > 9 ? tempPos : '0' + tempPos)] = item.get('reserved_varchar8');
                                        tempObj['amount_' + (tempPos > 9 ? tempPos : '0' + tempPos)] = Number(item.get('sales_amount')) *  Number(item.get('sales_price'));
                                        if(j === 0) {
                                            tempObj['estimate_time_' + (tempPos > 9 ? tempPos : '0' + tempPos)] = '';
                                            tempObj['payment_cond'] = item.get('item_pancond_kr');
                                            tempObj['shpping_term'] = item.get('item_incoterms');
                                        }
                                        tempObj['ship_by_' + (tempPos > 9 ? tempPos : '0' + tempPos)] = '';

                                        total = total + Number(item.get('sales_amount'));
                                        tempPos++;
                                    }
                                    ...
                                    var salesTemplate = Ext.create('Rfx2.view.company.bioprotech.template.ProformaInvoiceTemplate');

                                    var result = salesTemplate.apply(tempObj);
                                    ...
                                    val['productJson'] = jsonData;
                                    val['tempFileNum'] = ranNum;
                                    val['ahid_userlist'] = ahid_userlist;
                                    val['hid_userlist_role'] = ahid_userlist_role;
                                    val['hid_userlist_id'] = ahid_userlist_id;
                                    val['sales_form_html'] = result;
        ...
        this.editPoAction = Ext.create('Ext.Action', {
            iconCls: 'af-edit',
            text: this.getMC('CMD_ORDER_REVISION', '수주수정'),
            tooltip: this.getMC('CMD_ORDER_REVISION', '수주수정'),
            disabled: true,
            hidden: gu.setCustomBtnHiddenProp('editPoAction'),
            handler: function () {
        ...
        this.completeAction = Ext.create('Ext.Action', {
            ...
            handler: function () {
                ...
                Ext.MessageBox.show({
                    ...
                    msg: gm.getMC('msg_btn_prd_confirm_msg', '수주확정을 하시겠습니까?'),
                    buttons: Ext.MessageBox.YESNO,
        ...
        this.fileattachAction = Ext.create('Ext.Action', {
            ...
            disabled: true,
            text: gm.getMC('CMD_FILE_DOWNLOAD', '파일다운로드'),
        ...
        this.excelUploadAction = Ext.create('Ext.Action', {
            ...
            disabled: true,
            text: gm.getMC('CMD_FILE_UPLOAD', '파일업로드'),
            tooltip: this.getMC('수주등록시 필요문서를 일괄 업로드 합니다.'),
            hidden: gu.setCustomBtnHiddenProp('excelUploadAction'),
            handler: function () {
        ...
        this.readAndPublicStore.getProxy().setExtraParam('v001', 'SA');
            ...
            listeners: {
                edit: function (editor, e, eOpts) {
                    ...
                    for (var i = 0; i < item.length; i++) {
                        ...
                        var sales_amount = item_desc.get('po_qty');
    ...
    getPrdAdd: function () {
        var action = Ext.create('Ext.Action', {
            ...
            handler: function (widget, event) {
                ...
                * */
                gm.me().searchDetailStoreOnlySrcMap.getProxy().setExtraParam('fix_type', 'SE');
                if (gu.getCmp('reserved_varcharb').getValue() === 'N' || gu.getCmp('reserved_varcharb').getValue() === 'P' || gu.getCmp('reserved_varcharb').getValue() === 'C') {
                    if (gu.getCmp('order_com_unique').getValue() == null) {
                        Ext.MessageBox.alert(gm.me().getMC('msg_message_box_alert', '알림'), gm.me().getMC('msg_message_box_poadd_alert2', '거래구분이 정상거래, 유상샘플, 법인거래가 선택 되었을 경우 고객명이 반드시 입력되어야 합니다'));
                        return;
                    } else {
    ...
    stores: [],
    rtgapp_store: Ext.create('Rfx2.store.RtgappStore', {}),
    ingredientList: [],
    storecount: 0,
    // gridContent2: null,
    fields: [],
    deleteRtgappConfirm: function (result) {
        ...
        var selections = gu.getCmp('rtgAppGrid').getSelectionModel().getSelection();
        if (selections) {
            ...
            if (result == 'yes') {
                ...
                gu.getCmp('rtgAppGrid').getStore().remove(selections);
            }
        }
    },
