//수주관리 메뉴
Ext.define('Rfx2.view.company.bioprotech.salesDelivery.RecvMgmtKbTechVerView', {
    ...
    initComponent: function () {
        ...
        this.changePaymentMethod = Ext.create('Ext.Action', {
            iconCls: 'af-edit',
            disabled: true,
            text: this.getMC('CMD_PAYMENT_METHOD', '결제방법 변경'),
            tooltip: '선택한 수주에 대하여 수주변경을 실시합니다.',
            hidden: gu.setCustomBtnHiddenProp('changePaymentMethod'),
            handler: function () {
                var record = gm.me().grid.getSelectionModel().getSelection();
                var rec = record[0];
                ...
                var paymentCodeStore = Ext.create('Mplm.store.PaytermStore');
                paymentCodeStore.load();

                if (rec.get('status_bio') === 'DC') {
                    Ext.MessageBox.alert('알림', '출하 완료상태에서는 수주변경이 불가합니다.');
                    return;
                } else {
                    console_logs('>>>> vCUR_USERTYPE_STR', vCUR_USERTYPE_STR);
                    if(vCUR_USERTYPE_STR.indexOf('SYS') == -1) {
                        Ext.MessageBox.alert('알림', 'SYS권한을 가진 사용자만 사용가능합니다.');
                        return;
                    } else {
                        var form = Ext.create('Ext.form.Panel', {
                            id: 'editPoForm',
                            ...
                            items: [
                                {
                                    xtype: 'fieldset',
                                    collapsible: false,
                                    title: gm.me().getMC('msg_order_dia_payment_title', '변경할 결제방법을 선택하십시오.'),
                                    width: '100%',
                                    style: 'padding:10px',
                                    defaults: {
                                        labelStyle: 'padding:10px',
                                    ...
                                    items: [
                                        {
                                            ...
                                            items: [
                                                {
                                                    xtype: 'combo',
                                                    id: gu.id('payment_code'),
                                                    name: 'reserved3',
                                                    padding: '0 0 5px 30px',
                                                    displayField: 'code_name_kr',
                                                    store: paymentCodeStore,
                                                    valueField: 'system_code',
                                                    allowBlank: false,
                                                    typeAhead: false,
                                                    hideLabel: false,
                        ...
                        var win = Ext.create('Ext.Window', {
                            modal: true,
                            title: gm.me().getMC('CMD_PAYMENT_METHOD', '결제방법 변경'),
                            width: 500,
                            height: 200,
                            plain: true,
                            items: form,
                                ...
                                handler: function (btn) {
                                    if (btn == "no" || btn == "cancel") {
                                        ...
                                        if (form.isValid()) {
                                            ...
                                            form.submit({
                                                submitEmptyText: false,
                                                url: CONTEXT_PATH + '/production/schdule.do?method=changePaymentMethod',
                                                params: {
                                                    unique_id_long : rec.get('unique_uid'),
        ...
        buttonToolbar.insert(7, this.changePaymentMethod);
        ...
        this.setGridOnCallback(function (selections) {
            if (selections.length > 0) {
                ...
                this.changePaymentMethod.enable();
            } else {
                ...
                this.changePaymentMethod.disable();
            }
        });
