 ...
 *
 */
Ext.define('Rfx2.view.company.bioprotech.purStock.CreatePoOutView', {
    ...
    initComponent: function () {
        ...
        this.pageSize = gm.unlimitedPageSize;
        this.createStoreSimple({
            modelClass: 'Rfx2.model.company.bioprotech.AssyModel',
        ...
        this.newPoCartListStore = Ext.create('Rfx2.store.company.bioprotech.AssyStockAllStore', {});

        this.whouseStore = Ext.create('Ext.data.Store', {
            fields: ['whouse_uid', 'dept_code'],
            data: [
                {"whouse_uid": "11030245000005", "name": "전북반도체", "dept_code": "BPH804"},
                {"whouse_uid": "11030245000006", "name": "피아이프로텍"},
                {"whouse_uid": "11030245000007", "name": "대산 S&T"},
                {"whouse_uid": "11030245000008", "name": "에이원", "dept_code": "BPH803"},
                {"whouse_uid": "11030245000009", "name": "엘스엘테크"},
                {"whouse_uid": "11030245000010", "name": "세신산업"},
                {"whouse_uid": "11030245000011", "name": "한국코스틱"},
                {"whouse_uid": "11030245000012", "name": "새한씰크"},
                {"whouse_uid": "11030245000013", "name": "씨테크"}
            ]
        });
        ...
        this.createPoOutAction = Ext.create('Ext.Action', {
            ...
            handler: function () {
                ...
                var cnt = gm.me().poCartListStore.getCount();

                if (cnt > 0) {
                    gm.me().createPoOut();
                } else {
                    Ext.Msg.alert('', '하위 자재 리스트가 적어도 한 개 이상이어야 합니다.');
                }
        ...
        this.gridPoList = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                {
                    text: 'MOQ', width: 60, dataIndex: 'moq', align: 'right',
                ...
                {
                    text: '단위수량', width: 80, dataIndex: 'unit_mass', align: 'right',
                    renderer: function (value) {
                        return Ext.util.Format.number(value, '0,00/i');
                    }
                },
                {
                    text: '소요량', width: 60, dataIndex: 'bm_quan', align: 'right'
                },
        ...
        this.setGridOnCallback(function (selections) {
            if (selections.length) {
                ...
                gm.me().newPoCartListStore.getProxy().setExtraParam('whouse_uid', '11030245000002');
                gm.me().newPoCartListStore.load();
            } else {
    ...
    createPoOut: function () {
        this.partListGrid = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                {
                    text: '단위수량', width: 60, dataIndex: 'unit_mass', align: 'right',
                    renderer: function (value) {
                        return Ext.util.Format.number(value, '0,00/i');
                    }
                },
                ...
                {
                    text: '외주사', width: 85, hidden: false, dataIndex: 'quan_01',
                    renderer: function (value) {
                        return Ext.util.Format.number(value, '0,00/i');
                    }
                }
                    ...
                    items: [
                        {
                            ...
                            listeners: {
                                select: function (combo, record) {
                                    ...
                                    gm.me().newPoCartListStore.getProxy().setExtraParam('whouse_uid_out', record.get('whouse_uid'));
                                    gm.me().newPoCartListStore.load();
                        ...
                        Ext.MessageBox.show({
                            ...
                            fn: function (result) {
                                if (result == 'yes') {
                                    ...
                                    var reserved_varchar6 = /*gu.getCmp('reserved_varchar6').getValue();*/ null;
                                    var dept_code = '';
                                    ...
                                    Ext.Ajax.request({
                                        ...
                                        success: function () {
                                            ...
                                            Ext.Msg.alert('', '외주주문 요청이 완료 되었습니다.');
                                            if (productWin) {
                                                productWin.close();
                                            }
                                        },
                                        failure: function () {
                                            productWin.setLoading(false);
                                            Ext.Msg.alert('', '외주주문 요청중 문제가 발생하였습니다.');
                                            if (productWin) {
                                                productWin.close();
                                            }
                                        }
