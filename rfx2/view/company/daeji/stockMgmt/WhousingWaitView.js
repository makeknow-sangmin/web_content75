...
Ext.define('Rfx2.view.company.daeji.stockMgmt.WhousingWaitView', {
    ...
    initComponent: function () {
                ...
                Ext.MessageBox.show({
                    ...
                    fn: function (btn) {
                        ...
                        for (let i = 0; i < selections.length; i++) {
                            let unique_id = selections[i].get('unique_id_long');
        ...
        this.grid.on('edit', function (editor, e) {
            ...
            let field = e.field;
            let gr_qty = gm.me().grid.getSelectionModel().getSelection()[0].get('curGr_qty');
            ...
            switch (field) {
                case 'curGr_qty':
                    ...
                    let selectionInner = gm.me().grid.getSelectionModel().getSelection();
                    let recInner = selectionInner[0];
                    selectionInner[0].set('curGr_qty', recInner.get(field));
                    if (ctr_flag == 'M') {
                        selectionInner[0].set('sales_amount', Math.floor(selectionInner[0].get('sales_price') * selectionInner[0].get('mass')));
                    } else {
                        selectionInner[0].set('sales_amount', Math.floor(recInner.get(field) * selectionInner[0].get('sales_price')));
                    }
                ...
                case 'sales_price':
                    ...
                    let ctr_flag = selection[0].get('ctr_flag');
                    if (ctr_flag == 'M') {
            ...
            let rec = e.record;
            let request_qty = rec.get('request_qty');
            let sales_price = rec.get('sales_price');
            let po_no = rec.get('po_no');
            let item_name = rec.get('item_name');
            let unique_id = rec.get('unique_id_long');
            let pj_code = rec.get('pj_code');

            let unit_code = rec.get('unit_code');
            let sales_amount = rec.get('sales_amount');
    ...
    treatGr: function () {
        ...
        comboWhouse.store.load(
            function () {
                this.each(function (record) {
                    let wh_code = record.get('wh_code');
                    if (wh_code == 'WH200') {
