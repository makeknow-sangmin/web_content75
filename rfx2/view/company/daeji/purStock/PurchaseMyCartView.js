//자재 관리
Ext.define('Rfx2.view.company.daeji.purStock.PurchaseMyCartView', {
                ...
                if (isExistAssy && !isExistOthers && selections.length > 1) {
                    ...
                    if (selections.length) {
                        ...
                        for (var i = 0; i < selections.length; i++) {
                            ...
                            var obj = {};
                            var o = selections[i];
                            var pl_no = o.get('pl_no');
                            if (pl_no !== null) {
                                var plnoNum = Number(pl_no);
                                obj['num'] = plnoNum;
                                obj['pos'] = i;
                                poses_pl_no.push(obj);
                            } else {
                                poses_not_pl_no.push(i);
                            }
                        }

                        poses_pl_no = poses_pl_no.sort(function (a, b) {
                            var x = a.num;
                            var y = b.num;
                            if (x < y) {
                                return -1;
                        ...
                        for (var i = 0; i < poses_not_pl_no.length; i++) {
                            var o = selections[poses_not_pl_no[i]];
                        ...
                        for (var i = 0; i < poses_pl_no.length; i++) {
                            var o = selections[poses_pl_no[i]['pos']];
                            var srcahd_uid = o.get('unique_id');
                            var mycart_uid = o.get('id');
                            var pr_quans = o.get('pr_quan');
                            var child = o.get('child');
