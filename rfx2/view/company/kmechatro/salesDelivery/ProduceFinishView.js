//생산완료 현황
Ext.define('Rfx2.view.company.kmechatro.salesDelivery.ProduceFinishView', {
    ...
    initComponent: function () {
        ...
        this.assignShipmentAction = Ext.create('Ext.Action', {
            ...
            text:  this.getMC('ppo1_request', '출하 요청'),
            tooltip: '출하 요청',
            disabled: true,
            handler: function () {
                ...
                var form = Ext.create('Ext.form.Panel', {
                    ...
                    items: [
                        {
                            xtype: 'fieldset',
                            title: gm.me().getMC('msg_order_request_pop_up_title_announce', '출하요청일과 운송방법을 입력하시기 바랍니다.'),
                            items: [
                                {
                                    xtype: 'datefield',
                                    id: gu.id('req_date'),
                                    ...
                                    format: 'Y-m-d',
                                    value: new Date(),
                                    fieldLabel:  gm.me().getMC('msg_order_request_pop_up_release_msg', '출하요청일')
                                },
                                {
                                    xtype: 'combo',
                                    fieldLabel: gm.me().getMC('msg_order_request_pop_up_transportation', '운송방법'),
                                    ...
                                    emptyText: gm.me().getMC('msg_common_emptytext_select', '운송방법'),
                                    listConfig: {
                                        loadingText: '검색중...',
                ...
                var prWin = Ext.create('Ext.Window', {
                    modal: true,
                    title: gm.me().getMC('msg_order_request_pop_up_release_title', '출하예정등록'),
                    width: 450,
                    height: 210,
                    items: form,
                    buttons: [
                                ...
                                Ext.MessageBox.show({
                                    ...
                                    fn: function (btn) {
                                        if (btn == "no") {
                                            ...
                                            Ext.Ajax.request({
                                                url: CONTEXT_PATH + '/sales/productStock.do?method=requestShipment',
                                                params: {
                                                    acUid: -1,
                                                    prQuanArr: prQuanArr,
            ...
            handler: function (widget, event) {
                ...
                for (var i = 0; i < selections.length; i++) {
                    ...
                    if (pr_quan === 0) {
                        ...
                        var unique_id = rec.get('assymap_uid');
        ...
        this.cartGoReq = Ext.create('Ext.Action', {
            xtype: 'button',
            iconCls: 'mfglabs-step_forward_14_0_5395c4_none',
            text:  this.getMC('msg_car_request', '카트일괄요청'),
                                        ...
                                        if (btn == "no") {
                                            ...
                                            if (length > 0) {
                                                ...
                                                for (var i = 0; i < storeData.data.items.length; i++) {
                                                    ...
                                                    assyMapArr.push(item.get('assymap_uid'));
        ...
        this.gridContractCompany = Ext.create('Ext.grid.Panel', {
            ...
            dockedItems: [
                ...
                },
                {
                    ...
                    items: [{
                        xtype: 'triggerfield',
                        emptyText: this.getMC('msg_product_add_dia_model', '제품명'),
                        id: gu.id('item_name'),
                        ...
                        xtype: 'triggerfield',
                        emptyText: this.getMC('mes_sro5_pln_column_ecustomer', '최종고객사'),
                        ...
                        xtype: 'triggerfield',
                        emptyText: this.getMC('msg_customer_po', '고객PO번호'),
                    ...
                    {
                        xtype: 'triggerfield',
                        emptyText: this.getMC('msg_order_no', '수주번호'),
            ...
            columns: [
                { text: this.getMC('msg_order_no', '수주번호'), width: 100, style: 'text-align:center', dataIndex: 'orderNumber'/*, sortable: false*/ },
                { text: this.getMC('msg_order_dia_order_end_customer', '최종고객사'), width: 100, style: 'text-align:center', dataIndex: 'final_wa_name'/*, sortable: false*/ },
                {
                    text: this.getMC('msg_qtv', '요청수량'), width: 95, style: 'text-align:center', dataIndex: 'ap_Wquan', align: 'right',
                    editor: 'numberfield',
                    renderer: function (value, context, tmeta) {
                        return Ext.util.Format.number(value, '0,00/i');
                    },
                ...
                { text: this.getMC('toolbar_pj_product_type', gm.getMC('CMD_Product', '제품군')), width: 80, style: 'text-align:center', dataIndex: 'class_name'/*, sortable: false*/ },
                { text: this.getMC('msg_product_add_dia_model', '제품명'), width: 100, style: 'text-align:center', dataIndex: 'item_name'/*, sortable: false*/ },
                { text: this.getMC('msg_base_model', '기준모델'), width: 100, style: 'text-align:center', dataIndex: 'description'/*, sortable: false*/ },
                {
                    text: this.getMC('msg_order_grid_prd_delivery_date', '고객요청일'), width: 100, style: 'text-align:center', dataIndex: 'reserved_timestamp1'/*, sortable: false*/,
                    renderer: function (value, context, tmeta) {
                ...
                { text: this.getMC('msg_released_orderqtv', '요청된수량/오더수량'), width: 150, style: 'text-align:center', dataIndex: 'apBmQuan', align: 'right' },
                { text: this.getMC('msg_customer_po', '고객PO번호'), width: 120, style: 'text-align:center', dataIndex: 'project_varchard' },
                { text: this.getMC('msg_remarks', '수주특기사항'), width: 150, style: 'text-align:center', dataIndex: 'reserved1' },
                ...
                {
                    text: this.getMC('msg_order_order_date', '수주일자'), width: 100, style: 'text-align:center', dataIndex: 'regist_date'/*, sortable: false*/,
                    renderer: function (value, context, tmeta) {
                        if (value !== null && value.length > 0) {
                            return value.substring(0, 10);
                        }
    ...
    cartDoListStore: Ext.create('Rfx2.store.company.bioprotech.CartDoListStore'),
});