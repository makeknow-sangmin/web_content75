//자재 관리
Ext.define('Rfx2.view.company.kbtech.purStock.MyCartGoView', {
        ...
        this.reReceiveAction = Ext.create('Ext.Action', {
            ...
            handler: function () {
                ...
                for (var i = 0; i < selections.length; i++) {
                    ...
                    tempoary_lot_no.push(rec.get('lot_no'));
                }

                console_logs('tempoary_lot_no >>', tempoary_lot_no);
                var isLot = false;
                for (var j = 0; j < tempoary_lot_no.length; j++) {
                    if (j > 0) {
                        var before_lot_no = tempoary_lot_no[j - 1];
                        var after_lot_no = tempoary_lot_no[j];
                        if (before_lot_no === after_lot_no) {
                            isLot = true;
                        } else {
                            isLot = false;
                            break;
                ...
                if (isLot === true) {
                    ...
                    Ext.MessageBox.alert('알림', 'LOT 번호가 중복되지 않습니다.');
                }
    ...
    updateDesinComment: function (rec) {
        ...
        var nstock_wh_qty = rec.get('nstock_wh_qty');

        if (reserved1 == 'Y') {
            stock_qty = nstock_wh_qty + old_value;
        } else {
            stock_qty = nstock_wh_qty;
        }
        console_logs('====> 올드값이 더해진 stock_qty', stock_qty);
        console_logs('====> assymap_uid', assymap_uid);

        if (pr_quan > nstock_wh_qty) {
            Ext.Msg.alert('다시 입력', '요청수량은 총재고를 초과할 수 없습니다.');
