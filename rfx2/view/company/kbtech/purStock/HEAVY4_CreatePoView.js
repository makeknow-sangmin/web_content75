 ...
 *
 */
Ext.define('Rfx2.view.company.kbtech.purStock.HEAVY4_CreatePoView', {
    ...
    initComponent: function () {
        ...
        this.goCartListStore.on('load', function (store, records, successful, eOpts ){
            console_logs('goCartListStore -------------------------------------------', '');
            console_logs('goCartListStore records', records);
            console_logs('goCartListStore -------------------------------------------', '');
        ...
        this.gridCartList = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                {text: '사용유무', width: 80, align: 'center',
                    xtype: 'checkcolumn',
                    dataIndex: 'isUseItem',
                    listeners: {
                        checkchange: function(comp, rowIndex, checked, eOpts) {
                            var record = this.up('grid').getStore().getAt(rowIndex);
                            gm.editAjax('assymap', 'req_flag', checked ? 'N' : 'Y', 'unique_id', record.get('coord_key3'), {type: ''});
                            gm.editAjax('cartmap', 'req_flag', checked ? 'N' : 'Y', 'unique_id', record.id, {type: ''});
                ...
                {text: '공급사', dataIndex: 'contract_supplier', width: 250},
                {text: '소요량', dataIndex: 'pr_quan', editor: 'numberfield'},
                //{text: '요청수량', dataIndex: 'pr_quan'},
                {text: '총재고', dataIndex: 'wh_qty', width:0},
                {text: '가용재고', dataIndex: 'stock_qty_useful'},
                {text: '주문수량', dataIndex: 'new_po_quan'/*, editor: 'numberfield'*/},
                {text: '주문단가', dataIndex: 'new_sales_pice', editor: 'textfield'},
                {text: '주문금액', dataIndex: 'new_total_price'},
                {
            ...
            listeners: {
                ...
                edit: function (editor, e, eOpts) {
                    ...
                    var _quan = rec.get('pr_quan') / rec.get('bm_quan');
                    ...
                    switch (columnName) {
                        case 'bm_quan':
                            ...
                            break;
                        case 'quan':
                            gm.editAjax(tableName, 'quan', value, 'unique_id', unique_id, {type: ''});
                            break;
                        default:
                            break;
                    }
        ...
        this.gridPoList = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                {text: '품명', dataIndex: 'item_name'},
                {text: '규격', dataIndex: 'specification'},
                {text: '공급사', dataIndex: 'supplier_name',  width: 250,
                    editor: {
                        xtype: 'combo',
                ...
                {text: '요청수량', dataIndex: 'pr_quan'},
                {text: '총재고', dataIndex: 'stock_qty', width:0},
                {text: '가용재고', dataIndex: 'stock_qty_useful'},
                {text: '주문수량', dataIndex: 'new_po_quan', editor: 'numberfield'},
                {text: '주문단가', dataIndex: 'new_sales_pice', editor: 'textfield'},
                {text: '주문금액', dataIndex: 'new_total_price'},
        ...
        this.gridGoList = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                {text: '총재고', dataIndex: 'stock_qty', width:0},
                {text: '가용재고', dataIndex: 'stock_qty_useful'},
                {text: '불출수량', dataIndex: 'go_quan', editor: 'numberfield'},
                {text: '주문단가', dataIndex: 'new_sales_pice', editor: 'textfield', hidden: true},
                {text: '주문금액', dataIndex: 'new_total_price', hidden: true},
                {
                    text: '납기일', type: 'date', dataIndex: 'req_date', dateFormat: 'Y-m-d',
                    editor: {
                        xtype: 'datefield',
    ...
    //             var quan = rec.get('new_po_quan');
    //             var new_sales_pice = rec.get('new_sales_pice');
    //             if (quan < 0.0000001) {
    //                 qty_is_zero++;
    //             }
    //             if (new_sales_pice < 0.0000001) {
    //                 price_is_zero++;
    //             }
    ...
    //             for (var i = 0; i < selections.length; i++) {
    //                 var rec = selections[i];
    //                 var total_price = rec.get('new_total_price');
    //                 total = total + total_price;
    ...
    treatPo: function (po_no) {
        ...
        if (uniqueId == undefined || uniqueId < 0) {
            ...
            for (var i = 0; i < selections.length; i++) {
                ...
                pjArr.push(rec.get('pj_code'));
                supArr.push(rec.get('coord_key1'));
                cartmapUids.push(rec.get('id'));

                var quan = rec.get('new_po_quan');
                var new_sales_pice = rec.get('new_sales_pice');
                
                new_po_quans.push(quan);
                new_sales_prices.push(new_sales_pice);
            ...
            console_logs('supArr', supArr);
            console_logs('cartmapUids', cartmapUids);
            console_logs('new_sales_prices', new_sales_prices);
            console_logs('new_po_quans', new_po_quans);
            ...
            if (pjArr.length > 1 && gm.me().canDupProject == false) {
                ...
                Ext.Msg.alert('알림', '공급사를 지정하지 않은 품목이 있습니다.', function () {
                });
            } else {
                var next = gUtil.getNextday(0);
                ...
                for (var i = 0; i < selections.length; i++) {
                    ...
                    if (vCompanyReserved4 == 'KWLM01KR') {
                        ...
                        var rec = selections[i];
                        total_price = rec.get('new_total_price');
                    }
                    total = total + total_price
                }
                ...
                var formItems = [{
                    ...
                    items: [
                        ...
                        },
                        {
                            ...
                            emptyText: '선택',
                            value: gm.me().gridPoList.getSelectionModel().getSelection()[0].get('coord_key1'),
                            allowBlank: false,
                            sortInfo: {
                                field: 'create_date',
                                direction: 'DESC'
                        ...
                        new Ext.form.Hidden({
                            ...
                            name: 'new_sales_prices',
                            value: new_sales_prices
                        }), new Ext.form.Hidden({
                            name: 'coord_key3',
                            value: selections[0].get('coord_key3')
                        }), new Ext.form.Hidden({
    ...
    updateDesinComment: function (rec, field) {
        ...
        if (vCompanyReserved4 != 'KWLM01KR') {
            if (new_sales_pice != null && new_sales_pice < 0) {
                new_sales_pice = 0;
            }
        }
        ...
        Ext.Ajax.request({
            ...
            params: {
                quan: quan,
                child: child,
                static_sales_price: new_sales_pice,
            ...
            success: function (result, request) {
                ...
                var sales_price = rec.get('new_sales_pice');
                var quan = rec.get('new_po_quan');
                var unit_mass = rec.get('unit_mass');

                switch (vCompanyReserved4) {
                    case 'KWLM01KR':
                        if (field != null && (field == 'sales_price' || field == 'quan' || field == 'new_sales_pice' || field == 'mass')) {
                            if (rec.get('unit_code') == 'Kg' && field == 'mass') {
                                rec.set('sales_amount', Math.floor(sales_price * mass));
                                rec.set('mass', mass);
                            } else if (rec.get('unit_code') == 'Kg' && (field == 'sales_price' || field == 'new_sales_pice')) {
                                rec.set('sales_amount', Math.floor(sales_price * mass));
                                rec.set('mass', mass);
                            } else {
                        ...
                        for (var i = 0; i < selections.length; i++) {
                            ...
                            var qty = selections[i].get('new_po_quan');
                            total_qty += qty;
                        }
                        ...
                        fn: function (btn) {
                            ...
                            if (btn == "no") {
                                ...
                                for (var i = 0; i < selections.length; i++) {
                                    ...
                                    //불출수량의 수량 = go_quan
                                    var quan = rec.get('go_quan');

                                    quanArr.push(quan);
                                    cartmapArr.push(uid);
                                    srcahdArr.push(srcahd_uid);
                                    curArr.push(currency);
                                    priceArr.push(new_sales_pice);
                                    nameArr.push(item_name);
                                    coordArr.push(coord_key3);

                                }
                                var pj_name = rec.get('pj_name');
                                var new_sales_pice = rec.get('new_sales_pice'); //cartmap.sales_price
    ...
    setSelections: function (selections) {
        ...
        if (selections.length) {
            ...
            gm.me().vSELECTED_QUAN = rec.get('new_po_quan');
                ...
                if (uniqueId == undefined || uniqueId < 0) {
                    ...
                    Ext.Ajax.request({
                        ...
                        params: {
                            ...
                            unique_uids: cartmapUids,
                            new_sales_prices: new_sales_prices,
                            new_po_quans: new_po_quans,
