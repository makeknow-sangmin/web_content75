Ext.define('Rfx2.view.company.kbtech.salesDelivery.IssueTaxInvoiceView', {
    extend: 'Rfx2.base.BaseView',
    xtype: 'issue-tax-invoice-view',
    initComponent: function(){

        //모델을 통한 스토어 생성
        this.createStore('Rfx2.model.company.kbtech.IssueTaxInvoice', [{
        ...
        //매출마감
        this.issueTaxInvoiceAction = Ext.create('Ext.Action',{
            iconCls: 'af-list-ul',
            text: '계산서발행',
            tooltip:'세금계산서발행',
            disabled: true,
        ...
        this.addSearchField ({
            type: 'checkbox',
            field_id: 'hasNotIssuedTaxInvoice',
            items: [
                {
                    boxLabel: '미발행 수주만',
                    checked: false
        ...
        buttonToolbar.insert(1, this.issueTaxInvoiceAction);
        ...
        this.setGridOnCallback(function(selections) {
            if (selections.length) {
                gm.me().issueTaxInvoiceAction.enable();
                gm.me().carryOverAction.enable();
            } else {
                gm.me().issueTaxInvoiceAction.disable();
                gm.me().carryOverAction.enable();
            }
                        ...
                        Ext.MessageBox.show({
                            ...
                            fn: function (result) {
                                ...
                                if (result == 'yes') {
                                    ...
                                    form.submit({
                                        url: CONTEXT_PATH + '/sales/delivery.do?method=issueTaxInvoide',
                                        params: {
                                            closalArr: closalArr
                                        },
                                        success: function (val, action) {
                                            Ext.Msg.alert('완료', '계산서 발행이 처리 되었습니다.');
                                        ...
                                        failure: function (val, action) {
                                            Ext.Msg.alert('', '계산서 발행을 처리하던 도중 오류가 발생하였습니다.');
                                            prWin.setLoading(false);
                                            prWin.close();
                                        }
                                    });
