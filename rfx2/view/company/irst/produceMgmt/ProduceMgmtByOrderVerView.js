Ext.define('Rfx2.view.company.irst.produceMgmt.ProduceMgmtByOrderVerView', {
    ...
    initComponent: function () {
        ...
        this.prEstablishAction = Ext.create('Ext.Action', {
            ...
            disabled: true,
            handler: function () {
                if (vCompanyReserved4 === 'IRST01KR' || vCompanyReserved4 === 'YNJU01KR') {
                    // 소공정 선택 화면을 만들기 위함
                    gm.me().producePlanOpSelectPcs();
                } else {
                    gm.me().producePlanOp();
                }
        ...
        Ext.each(this.columns, function (columnObj, index) {
            ...
            switch(dataIndex){
                case 'reserved_varchar1' :
                    ...
                    columnObj["editor"] = {
                        xtype : 'combo',
                        store : Ext.create('Mplm.store.PcsMchnStore',{}),
                        ...
                        listeners : {
                            select(combo, record, eOpts) {
                                var project_uid = gm.me().grid.getSelectionModel().getSelection()[0].get('ac_uid');
                                gm.editAjax('project','reserved_varchar1',record.get('name_ko'),'unique_id',project_uid,{type : ''})
                            }
                        }
                    }
            }
    ...
    producePlanOpSelectPcs: function () {
        var selection = this.grid.getSelectionModel().getSelection()[0];
        console_logs('selection ????', selection);
        var myWidth = 465;
        var myHeight = 480;
        gm.me().lineDetailStore.getProxy().setExtraParam('parent_code', 'PJ');
        gm.me().lineDetailStore.getProxy().setExtraParam('pcs_level', 1);
        gm.me().lineDetailStore.load();
        var processList = Ext.create('Ext.grid.Panel', {
            cls: 'rfx-panel',
            store: gm.me().lineDetailStore,
            id: gu.id('pcsGrid'),
            autoScroll: true,
            autoHeight: true,
            collapsible: false,
                    ...
                    handler: function (btn) {
                        if (btn == 'no') {
                            ...
                            if (form.isValid()) {
                                ...
                                var sel = processList.getSelectionModel().getSelection();
                                var assignProcessList = [];
                                for (var i = 0; i < sel.length; i++) {
                                    var sellist = sel[i];
                                    console_logs('>>>>> sellist', sellist);
                                    var code = sellist.get('code');
                                    assignProcessList.push(code);

                                }
                                assignProcessList.sort();
                                if (assignProcessList.length > 0) {
                                    console_logs('>>>>> assignProcessList', assignProcessList);
                                    form.submit({
                                        ...
                                        params: {
                                            ...
                                            process_list: assignProcessList,
                                    ...
                                    Ext.MessageBox.alert('알림', '최소 한개 이상의 진행할 공정이 선택되어야 합니다.');
                                }
    ...
    producePlanOp: function () {
        ...
        if (vCompanyReserved4 == 'MSTP01KR' || vCompanyReserved4 == 'HSST01KR' || vCompanyReserved4 == 'YNJU01KR') {
            var form = Ext.create('Ext.form.Panel', {
                xtype: 'form',
                frame: false,
                border: false,
                ...
                items: [
                    {
                        ...
                        items: [
                            ...
                            //         fieldLabel: '<span style="color: rgb(255, 0, 0); padding-left: 2px;">*</span>' + '년 / 월',
                            //         anchor: '100%',
                            //         width: '100%',
                            //         layout: 'hbox',
                            //         defaults: {
                ...
                items: [
                    {
                        ...
                        items: [
                            ...
                            },
                            {
                                ...
                                width: '99%',
                                editable: true,
                                fieldLabel: '<span style="color: rgb(255, 0, 0); padding-left: 2px;">*</span>' + '사용설비',
                                displayField: 'name_ko',
    ...
    lineDetailStore: Ext.create('Rfx2.store.company.bioprotech.LineDesignStore', {}),
