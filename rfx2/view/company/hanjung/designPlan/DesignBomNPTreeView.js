Ext.define('Rfx2.view.company.hanjung.designPlan.DesignBomNPTreeView', {
    extend: 'Rfx2.base.BaseView',
    xtype: 'design-bom-np-tree-view',
    total_sales_price : 0,
    materialDescStore: Ext.create('Rfx2.store.company.hanjung.BomMaterialDescStore', {}),
    materialDescRecord: null,
    initComponent: function () {
            ...
            switch (dataIndex) {
                case 'sales_price':
                    ...
                    o['summaryRenderer'] = function(value) {
                        console_logs('>>>>>>>>>>> sales_price', gUtil.renderNumber(value))
                ...
                case 'total_sales_price':
                    o['summaryType'] = 'sum';
                    o['summaryRenderer'] = function(value) {
                        console_logs('>>>>>>>>>>> total_sales_price', gUtil.renderNumber(value));
                        total_sales_price =  gUtil.renderNumber(value);
                        return gUtil.renderNumber(value);
                    };
                    break;
                default:
        ...
        this.createGrid([buttonToolbar1/*, this.buttonToolbar2 */], null, option);
    ...
    itemdblclick: function (view, record, htmlItem, index, eventObject, opts) {
        ...
        console_logs('>>>>> record !!!!!', rec);
        gm.me().materialDescStore.getProxy().setExtraParam('parent', rec.get('unique_id'));
        gm.me().materialDescStore.getProxy().setExtraParam('parent_uid', rec.get('unique_uid'));
        gm.me().materialDescStore.getProxy().setExtraParam('ac_uid', rec.get('ac_uid'));
        gm.me().materialDescStore.load(function (record) {
            ...
            if (gm.me().materialDescRecord.length > 0) {
                            ...
                            items: [
                                {
                                    ...
                                    html: '['+selections[0].get('item_code')+'] '+selections[0].get('item_name')+ ' ' + selections[0].get('specification')
                                },
                                {
                                    columnWidth: .5,
                                    xtype: 'component',
                                    style: 'font-weight:normal;text-align:right;padding-bottom: 7px; padding-left: 5px; padding-right: 5px; padding-top: 7px;',
                                    html: '<font color=yellow>총 금액 : ' + Ext.util.Format.number(selections[0].get('total_sales_price'), '0,00/i') + '</font>'
                                }
                            ]
                        }
                    ],
                ...
                Ext.MessageBox.show({
                    ...
                    msg: '해당 Assembly에 자재리스트가 등록되지 않거나 자재가 선택되었습니다.<br><b>자재가 등록이 안된 Assembly를 먼저 수주 BOM에 등록 후에<br>Part를 추가한 경우 해당 Assembly를 삭제 후 재등록 하십시오.</b>',
                    buttons: Ext.MessageBox.OK,
                    width: 510

                })

                // Ext.MessageBox.alert('알림','해당 Assembly에 자재리스트가 등록되지 않거나 자재가 선택되었습니다.<br>자재가 등록이 안된 Assembly를 먼저 등록 이후에 Part를 추가한 경우에는<br>해당 Assembly를 삭제 후 재등록 하십시오.')
            }
        })
    },
    ////
    ...
    createCenter: function () {
        ...
        this.bomAll =
            Ext.create('Rfx2.view.company.hanjung.grid.BomGridHeavyHanjung', {
                ...
                items: [
                    {
                        ...
                        displayTpl: Ext.create('Ext.XTemplate',
                            '<tpl for=".">',
                            '[{reserved_varcharh}] {reserved_varchar2}/{reserved_varchar3}',
                            '</tpl>'
                        ),
                        valueField: 'srcahd_uid',
                        store: Ext.create('Rfx2.store.company.hanjung.ProductStorePj', {}),
                        sortInfo: { field: 'pj_code', direction: 'desc' },
                        minChars: 1,
                        typeAhead: true,
                        ...
                        listConfig: {
                            ...
                            getInnerTpl: function () {
                                return '<div data-qtip="{reserved_varcharh}">[{reserved_varcharh}] <small><font color=blue>{reserved_varchar2}/{reserved_varchar3}</font></small></div>';
                            }
                        },
                        pageSize: 25,
                        triggerAction: 'all',
                        listeners: {
                            keyup: function () {
                                ...
                                gm.me().claastReset(0);
                                gu.getCmp('productcombo-DBM7_TREE').store.getProxy().setExtraParam('itemCodeOldAndNew', value);
                            },
    ...
    pasteAssyAction: Ext.create('Ext.Action', {
        ...
        disabled: true,
        handler: function (widget, event) {
            console_logs('gm.me().selectedPjUid', gm.me().selectedPjUid);
            ...
            if (gm.me().selectedAssyUid == null || gm.me().selectedAssyUid == '' || gm.me().selectedPjUid == null || gm.me().selectedPjUid == '') {
                ...
                Ext.Ajax.request({
                    ...
                    success: function (result, request) {
                        ...
                        gm.me().cloudAssemblyTreeStore.load({
                            callback: function (records, operation, success) {
                                console_log('load tree store');
                                console_log('ok');
                                Ext.MessageBox.alert('알림', 'Asssembly 복사등록 되었습니다.');
                                // pjTreeGrid.setLoading(false);
                                // my_treepanel.expandAll();
                            }
                        });
                    },
                    failure: extjsUtil.failureMessage
    ...
    selectAssymapCombo: function (record) {
        console_logs('record >>>', record);
        var reserved_varcharh = record.get('reserved_varcharh');
        var reserved_varchar2 = record.get('reserved_varchar2');
        var reserved_varchar3 = record.get('reserved_varchar3');
        var carMaker = record.get('carMaker');
        var pm_name = record.get('pm_name');

        var pjuid = record.get('ac_uid');
        ...
        var targetPjCode = '<div>수주번호 : ' + reserved_varcharh + ' (영업 담당자 : ' + pm_name +')<font color=yellow><br/>' + reserved_varchar2 + ' / [' + carMaker + ']' + reserved_varchar3 + '</font></div>';
            ...
            this.store.load(function (records) {
                ...
                if (pl_no == '---') {
                    ...
                    BomGrid.store.load(function (records) {
                        console_logs('>>>>> new total_sales_price', total_sales_price);
                        gu.getCmp('target_totalPrice').setHtml('<font size=10><font color=yellow>총 금액 : ' + gUtil.renderNumber(total_sales_price) + ' 원</font> ');
                    });
                }
            });
        }
    ...
    addPartAction: Ext.create('Ext.Action', {
        ...
        handler: function (widget, event) {
            ...
            if (reserved_integer1 === 0) {
                reserved_integer2 = gm.me().selected_tree_record.get('unique_uid');
            }
                        ...
                        dockedItems: [
                            {
                                ...
                                items: [
                                    ...
                                    },
                                    {
                                        ...
                                        listeners: {
                                            ...
                                            specialkey: function (f, e) {
                                                if (e.getKey() == e.ENTER) {
                                                    console_logs('ENTER');
                                                    if(gu.getCmp('search_item_code_part').getValue() != '') {
                                                        gm.me().searchStore.getProxy().setExtraParam('item_code', '%' + gu.getCmp('search_item_code_part').getValue() + '%');
                                                        gm.me().searchStore.load();
                                                    }
                                                }
                                            }
                                    ...
                                    },
                                    {
                                        ...
                                        listeners: {
                                            ...
                                            specialkey: function (f, e) {
                                                if (e.getKey() == e.ENTER) {
                                                    console_logs('ENTER');
                                                    if(gu.getCmp('search_item_name_part').getValue() != '') {
                                                        gm.me().searchStore.getProxy().setExtraParam('item_name', '%' + gu.getCmp('search_item_name_part').getValue() + '%');
                                                        gm.me().searchStore.load();
                                                    }
                                                }
                                            }
                                    ...
                                    },
                                    {
                                        ...
                                        listeners: {
                                            ...
                                            specialkey: function (f, e) {
                                                if (e.getKey() == e.ENTER) {
                                                    console_logs('ENTER');
                                                    if(gu.getCmp('search_specification_part').getValue() != '') {
                                                        gm.me().searchStore.getProxy().setExtraParam('specification', '%' + gu.getCmp('search_specification_part').getValue() + '%');
                                                        gm.me().searchStore.load();
                                                    }
                                                }
                                            }
                                    ...
                                    {
                                        field_id: 'search_maker_name',
                                        width: 120,
                                        fieldStyle: 'background-color: #D6E8F6; background-image: none;',
                                        id: gu.id('search_maker_name_part'),
                                        name: 'search_maker_name',
                                        xtype: 'triggerfield',
                                        margin: '3 3 3 3',
                                        emptyText: '제조사',
                                        trigger1Cls: Ext.baseCSSPrefix + 'form-clear-trigger',
                                        ...
                                        listeners: {
                                            change: function (fieldObj, e) {
                                                if (e.trim().length > 0) {
                                                    gm.me().searchStore.getProxy().setExtraParam('maker_name', '%' + e + '%');
                                                } else {
                                                    delete gm.me().searchStore.proxy.extraParams.maker_name;
                                                }
                                            },
                                            render: function (c) {
                                                Ext.create('Ext.tip.ToolTip', {
                                            ...
                                            specialkey: function (f, e) {
                                                if (e.getKey() == e.ENTER) {
                                                    console_logs('ENTER');
                                                    if(gu.getCmp('search_maker_name_part').getValue() != '') {
                                                        gm.me().searchStore.getProxy().setExtraParam('maker_name', '%' + gu.getCmp('search_maker_name_part').getValue() + '%');
                                                        gm.me().searchStore.load();
                                                    }
                                                }
                                            }
                                    ...
                                    {
                                        field_id: 'search_sales_price',
                                        width: 120,
                                        fieldStyle: 'background-color: #D6E8F6; background-image: none;',
                                        id: gu.id('search_sales_price_part'),
                                        ...
                                        listeners: {
                                            change: function (fieldObj, e) {
                                                if (e.trim().length > 0) {
                                                    gm.me().searchStore.getProxy().setExtraParam('sales_price', '%' + e + '%');
                                                } else {
                                                    delete gm.me().searchStore.proxy.extraParams.sales_price;
                                                }
                                            },
                                            render: function (c) {
                                                Ext.create('Ext.tip.ToolTip', {
                                            ...
                                            specialkey: function (f, e) {
                                                if (e.getKey() == e.ENTER) {
                                                    console_logs('ENTER');
                                                    if(gu.getCmp('search_sales_price_part').getValue() != '') {
                                                        gm.me().searchStore.getProxy().setExtraParam('sales_price', '%' + gu.getCmp('search_sales_price_part').getValue() + '%');
                                                        gm.me().searchStore.load();
                                                    }
                                                }
                                            }
    ...
    getaddedPlno : function (top_pl_no) {
        var child = gm.me().selected_tree_record.get('child');
        var ac_uid = gm.me().selected_tree_record.get('ac_uid');
        var unique_uid = gm.me().selected_tree_record.get('unique_uid');
        var reserved_integer1 = gm.me().selected_tree_record.get('reserved_integer1');
        ...
        Ext.Ajax.request({
            url: CONTEXT_PATH + '/sales/poreceipt.do?method=getAssyPlno2',
                ...
                success: function (result, request) {
                    ...
                    gu.getCmp('assy_pl_no').setValue(top_pl_no + str);
                },
                failure: extjsUtil.failureMessage
        });
    },
        ...
        handler: function (widget, event) {
            ...
            Ext.Ajax.request({
                ...
                success: function (result, request) {
                    ...
                    gm.me().searchStoreAssy.removeAll();
                    ...
                    this.gridViewTable = Ext.create('Ext.grid.Panel', {
                        ...
                        listeners: {
                            select: function (selModel, record, index, options) {
                                gu.getCmp('assy_unique_id').setValue(record.get('unique_id_long'));
                                gu.getCmp('assy_item_code').setValue(record.get('item_code'));
                                gu.getCmp('assy_item_name').setValue(record.get('item_name'));
                                gu.getCmp('assy_specification').setValue(record.get('specification'));
                                gu.getCmp('assy_maker_name').setValue(record.get('maker_name'));
                                gu.getCmp('assy_unit_code').setValue(record.get('unit_code'));
                                gu.getCmp('assy_sales_price').setValue(record.get('sales_price'));
                                gu.getCmp('assy_currency').setValue(record.get('currency'));
                            }
                        },
                        dockedItems: [
                            {
                                        ...
                                        listeners: {
                                            ...
                                            specialkey: function (f, e) {
                                                if (e.getKey() == e.ENTER) {
                                                    console_logs('ENTER');
                                                    if(gu.getCmp('search_item_code_part').getValue() != '') {
                                                        gm.me().searchStoreAssy.getProxy().setExtraParam('item_code', '%' + gu.getCmp('search_item_code_part').getValue() + '%');
                                                        gm.me().searchStoreAssy.load();
                                                    }
                                                }
                                            }
                                        ...
                                        listeners: {
                                            ...
                                            specialkey: function (f, e) {
                                                if (e.getKey() == e.ENTER) {
                                                    console_logs('ENTER');
                                                    if(gu.getCmp('search_item_name_part').getValue() != '') {
                                                        gm.me().searchStoreAssy.getProxy().setExtraParam('item_name', '%' + gu.getCmp('search_item_name_part').getValue() + '%');
                                                        gm.me().searchStoreAssy.load();
                                                    }
                                                }
                                            }
                                        ...
                                        listeners: {
                                            ...
                                            specialkey: function (f, e) {
                                                if (e.getKey() == e.ENTER) {
                                                    console_logs('ENTER');
                                                    if(gu.getCmp('search_specification_part').getValue() != '') {
                                                        gm.me().searchStoreAssy.getProxy().setExtraParam('specification', '%' + gu.getCmp('search_specification_part').getValue() + '%');
                                                        gm.me().searchStoreAssy.load();
                                                    }
                                                }
                                            }
                    ...
                    gm.me().createPartForm = Ext.create('Ext.form.Panel', {
                        ...
                        items: [
                            ...
                            },
                            {
                                ...
                                width: '100%',
                                fieldLabel: gm.me().getColName('unit_code'),
                                id: gu.id('assy_unit_code'),
                                name: 'unit_code',
                                allowBlank: true
                            }, {
                                xtype: 'numberfield',
                                width: '100%',
                                id: gu.id('assy_sales_price'),
                                name: 'sales_price',
                                fieldLabel: gm.me().getColName('sales_price'),
                                allowBlank: true,
                                value: '0',
                                ...
                                emptyText: 'Level',
                                name: 'reserved_integer1',
                                value: gm.me().selected_tree_record.get('reserved_integer1') + 1,
                            }, {
                                xtype: 'textfield',
                                fieldLabel: '단위',
                                emptyText: '단위',
                                ...
                                if (form.isValid()) {
                                    ...
                                    Ext.Ajax.request({
                                        ...
                                        params: {
                                            ...
                                            item_name: val['assy_name'],
                                            level: val['reserved_integer1'],
                                            assytopUid: gm.me().selected_tree_record.get('unique_uid')
                                        },
                                        success: function (result, request) {
                                            gm.me().setLoading(false);
                                            if (gm.me().win_check) {
                                                gm.me().getaddedPlno(top_pl_no);
                                            }
                                            gm.me().reSelect();
                                        },
                                        failure: function () {
    ...
    searchStore: Ext.create('Mplm.store.MaterialRStore', {}),
    searchStoreAssy: Ext.create('Mplm.store.MaterialAStore', {}),
    ...
    claastReset: function (no) {
        var claastcombo1 = Ext.getCmp('DBM7_TREE-PRDLV1');
        var claastcombo2 = Ext.getCmp('DBM7_TREE-PRDLV2');
        var claastcombo3 = Ext.getCmp('DBM7_TREE-PRDLV3');
        var claastcombo4 = Ext.getCmp('DBM7_TREE-PRDLV4');
        switch (no) {
            case 0:
                break;
            case 1:
        ...
        var productcombo = Ext.getCmp('DBM7_TREE_srcahd_uid');
        productcombo.store.getProxy().setExtraParam({});
        var claastlevel1 = Ext.getCmp('DBM7_TREE-PRDLV1-ADD').getValue();
        var claastlevel2 = Ext.getCmp('DBM7_TREE-PRDLV2-ADD').getValue();
        var claastlevel3 = Ext.getCmp('DBM7_TREE-PRDLV3-ADD').getValue();
        var claastlevel4 = Ext.getCmp('DBM7_TREE-PRDLV4-ADD').getValue();
    ...
    claastReset2: function (no) {
        var claastcombo1 = Ext.getCmp('DBM7_TREE-PRDLV1-ADD').getValue();
        var claastcombo2 = Ext.getCmp('DBM7_TREE-PRDLV2-ADD');
        var claastcombo3 = Ext.getCmp('DBM7_TREE-PRDLV3-ADD');
        var claastcombo4 = Ext.getCmp('DBM7_TREE-PRDLV4-ADD');
        switch (no) {
            case 1:
