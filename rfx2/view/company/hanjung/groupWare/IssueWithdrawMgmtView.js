Ext.define('Rfx2.view.company.hanjung.groupWare.IssueWithdrawMgmtView', {
    ...
    storeViewProp: Ext.create('Rfx2.store.company.hanjung.WthOutTypeStore', {}),
    ...
    initComponent: function () {
        ...
        var addWdInHistoryEstBtn = Ext.create('Ext.Action', {
            xtype: 'button',
            iconCls: 'af-plus-circle',
            text: '입금 비고내역',
            tooltip: '비고내역',
            ...
            handler: function () {
                ...
                var pj_uid = rec.get('ac_uid');
                console_logs('>>>> pj_uid >>>', pj_uid);
                gm.me().addWthInWindowEst(pj_uid);
            }
        });

        var addWdOutHistoryEtcBtn = Ext.create('Ext.Action', {
            xtype: 'button',
            iconCls: 'af-plus-circle',
            text: '출금내역 작성', //'출금 비고내역',
            tooltip: '출금내역 작성', //'출금 비고내역',
            disabled: true,
            hidden: false,
            handler: function () {
                var selection = poStatusTemplate.getSelectionModel().getSelection();
                var rec = selection[0];
                var pj_uid = rec.get('ac_uid');
                console_logs('>>>> selection >>>', rec);
                console_logs('>>>> rec >>>', rec);
                console_logs('>>>> pj_uid >>>', pj_uid);
                gm.me().addWthOutWindowEst(pj_uid);
            }
        });
        ...
        var bigo1 = Ext.create('Ext.Action', {
            xtype: 'button',
            iconCls: 'af-plus-circle',
            text: '입금 비고',
            tooltip: '입금 비고',
            disabled: true,
            handler: function () {
                var selection = gridDimension.getSelectionModel().getSelection();
                var rec = selection[0];
                var pj_uid = rec.get('ac_uid');
                var unique_id = rec.get('unique_id');
                console_logs('>>>> selection >>>', selection);
                console_logs('>>>> pj_uid >>>', pj_uid);
                gm.me().addBigo1(pj_uid, unique_id);
            }
        });
        
        var bigo2 = Ext.create('Ext.Action', {
            xtype: 'button',
            iconCls: 'af-plus-circle',
            text: '출금 비고',
            tooltip: '출금 비고',
            ...
            handler: function () {
                ...
                console_logs('>>>> selection >>>', selection);
                console_logs('>>>> pj_uid >>>', pj_uid);
                gm.me().addBigo2(pj_uid, unique_id);
            }
        });
        ...
        var poStatusTemplate = Ext.create('Ext.grid.Panel', {
            ...
            },
            {
                ...
                align: "right",
                style: 'text-align:center',
                dataIndex: 'wthdraw_price_include_refund'
            },
            {
                text: '차액금',
                width: 70,
                ...
                renderer: function (value, meta) {
                    if (value > 0) {
                        meta.style = "background-color:#0984e3;color:#ffffff;text-align:right;text-format:0,000";
                    }
                    if (value < 0) {
        ...
        poStatusTemplate.getSelectionModel().on({
            selectionchange: function (sm, selections) {
                ...
                if (selections.length > 0) {
                    ...
                    addWdInHistoryEstBtn.enable();
                    addWdOutHistoryEtcBtn.enable();
            ...
            dockedItems: [
                {
                    ...
                    xtype: 'toolbar',
                    cls: 'my-x-toolbar-default2',
                    items: [addWdInHistory, addWdInHistoryEstBtn, editWdInHistory, deleteWdInAction, bigo1/*checkedIssueBillIn*/]
                }
            ]
        });
        ...
        gridDimension.getSelectionModel().on({
            selectionchange: function (sm, selections) {
                if (selections.length > 0) {
                    ...
                    in_description = selections[0].get('description1');
                    in_description = selections[0].get('description2');
            ...
            dockedItems: [
                {
                    ...
                    xtype: 'toolbar',
                    cls: 'my-x-toolbar-default2',
                    items: [addWdOutHistory, addWdOutHistoryEtcBtn, editWdOutHistory ,deleteWdOutAction, bigo2 /**checkedIssueBillOut,downloadFiles**/]
                }
            ] 
        });
        ...
        gridViewprop.getSelectionModel().on({
            selectionchange: function (sm, selections) {
                if (selections.length > 0) {
                    ...
                    out_description = selections[0].get('description1');
                    out_description = selections[0].get('description2');
                    sub_type = selections[0].get('sub_type_kr');
...
addWthOutWindow: function (pj_uid) {
 ...
 addWthInWindowEst: function (pj_uid) {

    var form = Ext.create('Ext.form.Panel', {
        id: gu.id('addWthInWindowEst'),
        xtype: 'form',
        frame: false,
        border: false,
        width: '100%',
    ...
    addWthOutWindowEst: function (pj_uid) {
        var form = Ext.create('Ext.form.Panel', {
            id: gu.id('addWthOutWindowEst'),
            xtype: 'form',
            frame: false,
            border: false,
            width: '100%',
            ...
            buttons: [{
                ...
                handler: function (btn) {
                    if (btn == 'no') {
                        ...
                        if (form.isValid()) {
                            var val = form.getValues(false);
                            var description1 = val['description1'];
                            form.submit({
                                url: CONTEXT_PATH + '/account/arap.do?method=attachedBigo1',
                                waitMsg: '비고내역을 저장중입니다.',
                                params: {
    ...
    editBillHistoryWindow: function (requestor, description, reserved_timestamp1_str, total_price, supPrice, taxPrice, unique_id, pj_uid) {
        var form = Ext.create('Ext.form.Panel', {
            id: gu.id('editBillHistoryWindow'),
            xtype: 'form',
            frame: false,
            border: false,
            width: '100%',
            ...
            buttons: [{
                ...
                handler: function (btn) {
                    if (btn == 'no') {
                        ...
                        if (form.isValid()) {
                            var val = form.getValues(false);
                            var description2 = val['description2'];
                            form.submit({
                                url: CONTEXT_PATH + '/account/arap.do?method=attachedBigo2',
                                waitMsg: '비고내역을 저장중입니다.',
                                params: {
