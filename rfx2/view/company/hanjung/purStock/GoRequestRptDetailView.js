Ext.define('Rfx2.view.company.hanjung.purStock.GoRequestRptDetailView', {
        ...
        this.reReceiveAction = Ext.create('Ext.Action', {
            ...
            handler: function () {
                Ext.MessageBox.show({
                    ...
                    fn: function (result) {
                        if (result == 'yes') {
                            console_logs('>>>>> rtgast', this.rtgast_uids);
...
//			            						unique_id: rtgast_uid
		...
		buttonToolbar.insert(2, this.removeBAction);
        ...
        this.setGridOnCallback(function (selections) {
            if (selections.length) {
                ...
                this.cartLineGrid.getStore().load(function (record) {
                    ...
                    buttonToolbar2.items.items[1].update('총 금액 : ' + gUtil.renderNumber(totalPrice) + ' 원');
                    buttonToolbar3.items.items[1].update('요청번호 : ' + rec.get('po_no'));
                });
		...
		// 			gm.me().cartLineGrid.getStore().getProxy().setExtraParam('rtgastuid', this.rtgast_uids);
		// 			console_logs("this.rtgast_uids>>>>>>>>>>", this.rtgast_uids);
		// 			var totalPrice = 0;
		// 			gm.me().cartLineGrid.getStore().load(function(record){
		// 				for (var i = 0; i < record.length; i++) {
		// 					var t_rec = record[i]
		// 					totalPrice += t_rec.get('sales_price') *  t_rec.get('quan');                
		// 				}
