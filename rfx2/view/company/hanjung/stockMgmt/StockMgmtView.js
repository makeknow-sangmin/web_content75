Ext.define('Rfx2.view.company.hanjung.stockMgmt.StockMgmtView', {
    ...
    initComponent: function () {
                    ...
                    var form = Ext.create('Ext.form.Panel', {
                        ...
                        items: [
                            {   
                                ...
                                store: Ext.create('Rfx2.store.company.hanjung.ProductStoreStock', {pageSize: 10}),
        ...
        this.store.getProxy().setExtraParam('existStoQty', 'true');
    ...
    addStockOut: function (val) {
        Ext.MessageBox.show({
            title: '창고 반출',
            msg: '창고에서 반출하시겠습니까?' + '\r 수량: ' + gu.getCmp('wh_qty_out').getValue() +'</br>(이미 소모된 품목은 추가 소모되지 않습니다.)',
            buttons: Ext.MessageBox.YESNO,
            fn: function (btn) {
                if (btn == 'yes') {
                    Ext.Ajax.request({
                        ...
                        params: {
                            ...
                            innout_desc: val['innout_desc'],
                            pj_uid: val['pj_uid'],
                            wh_qty: val['wh_qty'].replace(/,/gi,'') * (-1),
                            whouse_uid: 100
                        },
                        success: function (result, request) {
                            var resultText = result.responseText;
