...
Ext.define('Rfx2.view.company.dsmaref.designPlan.DesignBomTreeView', {
        ...
        Ext.Ajax.request({
            ...
            success: function(result, request) {
                ...
                gm.me().cloudprojectStore.load(function(records) {
                    ...
                    if(vCompanyReserved4 == 'KWLM01KR' || vCompanyReserved4 == 'DSMF01KR') {
...
//
    ...
    createWest: function() {
        ...
        this.cloudProjectTreeStore.getProxy().setExtraParam('pjuid', 100);
        ...
        this.assyGrid = Ext.create('Ext.tree.Panel', {
            ...
            dockedItems: [{
            ...
            },
                {
                    ...
                    items: [/*{
                        ...
                        queryMode: 'remote',
                        emptyText: '고객사',
                        hidden: (vCompanyReserved4 ==  'DSMF01KR') ? true : false,
                    ...
                    },
                        {
                            ...
                            listConfig: {
                                maxHeight:500,
                                getInnerTpl: function() {
                                    return '<div data-qtip="{unique_id}"><small><font color=blue>{pj_code}</font><br/> {pj_name}</small></div>';
                                }
                            },
                            // pageSize:25,
                            triggerAction: 'all',
            ...
            columns: [
                ...
                width: vCompanyReserved4 != 'DSMF01KR' ? 50 : 0,
...
//                                  });
            ...
            ,viewConfig: {
                ...
                //     console_logs('>>>r', r);
                //     var is_new = r.get('is_new');
                //     switch(is_new) {
                //     case true:
                //         return 'blue-row';
                //     }
                //     // var pcr_div = r.get('pcr_div');
                //     // if(pcr_div != null && pcr_div == 'MAKE') {
                //     //     var status = r.get('status');
                //     //     switch(status) {
                //     //         case 'BR': // 영업에서의 BOM 확정
                //     //         // r.collapse();
                //     //         return 'blue-row';
                //     //     }
                //     // }
    ...
    cloudprojectCopyStore: Ext.create('Mplm.store.cloudProjectStore', { hasNull:true}),
...
//                var result = result.responseText;
    ...
    confirmAction: Ext.create('Ext.Action', {
        itemId: 'confirmAction',
        iconCls: 'fa_4-7-0_paste_14_0_5395c4_none',
        text: '확정완료',
        disabled: true,
        handler: function(widget, event) {
    ...
    copyTreeAction: Ext.create('Ext.Action', {
        itemId: 'copyTreeAction',
        iconCls: 'af-copy',
        text: 'BOM복사',
        disabled: false,
        handler: function(widget, event) {
           gm.me().bomCopyHandler();
        }
    }),

    bomCopyHandler: function() {
    ...
    originGridFn: function() {
        this.originGrid = Ext.create('Ext.tree.Panel', {
            title: '원본',
            useArrows: true,
            id:'originGrid',
            rootVisible: false,
            ...
            viewConfig: {
                ...
                plugins: {
                   ...
                   dragText: '원하는 위치에 옮겨주세요.'
                },
                // listeners: {
                //    drop: function(node, data, dropRec, dropPosition) {
                //       var dropOn = dropRec ? ' ' + dropPosition + ' ' + dropRec.get('name') : ' on empty view';
                //       console_logs('>>>>>>>Origin Tree Drop', dropOn);
                //     }
                // }
             },
        });
    ...
    cloudProjectTreeCopyStore : Ext.create('Mplm.store.cloudProjectTreeCopyStore', {}),

    copyGridFn: function() {
        this.cloudProjectTreeCopyStore.getProxy().setExtraParam('pjuid', 100);
        this.copyGrid = Ext.create('Ext.tree.Panel', {
            ...
            rootVisible: false,
            layout:'fit',
            store: this.cloudProjectTreeCopyStore,
            // selModel: selModel,
            multiSelect: true,
            dockedItems: [
                {
            ...
            viewConfig: {
                ...
                listeners: {
                   drop: function(node, data, record, dropPosition) {
                        console_logs('>>>>>>>>>>>>>>>>> data', data);
                        console_logs('>>>>>>>>>>>>>>>>> record', record);
                        var originData = data.records[0];
                        var copyData = record;
                        console_logs('>>>>>>>>>>>>>>>>> parent_uid compare', originData.id + ' : ' + copyData.id);
                        console_logs('>>>>>>>>>>>>>>>>> ac_uid compare', originData.get('ac_uid') + ' : ' + copyData.get('ac_uid'));
                        ...
                        Ext.Ajax.request({
                            url: CONTEXT_PATH + '/design/bom.do?method=copyToTargetBOM',
                            params: {
                                from_uid:originData.id,
                                to_uid:copyData.id,
                                from_pj:originData.get('ac_uid'),
                                to_pj:copyData.get('ac_uid'),
                            },
                            timeout: 1000000,
                            success: function(result, request) {
                                ...
                                gm.me().cloudProjectTreeStore.load(function() {  
                                    gm.me().originGrid.expandAll();
                                });
                                gm.me().cloudProjectTreeCopyStore.load(function() {  
        ...
        this.copyGrid.expandAll();

        return this.copyGrid;
    }, 

    readyForMake: function(result) {
        if(result == 'yes') {
            ...
            for(var i=0; i<selections.length; i++) {
                ...
                var pcr_div = select.get('pcr_div');
                if(pcr_div == null || pcr_div.length<1 || pcr_div != 'MAKE' ) {
                    Ext.MessageBox.alert('경고', '1레벨 제품만 선택해주세요.');
                    return;
                }
                var status = select.get('status');
                if(status==null || status.length<1 || status =='BM') {
                    var id = select.get('unique_uid');
                    ids.push(id);
                } else {
                    Ext.MessageBox.alert('알림', '설계대기 상태가 아닙니다.');
                    return;
                }
            }
            
            var pj_assy_uid = selections[0].get('parentId');
            gm.me().assyGrid.setLoading(true);
            Ext.Ajax.timeout= 1000000; 
            Ext.override(Ext.form.Basic, { timeout: Ext.Ajax.timeout / 1000 });
            Ext.override(Ext.data.proxy.Server, { timeout: Ext.Ajax.timeout });
            Ext.override(Ext.data.Connection, { timeout: Ext.Ajax.timeout });
            Ext.Ajax.request({
                url: CONTEXT_PATH + '/design/bom.do?method=confirmSalesBOM',
                params: {
                    unique_ids:ids,
                    pj_assy_uid:pj_assy_uid
                },
                timeout: 1000000,
                success: function(result, request) {
                    gm.me().selectTreeGrid(null);
...
//        console_logs("srchSingleHandler b", b);
    ...
    srchCopyTreeHandler: function(my_treepanel, cloudProjectTreeStore, widName, parmName, b) {
            ...
            //     }
            // }
            this.cloudProjectTreeCopyStore.getProxy().setExtraParam(parmName, val);
            this.cloudProjectTreeCopyStore.load({
                callback: function(records, operation, success) {
                    console_logs('======> records', records);
    ...
    selectCopyProjectCombo: function(record) {
        this.srchCopyTreeHandler(this.copyGrid, this.cloudProjectTreeCopyStore, 'projectCopycombo', 'pjuid', true);
    },
