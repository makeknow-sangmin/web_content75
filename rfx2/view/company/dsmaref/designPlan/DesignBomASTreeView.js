...
Ext.define('Rfx2.view.company.dsmaref.designPlan.DesignBomASTreeView', {
...
//                },
        ...
        Ext.Ajax.request({
            ...
            success: function (result, request) {
                ...
                var comboStore = gu.getCmp('productcombo-DBM7_AST').store;
                comboStore.getProxy().setExtraParams({});
                comboStore.getProxy().setExtraParam('id', uidSrcahd);

                comboStore.load(function (records) {
                    comboStore.getProxy().setExtraParams({});
                    if (records != null && records.length > 0) {
                        for (var i = 0; i < records.length; i++) {
                            var rec = records[i];
                            if (rec.get('unique_id') == uidSrcahd) {
                                var combo = gu.getCmp('productcombo-DBM7_AST');
                                if (combo != undefined) {
                                    combo.select(rec);
                                    gm.me().selectAssymapCombo(rec);
                                }
                            }
                        }
                    }
...
//        this.selectedPjName = pj_name;
        ...
        var value = '<font color="#FAEE0E">['+item_code+']  ' + item_name + 
                '<font/><br/><font color="white">' + specification + '</font>';

        gu.getCmp('target-projectcode-DBM7_AST').update(value);
            ...
            if (pl_no == '---') {
                ...
                BomGrid.store.load(function (records) {
                    ...
                    for (var i = 0; i < records.length; i++) {
                        ...
                        // var total_sales_price = records[i].get('total_sales_price');
                        var total_sales_price = records[i].get('quan') * records[i].get('contract_price');
