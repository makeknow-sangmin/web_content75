//수주등록된 전체 제품 메뉴
Ext.define('Rfx2.view.company.dsmaref.salesDelivery.HEAVY4_RecvPoSubView', {
    ...
    initComponent: function () {
                    ...
                    buttons: [
                        {
                            ...
                            handler: function () {
                                ...
                                Ext.Ajax.request({
                                    url: CONTEXT_PATH + '/index/process.do?method=printStockedStatementExcel',
                                    params: {
        ...
        // buttonToolbar.insert(3, this.printStockedStatementAction);
            ...
            if (selections.length) {
                ...
                if (items != null) {
                    for (var i = 0; i < items.length; i++) {
                        console_logs('>>>items[i]', items[i]);
                        var item = items[i];
                        var id = item.id;
                        var back_id = id.split('-')[1];
                        var step_state = rec.get('step_state');
                        console_logs('>>>back_id', back_id);
                        switch(back_id) {
                            case 'start':
                                switch(step_state) {
                                    case 'W':
    ...
    createPcsToobars: function (code) {
        ...
        if (smallPcs != null && smallPcs.length > 0) {
            ...
            for (var i = 0; i < smallPcs.length; i++) {
                ...
                console_logs('code + code1', code + code1);
                    ...
                    handler: function () {
                        ...
                        if (text == null) {
                            ...
                            if (selections != null) {
                                ...
                                var pcs_type = gm.me().multi_grid_id;
                                
                                var uid_comast = vCOMAST_UID
                                var pcs_code = pcs_type;
                                var step_uid = select.get(pcs_type+'|step_uid')
                                var btn_type = 'start';
                                var output_qty = select.get('quan');

                                Ext.Ajax.request({
                                    url: CONTEXT_PATH + '/tablet.do?method=goProcessByBtn',  // &sp_code_list=KA,KB,KC,KL
                                    params: {
