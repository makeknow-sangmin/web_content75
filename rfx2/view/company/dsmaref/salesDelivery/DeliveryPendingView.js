...
Ext.define('Rfx2.view.company.dsmaref.salesDelivery.DeliveryPendingView', {
    ...
    initComponent: function () {
        ...
        //Rfx.model.RecvPoDsmfMN
        this.createStore('Rfx2.model.DeliveryMgmtForBOM', [{  // Rfx2.model.DeliveryMgmtForBOM
        ...
        Ext.each(this.columns, function(columnObj, index) {
            ...
            case 'reserved_timestamp1':
                o['editor'] = {xtype:'datefield', submitFormat: 'Y-m-d', dateFormat: 'Y-m-d'};
                o['css'] = 'edit-cell';
                o['renderer'] = function(value, meta) {
                    meta.css = 'custom-column';
            ...
            case 'remain_qty':
                o['renderer'] = function(value, meta, record) {
                    var quan = record.get('quan') == null || record.get('quan') == undefined ? 0 : record.get('quan');
                    var total_out_qty = record.get('total_out_qty') == null || record.get('total_out_qty') == undefined ? 0 : record.get('total_out_qty');
                    var remain_qty = quan - total_out_qty;
                    if(remain_qty == null || remain_qty == undefined) {
                        remain_qty = 0;
                    }
                    return  remain_qty;
                } 
        ...
        // 버튼 추가.
        buttonToolbar.insert(6, this.dlAction);
        ...
        this.store.getProxy().setExtraParam('not_pj_type_list', 'AS,R,S');
        this.store.getProxy().setExtraParam('having_not_status', 'BM');
        this.store.getProxy().setExtraParam('menu_type', 'DL');
    ...
    getDefaultColumns: function() {
        var columns = [];
        columns.push(
            { header:'진행상태', dataIndex:'status', width:'6%', align:'center', sortable:true, renderer:function(value, meta) {return gm.me().getAssyStatus(value, meta)} },
            { header:'제작번호', dataIndex:'make_no', width:'9%', align:'center',sortable:true },
            { header:'품번', dataIndex:'item_code', width:'10%', sortable:true, align:'left', style:'text-align:center' },
            { header:'품명', dataIndex:'item_name', width:'10%', sortable:true, align:'left', style:'text-align:center' },
            { header:'규격', dataIndex:'specification', width:'13%', sortable:true, align:'left', style:'text-align:center' },
            { header:'출하예정수량', dataIndex:'quan', width:'8%', sortable:true, align:'right', style:'text-align:center',
                renderer: function(value, meta) {
                    return  value;
                }
            },
            { header:'재고수량', dataIndex:'wh_qty', width:'6%', align:'right', style:'text-align:center', sortable:true },
            { header:'미납수량', dataIndex:'remain_qty', width:'6%', align:'right', style:'text-align:center', sortable:true, 
            ...
            { header:'출하수량', dataIndex:'possible_quan', width:'6%', align:'right', style:'text-align:center', sortable:true, 
                editor:{xtype:'numberfield'}, css: 'edit-cell',
                renderer: function(value, meta, record) {
                    meta.css = 'custom-column';
                    return value;
                }
            },{ header:'출하예정일', dataIndex:'reserved_timestamp1', width:'8%', align:'right', style:'text-align:center', sortable:true, 
                css: 'edit-cell', editor:{ xtype:'datefield', submitFormat: 'Y-m-d', dateFormat: 'Y-m-d' },
                renderer: function(value, meta) {
                    meta.css = 'custom-column';
                    if(value != null && value.length>0) {
                        return Ext.Date.format(new Date(value), 'Y-m-d');
            ...
            { header:'납품단가', dataIndex:'sales_price', width:'7%', sortable:true, align:'right', style:'text-align:center',
                renderer: function(value, meta) {
                    if(value!=null&&value.length>0) {
                        value = Ext.util.Format.number(value, '0,00/i');
                    } else {
            ...
            { header:'특이사항', dataIndex:'reserved6', width:'10%', sortable:true, align:'left', style:'text-align:center', editor:{},
                css: 'edit-cell',
                renderer: function(value, meta) {
                    meta.css = 'custom-column';
                    return value;
    ...
    dlAction: Ext.create('Ext.Action', {
    ...
    dlHandler: function() {
        ...
        for(var i=0; i<selects.length; i++) {
            ...
            var id = select.id;
            var possible_quan = select.get('possible_quan');
            var complished_yn = "N";
            var quan = select.get('quan');
            var total_out_qty = select.get('total_out_qty');
            var remain_qty = quan - total_out_qty;
            var wh_qty = select.get('wh_qty');
            if(wh_qty<remain_qty) {
                Ext.Msg.alert('안내', '재고수량이 부족합니다.', function() {});
                return;
            }
            if(possible_quan >= remain_qty) {
                complished_yn = "Y";
            };

            if(json.length) {
                json += ",{\"id\":" + id + ",\"possible_quan\":" + possible_quan + ",\"complished_yn\":" + "\""+complished_yn+"\"" + "}";
            } else {
                json += "{\"id\":" + id + ",\"possible_quan\":" + possible_quan + ",\"complished_yn\":" + "\""+complished_yn+"\"" + "}";
            }
        };
            ...
            items:[
                ...
                },
                {
                    ...
                    listConfig: {
                        ...
                        emptyText: 'No matching posts found.',
                        getInnerTpl: function() {
                            return '<div data-qtip="{unique_id}">{po_no} [{wa_name}]</div>';
                        }
                    },
                    listeners: {
                        select: function(combo, record) {
                            ...
                            if(value != null && value.length > 0) {
                                Ext.getCmp('release_date').setDisabled(true);
                            } else {
                                Ext.getCmp('release_date').setDisabled(false);
                            }
                        }
                    }
                }, {
        ...
        var win = Ext.create('Ext.Window', {
            ...
            buttons: [{
                ...
                handler: function(btn) {
                    ...
                    var release_date = Ext.getCmp('release_date').getValue();
                    if(delivery_no == null || delivery_no == undefined) {
                        delivery_no = null;
                    };
                    if(release_date != null && release_date.length>0) {
                        release_date = Ext.Date.format(release_date, 'Y-m-d');
                    };

                    Ext.Ajax.request({
                        url: CONTEXT_PATH + '/sales/productStock.do?method=addStateDelivery',
                        params:{
                            jsonDatas:json,
    ...
    createCenter: function() {
        ...
        this.prdGrid.getSelectionModel().on({
            selectionchange: function(sm, selections) {
                if(selections.length) {
                    gUtil.enable(gm.me().dlAction);
                } else {
                    gUtil.disable(gm.me().dlAction);
                }
            }
        });
        ...
        this.prdGrid.on('edit', function(editor, e) {
            ...
            var comment = rec.get('reserved6');
            var reserved_timestamp1 = rec.get('reserved_timestamp1');

            console_logs('>>>Edit field', field);
            switch(field) {
                case 'possible_quan':
                    var value = rec.get('possible_quan');
                    console_logs('>>>Edit value', value);
                    rec.set('possible_quan', value);
                    rec.commit();
                return;
            }

            Ext.Ajax.request({
                url: CONTEXT_PATH + '/sales/delivery.do?method=editDeliveryItems',
                params: {
                    unique_id:unique_id,
                    comment:comment,
...
//	                       						result = result.substring(0,6)+'-'+result.substring(6,9);
    ...
    prdStore: Ext.create('Mplm.store.RecvPoDsmfDeliveryPRD', {}),
    ...
    rtgastDlStore : Ext.create('Mplm.store.RtgAstDlStore', {}),
});
