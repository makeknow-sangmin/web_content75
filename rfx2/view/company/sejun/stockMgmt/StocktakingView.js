//자재 관리
Ext.define('Rfx2.view.company.sejun.stockMgmt.StocktakingView', {
        ...
        */
        ...
        this.downloadSheetAction = Ext.create('Ext.Action', {
            ...
            handler: function () {
                ...
                var choiceform = Ext.create('Ext.form.Panel', {
                    xtype: 'form',
                    id: gu.id(gu.id('choiceform1')),
                    frame: false,
                    border: false,
                    bodyPadding: 15,
                    region: 'center',
                    ...
                    buttons: [{
                        ...
                        handler: function (btn) {
                            if (btn == "no") {
                                ...
                                console_logs('newstoshkchoice', gu.getCmp('newstoshkchoice').getValue()['stochkchoice2']);

                                var store = Ext.create('Rfx2.model.company.bioprotech.StoqtyLine', {});

                                if (gu.getCmp('newstoshkchoice').getValue()['stochkchoice2'] === 'new') {
                                    ...
                                    store.getProxy().setExtraParam("menuCode", 'PMS2_EXL_NEW');
                                    store.getProxy().setExtraParam('orderBy', 'item_code');
                                    store.getProxy().setExtraParam('wh_name', gu.getCmp('wh_name').getValue());
                                    store.getProxy().setExtraParam('new_stoshk_choice', gu.getCmp('newstoshkchoice').getValue());
                                } else {
                                    store.getProxy().setExtraParam("srch_type", 'excelPrint');
                                    store.getProxy().setExtraParam("srch_fields", 'major');
                                    store.getProxy().setExtraParam("srch_rows", 'all');
        ...
        this.gridStochkList = Ext.create('Ext.grid.Panel', {
            ...
            margin: '0 0 0 0',
            columns: [
                { text: '자재번호', width: 60, style: 'text-align:center', align: 'left', dataIndex: 'item_code' },
                { text: '품명', width: 130, align: 'left', style: 'text-align:center', dataIndex: 'item_name' },
                //{ text: '위치', width: 100, align: 'left', style: 'text-align:center', dataIndex: 'stock_pos' },
                {
                    text: '일부인', width: 80, align: 'right', style: 'text-align:center', dataIndex: 'exp_date',
                    renderer: function (value) {
                        return Ext.util.Format.date(value, 'Y-m-d');
                    }
                },
                { text: '조사재고', width: 80, align: 'right', style: 'text-align:center', dataIndex: 'check_qty' },
                { text: '시스템재고', width: 80, align: 'right', style: 'text-align:center', dataIndex: 'system_qty' },
                { text: '차이수량', width: 80, align: 'right', style: 'text-align:center', dataIndex: 'variable_qty' }
            ],
            name: 'cart',
            autoScroll: true,
            listeners: {
    ...
    whouseStore: Ext.create('Mplm.store.WareHouseStore', {})
