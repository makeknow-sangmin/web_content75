...
Ext.define('Rfx2.view.company.sejun.stockMgmt.MonthlySalesView', {
    extend: 'Rfx2.base.BaseView',
    xtype: 'monthly-sales-view',
    requires: [
    ...
    monthSalesStore: Ext.create('Rfx2.store.company.sejun.MonthSalesStore', { saveType:'STOCK-RI', classCode :' RI' }),
    ...
    initComponent: function () {
        ...
        this.monthSalesStore.on('load', function (me, records, c, d) {
                ...
                columns: [ 
                    {
                        ...
                        align: "right",
                        style: 'text-align:center',
                        dataIndex: 'transfer_total_gr_qty',
                        summaryType: 'sum',
                        renderer: function (value, context, tmeta) {
                            if (context.field == 'transfer_total_gr_qty') {
                                context.record.set('transfer_total_gr_qty', Ext.util.Format.number(value, '0,00/i'));
                            }
                            if (value == null || value.length < 1) {
                                value = 0;
                            }
                    ...
                    },
                    {
                        ...
                        align: "right",
                        style: 'text-align:center',
                        dataIndex: 'transfer_total_gr_price',
                        summaryType: 'sum',
                        renderer: function (value, context, tmeta) {
                            if (context.field == 'transfer_total_gr_price') {
                                context.record.set('transfer_total_gr_price', Ext.util.Format.number(value, '0,00/i'));
                            }
                            if (value == null || value.length < 1) {
                                value = 0;
                            }
            ...
            // console.log('테스트', gm.me().westGrid.bodyWrap.component.config.columns.items);
            console.log('테스트', gm.me().westGrid);
            gm.me().westGrid.bodyWrap.component.config.columns[0].text = year +'년 ' + month +'월 ' + ' 판매현황';
            gm.me().westGrid.reconfigure(undefined, gridColumns);
           
          
            
            // gm.me().westGrid.Columns[0].HeaderText = '111';
            // gm.me().westGrid
            // 데이터 모양 변경
            var newRecords = [];
        });
