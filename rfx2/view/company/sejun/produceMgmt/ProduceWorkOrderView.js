Ext.define('Rfx2.view.company.sejun.produceMgmt.ProduceWorkOrderView', {
    ...
    initComponent: function () {
        ...
        this.writeWorkHistory = Ext.create('Ext.Action', {
            iconCls: 'af-plus',
            text: '작업시간 등록',
            disabled: true,
            handler: function () {
                ...
                var form = Ext.create('Ext.form.Panel', {
                    ...
                    items: [
                        {
                            xtype: 'fieldset',
                            title: '작업시작 시간과 종료시간을 입력하시기 주십시오.',
                            items: [
                                {
                                    xtype: 'timefield',
                                    name: 'start_time',
                                ...
                                Ext.MessageBox.show({
                                    ...
                                    fn: function (btn) {
                                        if (btn == "no") {
                                            ...
                                            var rtgast_uid = rec.get('unique_id');
                                            ...
                                            Ext.Ajax.request({
                                                url: CONTEXT_PATH + '/index/process.do?method=addWorkTime',
        ...
        this.addWorkOrder = Ext.create('Ext.Action', {
            ...
            handler: function () {
                ...
                for(var i = 0; i<selection.length; i++) {
                    var rec = selection[i];
                    if (rec.get('reserved_timestamp3') === null) {
                        Ext.MessageBox.alert('알림', '일부인이 지정되지 않는 계획은 작업지시가 불가합니다.');
                        isProcess =false;
                        return;
                    }
                }
        ...
        buttonToolbar.insert(3, this.writeWorkHistory);
        ...
        this.setGridOnCallback(function (selections) {
            ...
            if (selections.length) {
                ...
                if (gm.me().vSELECTED_STATE == 'I') {
                    ...
                    gm.me().writeWorkHistory.enable();
                    // gm.me().printPDFAction.disable();
                } else if (gm.me().vSELECTED_STATE == 'N') {
                    this.refreshButtons(true);
                    gm.me().denyWorkOrder.disable();
                    gm.me().writeWorkHistory.disable();
                    // gm.me().printPDFAction.enable();
                        ...
                        fn: function (btn) {
                            if (btn == "no") {
                                ...
                                for (var i = 0; i < selections.length; i++) {
                                    ...
                                    ac_uids.push(ac_uid);
                                    var order = rec.get('order_number');
                                    order_number.push(order);
                                    var srcahd = rec.get('srcahd_uid');
                                    srcahd_uid.push(srcahd);
                                    var tem_quan = rec.get('item_quan');
                                    item_quan.push(tem_quan);
                                    var type = rec.get('po_type');
                                    po_type.push(type);
                                    var spec = rec.get('specification');
                                    specification.push(spec);
                                    var assymap = rec.get('coord_key');
                                    assymap_uid.push(assymap);
                                    var cartmap = rec.get('coord_key1');
                                    cartmap_uid.push(cartmap);
                                }

                                prWin.setLoading(true);
                                form.submit({
                                    url: CONTEXT_PATH + '/index/process.do?method=addWorkOrderGeneralMulti',
                                    params: {
                                        order_numbers: order_number,
                                        srcahd_uids: srcahd_uid,
                                        item_quans: item_quan,
                                        po_types: po_type,
                                        specifications: specification,
                                        ac_uids: ac_uids,
                                        rtgast_uids: rtgastUids,
                                        assymap_uids: assymap_uid,
                                        cartmap_uids: cartmap_uid,
                                    },
                                    success: function (val, action) {
                                        var myWin = prWin;
                                        workOrder.disable();
