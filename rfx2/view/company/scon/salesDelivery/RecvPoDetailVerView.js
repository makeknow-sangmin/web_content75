Ext.define('Rfx2.view.company.scon.salesDelivery.RecvPoDetailVerView', {
                ...
                var form = Ext.create('Ext.form.Panel', {
                    ...
                    items      : [
...

                        {
                            xtype      : 'fieldset',
                            collapsible: false,
                            title      : '선택한 품목을 출하로 진행할 경우<br>아래 체크박스를 선택 후 차량번호를 기입하여 주시기 바랍니다.',
                            width      : '100%',
                            style      : 'padding:10px',
                            defaults   : {
                                labelStyle: 'padding:10px',
                                    ...
                                    items         : [
                                        {
                                            xtype     : 'checkbox',
                                            padding   : '0 0 5px 30px',
                                            name      : 'is_delivery_out',
                                            id        : gu.id('is_delivery_out'),
                                            fieldLabel: '출하진행 여부',
                                            name      : 'is_delivery_out',
                                            checked   : false
                                        },
                        ...
                        handler: function (btn) {
                            if (btn == "no") {
                                ...
                                if (form.isValid()) {
                                    ...
                                    var order_com_unique = selections.get('order_com_unique');
                                    win.setLoading(true);
                                    const is_delivery_out = gu.getCmp('is_delivery_out').getValue();
                                    console_logs('>>>> is_delivery_out ', is_delivery_out);
                                    var loss_item_name = '';
                                    if (is_delivery_out) {
                                        // 순차적인 재고 분석 후
                                        // 한번에 모아서 출하요청 후 납품서 생성하는 로직으로 한다.
                                        var srcahdUids = [];
                                        var prQuans = [];
                                        ...
                                        for (var i = 0; i < record.length; i++) {
                                            ...
                                            srcahdUids.push(selections.get('srcahd_uid'));
                                            prQuans.push(selections.get('po_qty'))
                                        ...
                                        if (isOk === true) {
                                            Ext.Ajax.request({
                                                ...
                                                success: function (result, request) {
                                                    ...
                                                    console_logs('판정결과 >>> ', resultTxt);
                                                    if (resultTxt === 'true') {
                                                        Ext.Ajax.request({
                                                            url    : CONTEXT_PATH + '/sales/delivery.do?method=directPoToDelivery',
                                                            params : {
                                                                sloast_uids      : sloastUids,
                                                                project_uid      : project_uid,
                                                                delivery_req_date: gu.getCmp('delivery_req_date').getValue(),
                                                                srcahd_uids      : srcahdUids,
                                                                car_uid          : gu.getCmp('carinfo').getValue(),
                                                                warehouse_uid : 101,
                                                        ...
                                                        Ext.MessageBox.show({
                                                            title  : '알림',
                                                            msg    : '재고가 부족한 품목이 있습니다.<br>그래도 진행하시겠습니까?',
                                                            buttons: Ext.MessageBox.YESNO,
                                                            icon   : Ext.MessageBox.QUESTION,
                                                            fn     : function (btn) {
                                                                if (btn == "no") {
                    ...
                    fn     : function (btn) {
                        if (btn == 'yes') {
                            ...
                            for (var i = 0; i < selections.length; i++) {
                                ...
                                if (rec.get('reserved6') === 'PDR' || rec.get('reserved6') === 'GOC' || rec.get('reserved6') === 'PDC') {
                                    Ext.MessageBox.alert('알림', '계약취소를 할 상태가 아닌 건이 선택되었습니다.<br>다시 확인해주세요.');
                                    isAction = false;
                                    break;
                                } else {
                            ...
                            if (isAction == true) {
                                ...
                                //     Ext.MessageBox.show({
                                //         title: '계약취소',
                                //         msg: '선택한 건들에 대하여 계약취소 처리를 하시겠습니까?',
                                //         buttons: Ext.MessageBox.YESNO,
                                //         fn: function (btn) {
                                //             if (btn == 'yes') {
                                Ext.Ajax.request({
