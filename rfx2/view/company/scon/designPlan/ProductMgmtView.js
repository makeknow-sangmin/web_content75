//자재 관리
Ext.define('Rfx2.view.company.scon.designPlan.ProductMgmtView', {
    ...
    initComponent: function () {
        ...
        //
        //     var mid_level = Ext.getCmp('design-plan-' + gm.me().link + '-mid_code');
        //     var store = mid_level.getStore();
        //
        //     mid_level.setValue(null);
        //     store.getProxy().setExtraParam('parent_class_code', o.value);
        //
        //     store.load();
        // });
        ...
        this.addCallback('SET_ITEM_CODE', function (o) {
            var sg_code = Ext.getCmp('design-plan-' + gm.me().link + '-sg_code');
            console_logs('>>>> sg_code', sg_code.value);
            gm.me().getInputJust('extendsrcahd|item_code').setValue(sg_code.value);
        });
            ...
            handler: function (widget, event) {
                Ext.MessageBox.show({
                    ...
                    fn: function (result) {
                        ...
                        var list = gm.me().grid.getSelectionModel().getSelection();
                        var selected = [];
                        $(list).each(function() {
                                selected.push(this.get('unique_id'));
                        });

                        console_logs("상민입니다.", selected);
                        if (result == 'yes') {
                            Ext.Ajax.request({
                                url: CONTEXT_PATH + '/index/process.do?method=deleteProducts',
                                params: {
                                    srcahd_uid: selected
                                },
                                success: function (result, request) {
