...
Ext.define('Rfx2.view.company.chmr.salesDelivery.ProductStockVerView', {
    ...
    doForcePrevStockChange: function () {
        var selections = gm.me().grid.getSelectionModel().getSelection();
        this.changeQtyInfoGrid = Ext.create('Ext.grid.Panel', {
            store      : new Ext.data.Store(),
            cls        : 'rfx-panel',
            id         : gu.id('changeQtyInfoGrid'),
            multiSelect: false,
            autoScroll : true,
            viewConfig : {
                markDirty: false
        ...
        this.changeQtyInfoGrid.getStore().remove();
        for (var i = 0; i < selections.length; i++) {
            this.changeQtyInfoGrid.getStore().add(selections[i]);
        }
        ...
        var form = Ext.create('Ext.form.Panel', {
            ...
            items        : [
                {
                    xtype: 'fieldset',
                    title: '<b>선택한 창고로 선택한 제품의 재고수량을 일괄 변경합니다.<br><font color=red>이 작업을 시행할 시 취소할 수 없습니다.</font></b>',
                    items: [
                        {
                            fieldLabel  : '창고선택',
                            xtype       : 'combo',
        ...
        var prWin = Ext.create('Ext.Window', {
            ...
            buttons: [{
                ...
                handler: function (btn) {
                    ...
                    var store = gu.getCmp('changeQtyInfoGrid').getStore();
                    ...
                    for (var i = 0; i < store.getCount(); i++) {
                        ...
                        var move_qty = rec.get('move_qty');
                        if (move_qty === 0) {
                            is_move = false;
                            break;
                        } else {
                            srcahd_uids.push(rec.get('unique_id_long'));
                            diff_qtys.push(rec.get('diff_qty'));
                        }
                    }

                    var whouse_uid = gu.getCmp('warehouse_uid').getValue();
                    var reason_text = gu.getCmp('reason_text').getValue();

                    if (selection.length > 0) {
                        var rec = selection[0];
                        form.submit({
                            url    : CONTEXT_PATH + '/index/process.do?method=forceMaterialPrevStockMulti',
                            params : {
    ...
    moveWareHouse: function () {
        ...
        this.moveQtyInfoGrid = Ext.create('Ext.grid.Panel', {
            store      : new Ext.data.Store(),
            cls        : 'rfx-panel',
            id         : gu.id('moveQtyInfoGrid'),
            multiSelect: false,
            autoScroll : true,
            viewConfig : {
                markDirty: false
        ...
        this.moveQtyInfoGrid.getStore().remove();
        for (var i = 0; i < selections.length; i++) {
            this.moveQtyInfoGrid.getStore().add(selections[i]);
        }
        ...
        var form = Ext.create('Ext.form.Panel', {
            ...
            items        : [
                {
                    xtype: 'fieldset',
                    title: '선택 제품에 대한 창고이동을 실시합니다.<br><b>선택한 제품에 대하여 출발창고와 도착창고를 지정하시기 바랍니다.</b>',
                    items: [
                        // {
                        //     xtype: 'textfield',
                        //     anchor: '100%',
                ...
                {
                    xtype: 'fieldset',
                    title: '선택한 제품들의 이동수량을 정확하게 기입해주시기 바랍니다.<br>이동수량이 입력되지 않으면 실행이 되지 않습니다.',
                    items: [
                        this.moveQtyInfoGrid
                        // {
                        //     xtype: 'numberfield',
                        //     anchor: '100%',
                        //     fieldLabel: '이동수량',
            ...
            buttons: [{
                ...
                handler: function (btn) {
                    ...
                    if (selection.length > 0) {
                        ...
                        var store = gu.getCmp('moveQtyInfoGrid').getStore();
                        ...
                        for (var i = 0; i < store.getCount(); i++) {
                            ...
                            if (move_qty === 0) {
                                ...
                                srcahd_uids.push(rec.get('unique_id_long'));
                                move_qtys.push(rec.get('move_qty'));
                            }
                        }
                        ...
                        if (is_move === true) {
                            ...
                            Ext.Ajax.request({
                                url    : CONTEXT_PATH + '/index/process.do?method=moveWarehouseQtyMulti',
                                waitMsg: '데이터를 처리중입니다.<br>잠시만 기다려주세요.',
                                params : {
                                    srcahd_uids   : srcahd_uids,
                                    move_qtys     : move_qtys,
                                    to_warehouse  : to_warehouse, // 도착창고
                                    from_warehouse: from_warehouse, // 출발창고
                                },
                                success: function (val, action) {
    ...
    adjustStockManual: function () {
        var store =  Ext.create('Rfx2.store.company.chmr.ProductListStore', {});

        this.adjustStockGrid = Ext.create('Ext.grid.Panel', {
            store      : store,
            cls        : 'rfx-panel',
            id         : gu.id('adjustStockGrid'),
            multiSelect: false,
            autoScroll : true,
            viewConfig : {
                markDirty: false
            ...
            items        : [
                {
                    ...
                    items: [
                        ...
                        },
                        {
                            ...
                            xtype       : 'combo',
                            anchor      : '100%',
                            id          : gu.id('select_product_type'),
                            name        : 'select_product_type',
                            mode        : 'local',
                            store       : Ext.create('Mplm.store.SpCodeFlagProductStockStore', {not_sp_code : 'ALL'}),
                            displayField: 'display',
                            valueField  : 'value',
                            emptyText   : '선택',
                            sortInfo    : {field: 'display', direction: 'DESC'},
                            typeAhead   : false,
                            minChars    : 1,
                            listConfig  : {
                                loadingText: '검색중...',
                            ...
                            listeners   : {
                                select: function (combo, record) {
                                    let sp_code = gu.getCmp('select_product_type').getValue();
                ...
                {
                    xtype: 'fieldset',
                    title: '해당일자의 반영재고를 정확하게 입력해주시기 바랍니다.',
                    items: [
                        this.adjustStockGrid
                    ]
                }
            ]
        });
        ...
        var prWin = Ext.create('Ext.Window', {
            ...
            buttons: [{
                ...
                handler: function (btn) {
                    ...
                    let item_codes = [];
                    let head_quarter_qty = [];
                    let first_yard_qty = [];
                    let second_yard_qty = [];
                    let stock_extras = [];

                    var store = gu.getCmp('adjustStockGrid').getStore();

                    for (var i = 0; i < store.getCount(); i++) {
                        var rec = store.getAt(i);
                        item_codes.push(rec.get('item_code'));
                        head_quarter_qty.push((rec.get('headquarter_stock') == null || rec.get('headquarter_stock') == undefined) ? 0.0 : rec.get('headquarter_stock'));
                        first_yard_qty.push((rec.get('first_yard_stock') == null || rec.get('first_yard_stock') == undefined) ? 0.0 : rec.get('first_yard_stock'));
                        second_yard_qty.push((rec.get('second_yard_stock') == null || rec.get('second_yard_stock') == undefined) ? 0.0 : rec.get('second_yard_stock'));
                        stock_extras.push(rec.get('reserved5').length == 0 ? '-' : rec.get('reserved5'));
                    }
                    console_logs('>>>> item_codes', item_codes);
                    console_logs('>>>> head_quarter_qty', head_quarter_qty);
                    console_logs('>>>> first_yard_qty', first_yard_qty);
                    console_logs('>>>> second_yard_qty', second_yard_qty);
                    console_logs('>>> reserved5' , stock_extras);

                    let full_date = gu.getCmp('aprv_date').getValue().getFullYear() + '-'
                       +'-' + (gu.getCmp('aprv_date').getValue().getMonth() + 1 < 10 ?  '0' + (gu.getCmp('aprv_date').getValue().getMonth() + 1) : gu.getCmp('aprv_date').getValue().getMonth() + 1 )
                        + '-'  + (gu.getCmp('aprv_date').getValue().getDate() < 10 ?  '0' + gu.getCmp('aprv_date').getValue().getDate() : gu.getCmp('aprv_date').getValue().getDate());

                    Ext.Ajax.request({
                        url : CONTEXT_PATH + '/sales/sps1.do?method=adjustStockManual',
                        waitMsg: '데이터를 처리중입니다.<br>잠시만 기다려주세요.',
                        timeout: 700000,
                        params : {
                            item_codes : item_codes,
                            head_quarter_qty : head_quarter_qty,
                            first_yard_qty  : first_yard_qty,
                            second_yard_qty: second_yard_qty,
                            pr_uid : 1,
                            aprv_date : full_date,
                            reserved_varchar1 : gu.getCmp('reserved_varchar1').getValue(),
                            stock_extras : stock_extras
                        },
                        success: function (val, action) {
                            prWin.setLoading(false);
                            Ext.Msg.alert('완료', '처리 되었습니다.');
    ...
    // productList : Ext.create('Rfx2.store.company.chmr.ProductListStore', {})
});
