    ...
    initComponent: function () {
        ...
        this.gridContractCompany = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                { text: this.getMC('msg_order_grid_prd_fam', '제품군'), width: 100, style: 'text-align:center', dataIndex: 'class_name', sortable: true },
            ...
            listeners: {
                ...
                edit: function (value, context, ditor, e, eOpts) {
                    ...
                    if (context.field === 'item_quan') {
                        ...
                        var box_pkg_qty = Number(record.get('finance_rate'));
                        var edit_box_qty = Math.round(load_qty * box_pkg_qty);
                        console_logs('수정된 박스 갯수 >>', edit_box_qty);
                        // if (tempBoxQuan > record.get('box_quan')) {
                        //     Ext.MessageBox.show({
                        //         title: '확인',
                        //         msg: '기존 계산된 BOX 갯수보다 입력 갯수가 작습니다.<br>수정을 진행하시겠습니까?',
                        //         buttons: Ext.MessageBox.YESNO,
                        //         fn: function (result) {
                        //             if (result == 'yes') {
                        //                 secondRecord = gu.getCmp('savelist').getStore().getAt(gu.getCmp('savelist').getStore().indexOf(record));
                        //                 secondRecord.set('box_quan', record.get('box_quan'));
                        //             } else {
                        //                 secondRecord = gu.getCmp('savelist').getStore().getAt(gu.getCmp('savelist').getStore().indexOf(record));
                        //                 secondRecord.set('box_quan', tempBoxQuan);
                        //             }
                        //         },
                        //         icon: Ext.MessageBox.QUESTION
                        //     });
                            ...
                            listeners: [{
                                click: function () {
                                    ...
                                    if (palletListSel != undefined && lotListSel != undefined) {
                                        ...
                                        var box_quan = lotListSel.get('pr_quan') / lotListSel.get('finance_rate');
                                        ...
                                        if (isInsert == true) {
                                            store.insert(store.getCount(), new Ext.data.Record({
                                                ...
                                                'item_quan': lotListSel.get('real_wh_qty') - preload_qty,
