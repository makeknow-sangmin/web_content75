Ext.define('Rfx2.view.company.chmr.designPlan.MaterialMixHistoryView', {
    ...
    initComponent: function () {
        ...
        this.mixGrid = Ext.create('Ext.grid.Panel', {
            region: 'west',
            frame: true,
            flex: 0.3,
            ...
            listeners: {
                'selectionchange': function (view, records) {
                    ...
                    if (records != null && records.length > 0) {
                        ...
                        const dateRangeId = 'MIX_HISTORY' + '-' + gMain.getSearchField('reserved_timestamp1');
                        console_logs('>>>>> dateRangeId', dateRangeId);
                        const sDateField = Ext.getCmp(dateRangeId + '-s');
                        const eDateField = Ext.getCmp(dateRangeId + '-e');
                        const sDate = Ext.Date.format(sDateField.getValue(), 'Y-m-d');
                        const eDate = Ext.Date.format(eDateField.getValue(), 'Y-m-d');
                        const dateFull = sDate + ':' + eDate;

                        gm.me().store.getProxy().setExtraParams({
                            bomver_uid: record.get('unique_id_long'),
                            reserved_timestamp1 : dateFull
                        })
        ...
        this.createStore('Rfx2.model.company.chmr.MaterialMixRate', [{
        }],
            gm.pageSize
            , {}
            , ['bomdtl']
    ...
    editPartAction: Ext.create('Ext.Action', {
        ...
        handler: function (widget, event) {
            ...
            var bomdtlRec = gm.me().grid.getSelectionModel().getSelection()[0];
                ...
                gm.me().createPartForm = Ext.create('Ext.form.Panel', {
                    ...
                    items: [
                        ...
                        },
                        {
                            ...
                            allowBlank: true,
                            fieldStyle: 'background-color: #EAEAEA; background-image: none;'
                            , value: bomdtlRec.get('specification')
                        },
                        {
                            //     xtype: 'textfield',
                            //     fieldLabel: gm.me().getColName('maker_name'),
                            ...
                            name: 'reserved_integer1',
                            // value: reserved_integer1 + 1,
                            value: bomdtlRec.get('reserved_integer1')
                        }, {
                            xtype: 'textfield',
                            fieldLabel: '단위',
                            emptyText: '단위',
                            ...
                            emptyText: 'BOMDTL UID',
                            // value: -1, // 추가시 -1
                            value: bomdtlRec.get('unique_id_long'),
                            width: '100%',
                            readOnly: true,
                            hidden: true,
                            fieldStyle: 'background-color: #EAEAEA; background-image: none;'
                            ...
                            name: 'reserved_integer2',
                            // value: reserved_integer2,
                            value: bomdtlRec.get('reserved_integer2'),
                            emptyText: '제품ASSY_UID',
                            readOnly: true,
                            hidden: true,
                            width: '100%',
    ...
    calcRatioConfirm: function (result) {
        if (result == 'yes') {
            ...
            data.items.forEach(el => {
                ...
                tempquanArr.push(el.get('temp_quan'));
            });
            data.items.forEach(el => {
    ...
    registPartFc: function (val) {
        ...
        if (standard_flag == 'A') {
            ...
            Ext.Ajax.request({
                ...
                success: function (result, request) {
                    gm.me().store.getProxy().setExtraParam('bomver_uid', gm.me().selected_tree_record.get('unique_id_long'));
