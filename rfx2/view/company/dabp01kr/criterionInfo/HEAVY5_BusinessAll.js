 ...
 */
Ext.define('Rfx2.view.company.dabp01kr.criterionInfo.HEAVY5_BusinessAll', {
...
//		this.addSearchField('state');
		...
		buttonToolbar.insert(2, this.confirmLogAction);
		// buttonToolbar.insert(3, this.returnLogAction);
				...
				handler: function(btn){
						...
						if(selections == null || selections.length < 1) {
							...
							for(var i=0; i<selections.length; i++) {
								...
								if(rec.get('state') == 'A' || rec.get('state') == 'I') {
									Ext.MessageBox.alert(error_msg_prompt, '이미 결재완료 혹은 팀장 결재중인 일지가 있습니다.');
									return;
								}
								ids.push(rec.get('unique_id_long'));
							}

							var reserved_varcharb = Ext.getCmp('reserved_varcharb').getValue();
