//수주관리 메뉴
Ext.define('Rfx2.view.company.dabp01kr.salesDelivery.RecvStockView', {
	extend: 'Rfx2.view.salesDelivery.RecvStockView',
    xtype: 'receved-stock-mgmt-view',
    inputBuyer : null,
    initComponent: function(){
    	...
    	this.setDefComboValue('pm_uid', 'valueField', -1); // Hidden
																		// Value임.
    	this.setDefComboValue('pj_type', 'valueField', 'R');
...
//        });
    	...
    	this.refreshActiveCrudPanel = function(source, selectOn, unique_id, record) {
    		if(selectOn==true) {
				...
				// gm.me().getInputJust('map|child').setValue(rec.get('srcahd_uid'));
				// gm.me().getInputJust('partline|stock_qty_safe').setValue(rec.get('stock_qty_safe'));
...
// this.addReadonlyField('unique_id');
        ...
        this.createStore('Rfx.model.RecevedStockMgmt', [{
	            property: 'unique_id',
	            direction: 'DESC'
	        }],
			gMain.pageSize,
			...
			/*this.store.getProxy().setExtraParam('ctr_flag', 'Y');*/
			this.store.getProxy().setExtraParam('is_def', 'Y');
			this.store.getProxy().setExtraParam('pj_type', 'R'); // 수주관리(재고생산)
			this.store.getProxy().setExtraParam('template', 'Y');
	...
	//				var recvPoAssyGrid = Ext.getCmp('recvPoAssyGrid');
					...
					if(recvPoAssyGrid == null || recvPoAssyGrid == undefined) {
						recvPoAssyGrid = Ext.getCmp('recvStockAssyGrid1');
					}
			...
			var removePartRowAction = Ext.create('Ext.Action',{
				...
				disabled : true,
				handler : function() {
					var recvPoAssyGrid = Ext.getCmp('recvStockAssyGrid1');
					// var recvPoAssyGrid = gMain.selPanel.recvPoPartGrid;
...
//							                    calQuan,
					...
					grid.on('edit', function(grid, data) {
						...
						// if(field == 'sp_code') {
						// 	var rec = Ext.getCmp('recvStockAssyGrid1').getSelectionModel().getSelection()[0];
...
//        	gMain.selPanel.vSELECTED_UNIQUE_ID = vCUR_USER_UID*(100);
            if (selections.length) {
				...
				gMain.selPanel.vSELECTED_COMPANY_CODE = rec.get('company_code');
...
//    			gMain.selPanel.prchHistory(rec.get('ac_uid'));
				...
				gMain.selPanel.recvDateStore.getProxy().setExtraParam('company_code', gMain.selPanel.vSELECTED_COMPANY_CODE);
            						    ...
            						    Ext.Ajax.request({
            								...
            								success : function(result, request) { 
            							          // gMain.selPanel.vSELECTED_UNIQUE_ID =
            										// vCUR_USER_UID*(100);
            							          var processGrid = Ext.getCmp('recvPoPcsGrid'/* gMain.getGridId() */);
            							          processGrid.getStore().getProxy().setExtraParam('assymap_uid',  gMain.selPanel.vSELECTED_UNIQUE_ID);// 사용자 UID																									// UID																								// 임시생성
            							          processGrid.getStore().load();
...
// 		var target_bm_quan = this.getInputTarget('bm_quan');
// 		var bm_quan = target_bm_quan.getValue();
// 		console_logs('>>>aaaa', Ext.getCmp(	this.link + '-'+ 'grid-top-pqty'+'2'));
// 		console_logs('>>>bbbb', val);
// 		console_logs('>>>cccc', item_name);
// 		console_logs('>>>dddd', bm_quan);
...
//					console_logs('prchHistory>>>>>>>>>>',mtrlInfo);
    ...
    StoreLoadRecordSet: function(records){
       	for(var i=0; records!=null && i<records.length; i++) {
			...
			var company_code = rec.get('company_code');
			...
			rec.set('company_code', company_code);
    ...
    savePcsstdHandler: function() {
		 ...
		 // console_logs('gridPcsStd', gridPcsStd);
			...
			var target_bm_quan = gMain.selPanel.getInputTarget('bm_quan');
			  ...
			  var prevQty = 0;
			  try {
				  prevQty = Number(target_bm_quan.getValue());
			  } catch (error) {
				  prevQty = 0;
			  }
	    	  // var tomCheck = false;
...
//		 },
        ...
        //      switch(newTab.title) {
 	    //         case '원지':
 	    //         	gMain.selPanel.recvPoPartGrid=Ext.getCmp('recvStockAssyGrid1');
        ...
        //  },
         refleshCopyForm: function(){
        	...
        	//console_logs('copyCallback', '수주 복사등록');
			 ...
			 console_logs('>>>> dqdqqd', gm.me().grid.getSelectionModel().getSelection()[0]);

			 var select = gm.me().grid.getSelectionModel().getSelection()[0];
				 class_code = select.get('class_code');
				 wa_code = select.get('wa_code');
				 item_first = wa_code.substring(0,1)+ class_code.substring(0,1);
			//  return;
 			// 마지막 수주번호 가져오기
 		   Ext.Ajax.request({
 				url: CONTEXT_PATH + '/sales/poreceipt.do?method=lastnoMes',
