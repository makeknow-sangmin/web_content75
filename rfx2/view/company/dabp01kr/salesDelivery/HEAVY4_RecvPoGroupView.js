//수주관리 메뉴 = 대동 (현대수주)
Ext.define('Rfx2.view.company.dabp01kr.salesDelivery.HEAVY4_RecvPoGroupView', {
    extend: 'Rfx2.base.BaseView',
    xtype: 'receved-mgmt-view',
    requires: [
		'Rfx.util.CheckboxGroupingFeature'
	     	],
    ...
    initComponent: function(){
		...
		case "DDNG01KR":
			this.addSearchField({
				...
				field_id: 'sub_group_name',       // js에서의 필드 명
				fieldName: 'reserved_varchar4' //실재 Table Field Name
			});
...
//	    }
    	...
    	this.createStoreSimple({
	        ...
	        deleteClass : 'assymap',
		    	...
		    	groupField: 'pj_reserved_varchar4'
	    });
        ...
        for(var i=0; i< this.columns.length; i++) {
        	...
        	case 'reserved_double2':
        		...
        		o['summaryRenderer'] = function(value, summaryData, dataIndex) {
                	...
                	value = '<div><font style="font-size:10pt; color:#FF0040;">'+value +'</font></div>'
                	return value;
                };
        		break;
        	...
        	case 'comment':
        		o['summaryType'] = 'count';
        		o['summaryRenderer'] = function(value, summaryData, dataIndex) {
                	value = '<div><font style="color:blue;">합계</font> '+'(' + value + ')'+'</div>'
                	return value;
                };
        ...
        case "DDNG01KR":
    		var option = {
				features: [{
			        ...
			        startCollapsed: false // produces strange bug
			      }]
        ...
        this.minLotAction = Ext.create('Ext.Action', {
			 ...
			 handler: function() {
				 var form = Ext.create('Ext.form.Panel',{
					 ...
					 items : [{
						 fieldLabel: '영업LOT',		//reserved13
		                   xtype : 'textfield',
		                   name : 'reserved13'
					 },{
						 fieldLabel: '납기',			//reserved14
		                   xtype : 'datefield',
		                   name : 'reserved14',
                           format: 'Y-m-d',
				 ...
				 //window
			        ...
			        buttons: [{
			        	...
			        	handler: function(btn){ 
			        		var form = Ext.getCmp('textform').getForm();
			            	...
			            	var selections = gm.me().grid.getSelectionModel().getSelection();
							 if (selections) {
					            	...
					            	gm.me().addCombineSubLotAuto(val['reserved13'], val['reserved14'], val['reserved15'], pj_uids);
							 } else {
							    	Ext.MessageBox.show({
							            title:'확인',
							            msg: '선택한 자재가 없습니다. 요청되지 않은 모든자재를 일괄진행 하시겠습니까?.',
							            buttons: Ext.MessageBox.YESNO,
							            fn:  function(result) {
					            	        if(result=='yes') {
					            	        	gm.me.addCombineSubLotAuto(val['reserved13'], val['reserved14'], val['reserved15'], null);
					            	        } else {
...
//});
        ...
        var gFeature = view.features[0]; //I júst have one feautre... so [0] works

        console_logs('gFeature', gFeature);
        gFeature.startCollapsed = true;
        gFeature.isCollapsed = true;
...
//    },
  ...
  //도장생산요청 - 신화
   /* doRequestPaint: function() {
    ...
    addCombineSubLotAuto : function(reserved13, reserved14, reserved15, pj_uids) {
    	
    	var params = {};
    	params['reserved13'] = reserved13;
    	params['reserved14'] = reserved14;
    	params['reserved15'] = reserved15;
    	
    	if(pj_uids==null) { //대기중인 모든자재
    		params['all_pending'] = true;
    	} else {
    		params['pj_uids'] = pj_uids;
    		params['all_pending'] = false;
    	}
    	
		
		Ext.Ajax.request({
			 url: CONTEXT_PATH + '/index/process.do?method=addCombineSubLotAuto',
			 params: params,
