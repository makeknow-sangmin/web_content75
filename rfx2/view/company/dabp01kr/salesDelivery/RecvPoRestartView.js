...
//수주관리 메뉴
Ext.define('Rfx2.view.company.dabp01kr.salesDelivery.RecvPoRestartView', {
    extend: 'Rfx2.base.BaseView',
    xtype: 'receved-restart-view',
    inputBuyer : null,
...
// this.addSearchField('수주건수');
		...
		this.addCallback('AUTO_PJCODE', function(o){
			if(this.crudMode=='EDIT') { // EDIT
				...
				if(pj_code.length==9){
					pj_code = pj_code.substring(0,9)+'R';
				}else{
					pj_code = pj_code.substring(0,10);
				}
...
//				target.setValue(target.getvalue()+'R1');
		...
		this.addCallback('SET_DEPT', function(combo, record){
			var dept_code = record.get('dept_code');
			gUtil.deptUserStore.getProxy().setExtraParam('dept_code', dept_code);
			gUtil.deptUserStore.load();
		...
		this.addCallback('THIS_PROCESS', function(combo, record){
			console_logs('THIS_PROCESS record', record);
			console_logs('combo', combo);

			var pcs_code = record.get('system_code');
			console_logs('THIS_PROCESS pcs_code', pcs_code);
			gUtil.deptUserStore.getProxy().setExtraParam('gubun', true);
			gUtil.deptUserStore.getProxy().setExtraParam('pcs_code', pcs_code);
			gUtil.deptUserStore.load();
		});

		this.addCallback('GET_DEPT_USER', function(combo, record){
			console_logs('GET_DEPT_USER record', record);
			console_logs('combo', combo);
		});
...
//			_reserved11.setValue(record.get('stock_qty'));
       ...
       buttonToolbar.insert(3, this.registCopyAction);
        ...
        this.createStore('Rfx.model.RecevedMgmtRestart', [{
...
//			                    fieldStyle: 'background-color: white; background-image: none;',
			                    listeners : {
			                    	change: function(field, value) {
			                    		reserved_varchar77 = Ext.getCmp('reserved_varchar7').getValue();
...
//                	}
        ...
        this.setStopView = Ext.create('Ext.Action', {
        	 xtype : 'button',
 			 text: '중단List',
 			 tooltip: '재작업대기 List',
 			pressed: true,
 			toggleGroup: 'stockviewType',
 			 handler: function() {
 				gm.me().store.getProxy().setExtraParams({});
 				gm.me().store.getProxy().setExtraParam('not_restart', 'R');
 				gm.me().store.load();
 				 
 			 }
 		});
      this.setRestartView = Ext.create('Ext.Action', {
       	 xtype : 'button',
 			 text: '재작업List',
 			 tooltip: '재작업진행 List',
 			toggleGroup: 'stockviewType',
 			 handler: function() {
 				gm.me().store.getProxy().setExtraParams({});
 				gm.me().store.getProxy().setExtraParam('restart', 'R');
 				gm.me().store.load();
 			 }
 		});
        ...
        buttonToolbar.insert(7, this.setRestartView);
        buttonToolbar.insert(7, this.setStopView);
...
//    				gUtil.disable(gMain.selPanel.editAction);   
    			...
    			gMain.selPanel.CostStore.getProxy().setExtraParam('ac_uid', rec.get('ac_uid'));
    			gMain.selPanel.CostStore.getProxy().setExtraParam('srcahd_uid', rec.get('srcahd_uid'));
...
//		var bm_quan1 = target_bm_quan1.getValue();
		Ext.getCmp(	this.link + '-'+ 'grid-top-paperQty' ).setText('원지수량: ' + bm_quan);
