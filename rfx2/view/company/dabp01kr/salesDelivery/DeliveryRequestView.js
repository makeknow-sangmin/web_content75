//출하 현황
Ext.define('Rfx2.view.company.dabp01kr.salesDelivery.DeliveryRequestView', {
...
//					var reserved_double5 = rec.get('reserved14');//적재수량
				...
				var sub_code = wa_code + '-SUB';
				var sub_store = Ext.create('Mplm.store.CombstSubStore',{});
				sub_store.getProxy().setExtraParam('wa_group_only', sub_code);
...
//									html: msg,
								...
								,{
									...
									fieldLabel: '납품처',
									name: 'sub_delivery',
									emptyText: '미지정시 고객사 주소로 이동',
									anchor: '100%',
									store: sub_store,
									displayField: 'wa_name',
									valueField: 'wa_code',
									innerTpl	: '<div data-qtip="{unique_id}">{wa_name}</div>',
					                triggerAction: 'all',
					                listeners: {
					                    select: function(combo, record) {
					                       console_logs('>>>>>>>>>>>>> sss', record);
										   var address = record.get('address_1');
										   var uid = record.get('unique_id');
										   Ext.getCmp('delivery_info').setValue(address);
										   Ext.getCmp('work_cursor').setValue(uid);
					                    }
					                }
								},new Ext.form.Hidden({
									id: 'work_cursor',
...
//    	} else {
			...
			SubDeliveryUid = val['work_cursor']; // 납품처가 있을시에 unique_id
