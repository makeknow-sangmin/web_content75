//자재 관리
Ext.define('Rfx2.view.company.dabp01kr.designPlan.MaterialMgmtView', {
        ...
        case 'DABP01KR' :
	        this.setRawMatView = Ext.create('Ext.Action', {
	   			...
	   			toggleGroup: 'stockviewType',
	   			 handler: function() {
	//   				 this.matType = 'RAW';
	   				gm.me().store.getProxy().setExtraParam('item_type', 'ROLL');
	   				gm.me().store.load();
	        ...
	        this.setSubMatView = Ext.create('Ext.Action', {
	         	 xtype : 'button',
	   			...
	   			toggleGroup: 'stockviewType',
	   			 handler: function() {
	//   				 this.matType = 'SUB';
	   				gm.me().store.getProxy().setExtraParam('item_type', 'SHEET');
	   				gm.me().store.load();
	   			 }
	   		});
	        	break;
