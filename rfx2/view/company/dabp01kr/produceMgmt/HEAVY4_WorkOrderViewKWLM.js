Ext.define('Rfx2.view.company.dabp01kr.produceMgmt.HEAVY4_WorkOrderViewKWLM', {
    extend: 'Rfx2.base.BaseView',
    xtype: 'workorder-view-kwlm',
    selectedRecord: null,
...
//        }
        ...
        this.setGridOnCallback(function(selections) {
            ...
            if (selections==null || selections.length ==0) {
                ...
                var rtgast_uid = select.get('unique_id_long');
                if(rtgast_uid != null) {
                    gm.me().lotDetailStore.getProxy().setExtraParam('rtgast_uid', rtgast_uid);
                    gm.me().lotDetailStore.load(function(data) {
                        var total_qty = 0;
                        for(var i=0; i<data.length; i++) {
                            var qty = data[i].get('bm_quan');
                            total_qty += qty;
                        }
                        gm.me().buttonToolbar3.items.items[1].update('총 제품수 : ' + data.length + ' / 총 수량 : ' + total_qty);
                    });
...
//    					});
    ...
    createEast: function() {
        this.lotDetailStore = Ext.create('Mplm.store.LotDetailStore', {});
        ...
        console_logs('>>tool', this.buttonToolbar3);

        this.gridLotDetail = Ext.create('Ext.grid.Panel', {
            id:'detailLot',
            store: this.lotDetailStore,
            // cls : 'rfx-panel',
            collapsible: false,
            multiSelect: false,
            autoScroll : true,
