Ext.define('Rfx2.view.company.dabp01kr.produceMgmt.ProduceAdjustPcsMultiView', {
    ...
    initComponent: function(){
		...
		searchToolbar.insert(0, {
			...
			handler: function(box, value) {
				console_logs('>>> aaa', gm.me().store);
				var store = gm.me().store;
				store.getProxy().setExtraParam('paln_check', value);
			}
		});
		...
		searchToolbar.insert(8, {
			...
			handler: function(box, value) {
				console_logs('>>> aaa', gm.me().store);
				var store = gm.me().store;
				store.getProxy().setExtraParam('delivery_check', value);
			}
		});
		...
		var paln_check = Ext.getCmp('paln_check').getValue();
		var delivery_check = Ext.getCmp('delivery_check').getValue();
		this.store.getProxy().setExtraParam('paln_check', paln_check);
		this.store.getProxy().setExtraParam('delivery_check', delivery_check);
			
        var arr=[];
        arr.push(buttonToolbar);
        arr.push(searchToolbar);

        this.subTabGridEach = true;
		  ...
		  //tab grid 생성
	       var tab = this.createTabGrid('Rfx.model.ProduceAdjustPcs', items, 'big_pcs_code', arr, function(curTab, prevtab) {
	                    ...
	                    gm.me().multi_grid_id = multi_grid_id;
						  ...
						  gUtil.pcsLink = gm.me().link + '#' + multi_grid_id;
...
//	                        	console_logs('records',records);
				  ...
				  tab.on('edit', function(editor, e) {
					console_logs('===ssstab', editor);
