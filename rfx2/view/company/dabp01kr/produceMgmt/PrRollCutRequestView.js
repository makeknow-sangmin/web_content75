...
//구매요청
Ext.define('Rfx2.view.company.dabp01kr.produceMgmt.PrRollCutRequestView', {
...
//       
        ...
        if(vCompanyReserved4=='DABP01KR'){
            ...
            for(var i=0; i<this.searchField.length; i++) {
                var field_id = this.searchField[i]['field_id'];
                if(field_id == 'req_date') {
                    var sdate = this.searchField[i]['sdate'];
                    var edate = this.searchField[i]['edate'];

                    s_date = Ext.Date.format(sdate, 'Y-m-d');
                    e_date = Ext.Date.format(edate, 'Y-m-d');
                }
            }
