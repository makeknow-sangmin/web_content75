//수주등록된 전체 자재 메뉴
Ext.define('Rfx2.view.company.dabp01kr.produceMgmt.HEAVY4_ProduceWorkView', {
    extend: 'Rfx2.base.BaseView',
    xtype: 'produce-work-view',
...
//		        					gm.me().store.getProxy().setExtraParam('pj_type', 'D');
		    						...
		    						store.getProxy().setExtraParam('orderBy', 'unique_id');
			        		        store.getProxy().setExtraParam('ascDesc', 'ASC');
			        		        store.getProxy().setExtraParam('big_pcs_code', 'PRD');
		        				break;
		        				case 'ACH':
		        				case 'ATM':
		        				case 'BND':
...
//		        					gm.me().store.getProxy().setExtraParam('pj_type', 'D');
		        					...
		        					store.getProxy().setExtraParam('orderBy', 'unique_id');
			        		        store.getProxy().setExtraParam('ascDesc', 'ASC');
			        		        store.getProxy().setExtraParam('big_pcs_code', 'MAT');
		    					break;
		        			}
	        				break;	
	        				default:
