    ...
    initComponent: function () {
            ...
            handler: function () {
                ...
                var form = Ext.create('Ext.form.Panel', {
                    ...
                    items: [
                        {
                            xtype: 'fieldset',
                            title: '기준이 되는 출하 시작일자와 종료일자를 선택하십시오.',
                            items: [
                                {
                                    xtype: 'datefield',
                                    id: gu.id('start_date'),
                    ...
                    buttons: [
                        {
                            ...
                            handler: function () {
                                ...
                                var store = Ext.create('Rfx2.store.company.chmr.DeliveryStateBySrcahdStore', {});
                                ...
                                store.getProxy().setExtraParam("menuCode", 'SDL_ORDER_ITEM_XLSX');
        ...
        this.downloadSheetAction = Ext.create('Ext.Action', {
            ...
            handler: function () {
                ...
                var store = Ext.create('Rfx2.store.DeliveryStateSummaryStoreExcel', {});
                store.getProxy().setExtraParam("srch_type", 'excelPrint');
                store.getProxy().setExtraParam("srch_fields", 'major');
                store.getProxy().setExtraParam("srch_rows", 'all');
                store.getProxy().setExtraParam("is_excel_print", 'Y');
                store.getProxy().setExtraParam('sloast_uids', uids)
                store.getProxy().setExtraParam("menuCode", 'SDL_ORDER_EXL');
        ...
        this.poPrdDetailStore = Ext.create('Rfx2.store.DeliveryStateSummaryStore', {});
        ...
        this.gridContractCompany = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                ...
                { text: this.getMC('msg_order_grid_prd_name', '품명'), width: 100, style: 'text-align:center', dataIndex: 'item_name'/*, sortable: false*/ },
                { text: this.getMC('msg_order_grid_prd_name', '규격'), width: 90, style: 'text-align:center', dataIndex: 'description'/*, sortable: false*/ },
                {
                    text: this.getMC('msg_order_grid_prd_unitprice', '수량'),
                    width: 100,
                    style: 'text-align:center',
                    decimalPrecision: 5,
                ...
                { text: '비고', width: 150, style: 'text-align:center', dataIndex: 'reserved_varchar4' },
            ],
            title: this.getMC('mes_reg_prd_info_msg', '출하이력'),
            name: 'po',
            autoScroll: true,
            listeners: {
        ...
        this.setGridOnCallback(function (selections) {
            ...
            if (selections.length) {
                ...
                for (var i = 0; i < selections.length; i++) {
                    ...
                    if (i === 0) {
                        var rec = selections[i];
                        sloast_uids.push(rec.get('unique_id_long'));

                    } else {
                        var prev_rec = selections[i - 1];
                        var present_rec = selections[i];
                        var prev_pj_uid = prev_rec.get('pj_uid');
                        var present_pj_uid = present_rec.get('pj_uid');
                ...
                this.poPrdDetailStore.getProxy().setExtraParam('sloast_uids', sloast_uids);
        ...
        this.store.getProxy().setExtraParam('is_req_del', 'Y');
