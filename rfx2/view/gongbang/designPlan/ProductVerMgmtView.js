//자재 관리
Ext.define('Rfx2.view.gongbang.designPlan.ProductVerMgmtView', {
    ...
    initComponent: function () {
        ...
        this.addCallback('SET_ITEM_CODE_TYPE_A', function (obj) {

            var column = obj.name.split('|')[1];
            var value = '';

            switch(column) {
                case 'item_name':
                    var sp_code = Ext.getCmp('design-plan-' + gm.me().link + '-sp_code');
                    var store = sp_code.getStore();
                    sp_code.setValue(null);
                    store.getProxy().setExtraParam('parent_class_code', obj.lastSelectedRecords[0].get('class_code'));
                    store.load();
                    value = obj.value;
                    break;
                case 'sp_code':
                    value = obj.lastSelectedRecords[0].get('class_name');
                    break;
                default:
                    value = obj.value;
            }

            var itemCodeObj = gm.me().getInputJust('extendsrcahd|item_code');
            itemCodeObj.setValue( gm.me().combineItemCodeTypeA(column, value));
        });

        this.addCallback('CHECK_CODE', function (o) {
            var target = gm.me().getInputJust('extendsrcahd|item_code');
        ...
        this.addCallback('CHECK_CODENUMBER', function (o) {
            var target = gm.me().getInputJust('extendsrcahd|item_code');
            console_logs('====target', target);
            var code = target.getValue();
            //var uppercode = code.toUpperCase();
        ...
        if (this.flag1 === 'Y') {
            ...
            buttonToolbar.insert(3, this.productDeleteAction);
        } else {
            buttonToolbar.insert(4, this.changeUsingStatusAction);
            buttonToolbar.insert(5, this.fileattachAction);
            buttonToolbar.insert(3, this.productDeleteAction);
    ...
    combinedItemCodeTypeA: '---',
    combineItemCodeTypeA: function(column, value) {
        /**
         * 품명, 분류, 규격, 거래처명으로 조합해서 품번 생성
         * @type {string[]}
         */
        var co = gm.me().combinedItemCodeTypeA.split('-');
        switch(column) {
            case 'item_name':
                co[0] = value;
                break;
        ...
        gm.me().combinedItemCodeTypeA = co[0] + '-' + co[1] + '-' + co[2] + '-' + co[3];
        return gm.me().combinedItemCodeTypeA;
    }
});
