    ...
    if (vUse_workspace == true) {
            ...
            items: [{
                ...
                handler: function () {
                    ...
                    mm.getLastId(function (lastUid) {
                        ...
                        //console.log('group_code lastUid', lastUid);
                        var url = CONTEXT_PATH + '/uploader.do?method=multi&group_code=' + lastUid;
                        ...
                        me.uploadDialog = Ext.create('Ext.ux.upload.Dialog', {
                            dialogTitle: '파일 첨부',
                            panel: uploadPanel
                        });

                        me.lastUid = lastUid;
                        me.mon(me.uploadDialog, 'uploadcomplete', function (uploadPanel, manager, items, errorCount) {

                            //console.log('this.mon uploadcomplete uploadPanel', uploadPanel);
                            //console.log('this.mon uploadcomplete manager', manager);
                            //console.log('this.mon uploadcomplete items', items);
                            //console.log('this.mon uploadcomplete errorCount', errorCount);
                            //console.log('this.mon uploadcomplete lastUid', me.lastUid);

                            if (items != null && items.length > 0) {

                                var attachedFileStore = Ext.create('Mplm.store.AttachedFileStore', {group_code: me.lastUid});
                                attachedFileStore.getProxy().setExtraParam('highlight', 'NONE');
                                attachedFileStore.load(function (records) {
                                    //console.log('디폴트 데이터', records);

                                    var files = null;

                                    for (var i = 0; i < records.length; i++) {
                                        var rec = records[i];
                                        var s = rec.get('object_name');

                                        var file_size = Ext.util.Format.fileSize(rec.get('file_size'));
                                        if (files == null) {
                                            files = s + ' (' + file_size + ')';
                                        } else {
                                            files = files + '<br>' + s + ' (' + file_size + ')';
                                        }
                                    }
                                    ...
                                    mm.makeMessage(me.lastUid, from_name, to_name, receiver_uid, owner_uid, parent_uid, '', msg_type, space, link, top, left, width, height, index, files);


                                });


                            } //endof items.length>0
        ...
        Ext.create('Ext.button.Segmented', {
            ...
            items: [{
                ...
                menu: [
                    {id: 'menuItem_cur_only', text: '현재 메모', handler: setViewNote},
                    {id: 'menuItem_view_all', text: '모두 보기', handler: setViewNote},
                    {id: 'menuItem_hide_all', text: '모두 끄기', handler: setViewNote}
                ]
            }]
        });
