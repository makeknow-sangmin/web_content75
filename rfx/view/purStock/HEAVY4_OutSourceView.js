...
//외주현황
Ext.define('Rfx.view.purStock.HEAVY4_OutSourceView', {
    ...
    initComponent: function(){
		...
		//ROW 추가
        this.addRowAction = Ext.create('Ext.Action', {
        	iconCls: 'af-plus',
        	text: '추가',
        	tooltip: '추가',
        	disabled: false,
        	handler: function() {		 	
			 	gMain.selPanel.addRowHandler();
        	}
		});
		...
		this.printPDFAction = Ext.create('Ext.Action',{
            ...
            handler: function(widget, event) {
                ...
                Ext.Ajax.request({
                    url: CONTEXT_PATH + '/pdf.do?method=printOs',
                    params:{
		...
		buttonToolbar.insert(3, this.addRowAction);
	...
	addRowHandler: function() {
		...
		Ext.Ajax.request({
			url: CONTEXT_PATH + '/purchase/request.do?method=createHisToryRow',
			params:{
				
			},
			success : function(result, request) {
				gm.me().store.load();
				gm.me().showToast('결과', '추가 되었습니다.');
			},
			failure: extjsUtil.failureMessage
		});
	},
