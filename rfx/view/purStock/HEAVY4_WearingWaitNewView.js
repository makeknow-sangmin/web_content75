Ext.define('Rfx.view.purStock.HEAVY4_WearingWaitNewView', {
    extend: 'Rfx.base.BaseView',
    xtype: 'wearing-wait-new-view',
    ...
    initComponent: function () {
            ...
            switch (dataIndex) {
                ...
                case 'req_info':
                    ...
                    case 'delivery_terms':
                        ...
                        columnObj["editor"] = {
                            xtype: 'combo',
                            store: pos_store = Ext.create('Rfx.store.DetailAddZeroStockStore', {}),
                            ...
                            listeners: {
                                ...
                                //     var nstock_uid = record.get('nstock_uid');
                                //     this.store.getProxy().setExtraParam('nstock_uid', nstock_uid);
                                //     this.store.load();
                                //     //this.pos_store.load();
                                //     console_logs("테스트22",record.get('nstock_uid'));
                                //     alert(record.get('nstock_uid'));
                                // },

                                expand: function (field) {
                                    
                                    var nstock_uid = gm.me().grid.getSelectionModel().getSelection()[0].get('nstock_uid');
                                    console_logs("테스트222",nstock_uid)
                                    if(nstock_uid > 0){
                                    this.store.getProxy().setExtraParam('nstock_uid', nstock_uid);
                                    }else{
                                        this.store.getProxy().setExtraParam('nstock_uid', -1);
                                    }
                                    this.store.load();
                                }
        ...
        this.createGoAction = Ext.create('Ext.Action', {
            ...
            handler: function () {
                ...
                for (var i = 0; i < selections.length; i++) {
                   if(selections[i].get('delivery_terms').length  > 0){
                       gMain.selPanel.treatPaperGoRoll();
                       return;
                   }else{
                        Ext.MessageBox.alert('알림', '적재위치를 지정해주세요.');
                    return;
                   }
                }
            ...
            buttons: [{
                ...
                handler: function (btn) {
                    ...
                    for (var i = 0; i < selections.length; i++) {
                        ...
                        stock_poses.push(selections[i].get('delivery_terms'))
                    }
