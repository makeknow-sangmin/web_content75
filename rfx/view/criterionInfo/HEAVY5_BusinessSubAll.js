 ...
 */
Ext.define('Rfx.view.criterionInfo.HEAVY5_BusinessSubAll', {
    extend: 'Rfx.base.BaseView',
    xtype: 'business-sub-all-view',
				...
				handler: function(btn){
						...
						if(selections == null || selections.length < 1) {
							...
							for(var i=0; i<selections.length; i++) {
								...
								if(rec.get('state') == 'A' || rec.get('state') == 'I' || rec.get('state') == 'D' || rec.get('state') == 'R') {
									Ext.MessageBox.alert(error_msg_prompt, '이미 결재완료 혹은 중간 결재중인 일지가 있습니다.');
									return;
								}
