...
Ext.define('Rfx.view.designPlan.DesignBomTreeView_DABP', {
...
//	            	break;
        ...
        Ext.apply(me, {
            layout: 'border',
            items: [me.createWest(), me.createCenter()/*, this.createEast()*/]
        });
...
//                        },
            ...
            columns: [{
					...
					// 		listConfig : {
					// 			getInnerTpl : function() {
					// 				return '<div data-qtip="{supplier_code}"><small>[{supplier_code}] {supplier_name}</small></div>';
                    //             },
                    //             listeners: {
                    //             'selectionchange': function(view, records) {
                    //                 var maker_code = records[0]['data']['maker_code'];
                    //                 var maker_name = records[0]['data']['maker_name'];
                    //                 var child = gm.me().assyChildUid;
                    //                 var ac_uid = gm.me().assyAcUid;
                    //                 var parent = gm.me().assyParent;
                                    
                    //                 Ext.Ajax.request({
                    //                     url : CONTEXT_PATH + '/index/generalData.do?method=updateMakerAssy',
                    //                     params: {
...
//                    }
        ...
        for(var i=0; i<this.columns.length; i++) {
            ...
            case 'item_name':
                stockColumn.push(this.columns[i]);
                break;
            }
        };
        ...
        this.gridStockWonji.getSelectionModel().on({
            selectionchange: function(sm, selections) {
	            if (selections.length) {
                    ...
                    var srcahd_uid = selections[0]['data']['uid_srcahd'];
                    var assySelect = gm.me().checkStockAssemblySelection(selections, child_uid, ac_uid, order_com_unique, srcahd_uid);
                    var node = JSON.parse(assySelect);
                        ...
                        Ext.Ajax.request({
                            ...
                            success: function(result, request) {
                                var result = result.responseText;
                                // gm.me().store.getProxy().setExtraParam('parent', child_uid);
                                gm.me().store.getProxy().setExtraParam('parent_uid', parent_uid);
                                // gm.me().store.getProxy().setExtraParam('child', srcahd_uid);
...
//                });
    ...
    selectTreeGrid: function(rec) {
        ...
        if (rec == null) {
            ...
            //console_logs('gm.me().sp_code', gm.me().sp_code);
            //console_logs('ogm.me().unique_uid', gm.me().unique_uid);

            //gm.me().onShowSpcodeClick(gm.me().sp_code, gm.me().unique_uid);
    ...
    updateDesinComment: function(rec) {
        ...
        planConfirmActionK : Ext.create('Ext.Action', {
            ...
            disabled: true,
            handler: function(widget, event) {
                gm.me().planConfirm(gm.me().gridK);
            }
        }),
        planConfirm : function(grid) {
...
//                    console_logs('selected reserved1', reserved1);
                    ...
                    switch(pcr_div) {
                        ...
                        break;
                        default:
                            if(reserved1!=null && reserved1=='true') {//재단있음.
    ...
    getFields : function(sp_code) {
        ...
        // //Editor에 roweding 을 위에 itemId를 부여.
        // for(var i=0; i < fields.length; i++) {
        //     var rec = fields[i];
        //     var dataIndex = rec['dataIndex'];
        //     var editor = rec['editor'];
        //     var itemId = dataIndex + assy_uid;
        //     if(editor!=null) {
    ...
    copyWonjiField: function() {
        if(this.fields_wonji.length == 0) {
            for(var i=0; i<this.columns.length; i++ ) {
                var rec = this.columns[i]; // gu.copyObj(this.columns[i]); //Object 복사해야 함.
                var dataIndex = rec['dataIndex'];
                switch(dataIndex) {
                    case 'spec_model_desc':
                    case 'item_name':
            ...
            //         modelClass: 'Rfx.model.Papertype',
            //         pageSize: 500,
            //         /*pageSize*/
            //         sorters: [{
            ...
            //     });
            // console_logs('=>aaa', this.store);

            this.fields_wonji.splice(2,0,{
                align : "left",
                dataIndex  : "item_name",
                dataType : "string",
                // editor :    {},
 ...
 //                   case 'spec_model_desc':
    ...
    editBaseGrid: function(editor, e, eOpts) {
        ...
        // var reserved1 = gm.me().assyReserved1;
        // if(reserved1 != null && (reserved1 == 'true' || 'reserved1'==  true || reserved1 == true ) ) {
        //     var reserved4 = gm.me().assyReserved4
        //     var bm_quan = gm.me().assyBm_quan 
        //     var unique_id = gm.me().assymapUidbom

        //     if(reserved4 == null || reserved4.length < 0 || reserved4 == '') {
        //         Ext.Ajax.request({
        //             url: CONTEXT_PATH + '/design/bom.do?method=updateProductQuan',				
        //             params:{
        //                 bm_quan : bm_quan,
        //                 unique_id:unique_id
        //             },
    ...
    //     console_logs('plugins', plugins);
        
    //     var rowEditingPlugin = plugins[0];
    //     //
