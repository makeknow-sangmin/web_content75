...
Ext.define('Rfx.view.designPlan.DesignBomTreeView', {
...
//                case 'reserved_varchar1':
        ...
        var buttonToolbar1 = Ext.create('widget.toolbar', {
            ...
            items: [
                ...
                '-',
                vCompanyReserved4 == 'SKNH01KR' ? this.getPartListAction : '',
                '-',
                this.getPartAllListAction,
                //this.req_quan_ten,
...
//                },
        ...
        this.setGridOnCallback(function(selections) {
            ...
            if (selections.length) {
                ...
                gUtil.enable(gm.me().req_quan_ten);

                //스카나 누적된 요청수량
                gm.me().AssyPrQuan = rec.get('pr_quan');
                console_logs("gridOnCallBack pr quan 출력 >>>>>" ,   gm.me().AssyPrQuan);
                ...
                gUtil.disable(gm.me().req_quan_ten);
        ...
        Ext.Ajax.request({
            ...
            success: function(result, request) {
                ...
                gm.me().conditionStore.getProxy().setExtraParam('query',pj_name);
                gm.me().conditionStore.load(function(records) {
                    if (records != null && records.length > 0) {
                        // console_logs('gm.me().cloudprojectStore.load records', records);
                        for (var i = 0; i < records.length; i++) {
                            var rec = records[i];
                            // console_logs('record ac_uid', rec.get('unique_id'));
                            // if (rec.get('unique_id') == ac_uid) {
                                var combo = gu.getCmp('pj_name');
                                //console_logs('combo', combo);
                                if(combo!=undefined) { 
                                    combo.select(rec);
                                    gm.me().selectHosunCombo(rec);
                                }
                            // }
                        }
           ...
           Ext.Ajax.request({
            ...
            success: function(result, request) {
                ...
                var arr = id.split(';');
                var wa_name = arr[0];
                var combst_uid = arr[1];
                gm.me().buyerStore.getProxy().setExtraParam('project_only', 'T');
                gm.me().buyerStore.getProxy().setExtraParam('query',wa_name);

                // gm.me().cloudprojectStore.getProxy().setExtraParam('order_com_unique', combst_uid);
                gm.me().conditionStore.getProxy().setExtraParam('parentCode', gm.me().link);
                gm.me().conditionStore.getProxy().setExtraParam('tableName', 'project');
                gm.me().conditionStore.getProxy().setExtraParam('fieldName', 'pj_name');
                gm.me().conditionStore.getProxy().setExtraParam('limit', 1000);
                gm.me().conditionStore.getProxy().setExtraParam('query', '');
                gm.me().conditionStore.getProxy().setExtraParam('sqlName', 'partLineItemDetail');/*cartmap 에서 Assymap으로 변경*/
                gm.me().conditionStore.getProxy().setExtraParam('wherelist',
                    'order_com_unique:' + combst_uid
                );
               
                gm.me().buyerStore.load(function(records) {
                    if (records != null && records.length > 0) {
                        // console_logs('gm.me().cloudprojectStore.load records', records);
                        for (var i = 0; i < records.length; i++) {
                            var rec = records[i];
                            // console_logs('record ac_uid', rec.get('unique_id'));
                            // if (rec.get('unique_id') == ac_uid) {
                                var combo = gu.getCmp('wa_name');
                                //console_logs('combo', combo);
                                if(combo!=undefined) { 
                                    combo.select(rec);
                                    gm.me().selectBuyerCombo(rec);
                                }
                            // }
                        }
        ...
        for (var i = 0; i < this.columns.length; i++) {
            ...
            switch (this.columns[i]['dataIndex']) {
                ...
                if(vCompanyReserved4 == 'SKNH01KR') {
                    console_logs( "pr quans dataIndex 출력  >>>>" , this.columns[i]['dataIndex']  );
                }
...
//
        ...
        if(vCompanyReserved4 == 'SKNH01KR') {
            this.myCartStore.getProxy().setExtraParam('type', 'BOM');
        }
            ...
            viewConfig: {
                ...
                listeners: {
                    'afterrender': function(gridMycart) {
                        ...
                        console_logs("gridMycart = ", gridMycart);
            ...
            if(records!=null) {
                for(var i=0; i<records.length; i++) {
                    ...
                    if(vCompanyReserved4 == 'SKNH01KR'){
                        ...
                        new_pr_quan = o.get('pr_quan');

                    }else{
                        new_pr_quan = o.get('new_pr_quan');
                    }
      ...
      //Define Remove Action
        ...
        if(this.ftpSetting) {
            ...
            this.bomTab = Ext.widget('tabpanel', {
                ...
                items: [this.grid, vCompanyReserved4 == 'SKNH01KR' ? null : this.fileGrid,
                 vCompanyReserved4 == 'SKNH01KR' ? this.gridValveList : this.inpuArea,
                 vCompanyReserved4 == 'SKNH01KR' ? this.partSumSearch : this.inpuArea
                ]
            });
        }
...
//        });
        ...
        this.cloudProjectTreeStore = Ext.create('Mplm.store.cloudProjectTreeStore', {pjuid : -1});
        ...
        var selValveNo2 =   Ext.create("Ext.selection.CheckboxModel", {} );
        ...
        this.gridValve = Ext.create('Ext.grid.Panel', {
            ...
            tabConfig: {
                listeners: {
                    click: function() {
                        //var changeValveGrid = Ext.getCmp('formPanelSendmail').getForm(); 

                        //Ext.ComponentQuery.query('tabpanel')[0].setActiveTab(1);
                        //var changeValveGrid = Ext.ComponentQuery.query("tabpanel")[4].setActiveTab(1);
                        //var changeValveGrid = Ext.ComponentQuery.query("tabpanel");
                        //console_logs("gridValve  ===>   ", changeValveGrid);
                        //Ext.ComponentQuery.query("tabpanel")[4].setActiveTab(1);
                        //this.bomTab.setActiveItem(this.fileGrid);
                        //alert(changeValveGrid);
                    }
                }
            },
        ...
        //valve No list button evnet
        this.uploadValveListAction = Ext.create('Ext.Action', {
            iconCls: 'af-edit',
             text: 'Upload',
             tooltip: 'Valve List 업로드',
             disabled: false,
             handler: function(){
                var parent = gm.me().selected_tree_record.get('child');
                var parent_uid = gm.me().selected_tree_record.get('unique_uid');
                var pj_code = gm.me().selected_tree_record.get('pj_code');
                var coord_key2 = gm.me().selected_tree_record.get('coord_key2');
                var class_code = gm.me().selected_tree_record.get('class_code');

                var uploadForm = Ext.create('Ext.form.Panel',{
                    id: gu.id('ValveExcelUpload'),
                    width : 400,
                    height : 30,
                ...
                var win = Ext.create('Ext.Window', {
                    ...
                    buttons: [{
                        text: CMD_OK,
                        handler: function() {
                            var frm = gu.getCmp('ValveExcelUpload').getForm();
                            //var frm = btn.up("form").getForm();

                            if(frm.isValid()){
                                frm.submit({
                                    url : CONTEXT_PATH + '/uploader.do?method=uploadValveListBySingle',
                                    params : {
                                    ...
                                    success : function(fp, res){
                                            ...
                                            this.valveNoStore.load(function(records){
        ...
        //         // var url =  CONTEXT_PATH + '/uploader.do?method=uploadValveListBySingle&ac_uid=' + 
        //         // gm.me().selectedPjUid + '&parent='+parent+'parent_uid='+parent_uid +
        //         // 'pj_code='+ pj_code;

        //         var url = CONTEXT_PATH + '/uploader.do?method=uploadValveListBySingle'
        ...
        //         this.mon(uploadDialog, 'uploadcomplete', function(uploadPanel, manager, items, errorCount) {
        //             console_logs("uploadDialog~~~~~~~~~~~");
        //             gm.me().uploadValveComplete(items);
        //             //if (!errorCount) {
        //                 uploadDialog.close();
        //             //}
        //         }, this);
        ...
        this.addValveListAction = Ext.create('Ext.Action', {
            iconCls: 'af-plus-circle',
            text: '추가',
            tooltip: 'Valve List 등록하기',
            disabled: false,
            handler: function(widget, event) {
            ...
            var win = Ext.create('ModalWindow', {
                ...
                buttons: [{
                    text: CMD_OK,
                    handler: function() {
                        var form = gu.getCmp('addformPanel_valve').getForm();
                        if (form.isValid()) {
                            var val = form.getValues(false);
                            console_logs("valve add   >>  ", val);
                            console_logs("unique_uid   >>  ", unique_uid);
                            console_logs("bm_quan   >>  ", bm_quan);
                            console_logs("ac_uid   >>  ", ac_uid);

                             Ext.Ajax.request({
                                 url: CONTEXT_PATH + '/design/bom.do?method=addValvePart',
                                 params: {
        ...
        this.gridValveList = Ext.create('Ext.grid.Panel', {
            title: 'Valve NO. List',
            store: this.valveNoStore,
            id: gu.id('gridValveList'),
            // /COOKIE//stateful: true,
            collapsible: true,
            multiSelect: false,
            selModel: selValveNo2,
            //stateId: 'gridValveList' + /* (G) */ vCUR_MENU_CODE,

            // tabConfig: {
            //     listeners: {
            //         click: function() {
            //             //this.uploadValveListAction.enable();
            //         }
            //     }
            // },
            ...
            dockedItems: [{
                ...
                cls: 'my-x-toolbar-default2',
                items: [
                    this.uploadValveListAction,
                    '-',
                    this.addValveListAction,
                    this.removeValveAction, '->',
                    {
                        xtype: 'component',
                        id: gu.id('valve_quan'),
        ...
        this.gridValveList.getSelectionModel().on({
            selectionchange: function(sm, selections) {
                gm.me().ongridValveListSelection(selections);
            }
        });
                ...
                //스카나 part 합계 수량 탭
                this.partSumSearch = Ext.create('Ext.grid.Panel', {

                    store: this.partSumSearchStore,

                    handler: function(z, x, c, f) {
                        var parent_uid=gm.me().selectedAssyUid;
                        //console_logs('parent_uid >>>' , c) ;
                    },
                    ...
                    tabConfig: {
                        ...
                        //         }
                        //     },
                        //     select: function(combo, record, assy_selections_sk) {
                        //     }
                        // }// 리스너 끝
...

                            {
                                ...
                                listeners : {
                                    change : function(fieldObj, e) {
                                        ...
                                        var assy_selections = gm.me().vSELECTED_SELECTION_SCANA
                                        ...
                                        for(var i=0; i<assy_selections.length; i++) {
                                            ...
                                            //console_logs(" >>>>> 88888888888888888888 assy_selections >>>>" , assy_selections);
                            
                                            console_logs(" >>>>> 888888888888888 i의 get pj code 출력 >>>>" , assy_selections[i].get('pj_code' ) );

                                            searchPjcode = assy_selections[i].get('pj_code' );
                            
                                            pjcodeScana.push(assy_selections[i].get('pj_code' ));
                                            //console_logs(" >>>>> 88888888888888 pjcodeScana 출력 >>>>" , pjcodeScana );
                                           
                                        }
                                        console_logs('체인지34343 combo 출력', e);
                                       
                                          // .value로 해야 값이 감
                                        gm.me().partSumSearchStore.getProxy().setExtraParam('query', e);
                                        gm.me().partSumSearchStore.getProxy().setExtraParam('searchPjcode', searchPjcode);
                                        gm.me().partSumSearchStore.load();
                                        //gm.me().partSumSearchStore.getProxy().setExtraParam('query', record.value);
                                ...
                                listeners : {
                                    change : function(fieldObj, e) {
                                        ...
                                        console_logs('체인지34343 combo 출력', e);
                                          ...
                                          // .value로 해야 값이 감
                                        gm.me().partSumSearchStore.getProxy().setExtraParam('query2', e);
                                        gm.me().partSumSearchStore.getProxy().setExtraParam('searchPjcode', searchPjcode);
                                        gm.me().partSumSearchStore.load();
                                        //gm.me().partSumSearchStore.getProxy().setExtraParam('query', record.value);
                    ...
                    columns: [
                            ...
                            //1.프로젝트명, 2. 엑튜에이터, 3.하위 4 

                        {text     : '프로젝트', width     : 200, sortable : true, dataIndex: 'pj_name'},
                        {text     : '품명', width     : 150, sortable : true, dataIndex: 'item_name'},
                        {text: "품목코드", width: 100, dataIndex: 'item_code', sortable: true},
                        {text: "서브어셈블리", width: 100, dataIndex: 'parent_item_name', sortable: true},
                        {text: "어셈블리", width: 100, dataIndex: 'high_paent_item_name', sortable: true},
                        {text: "규격", width: 100, dataIndex: 'specification', sortable: true},
                        {text: "수량", width: 100, dataIndex: 'bm_quan', sortable: true},
                ...
                this.partSumSearch.getSelectionModel().on({

                    selectionchange: function(sm, selections) {
                        console_log('partSumSearch getSelectionModel() on 출력>> '  );

                        gm.me().onpartSumSearchSelection(selections);
                    }
                });
        ...
        this.assyGrid = Ext.create('Ext.tree.Panel', {
            ...
            dockedItems: [{
                ...
                items: [
                    ...
                    //vCompanyReserved4 == 'SKNH01KR' ? this.pasteAssyAction : '',
                    ...
                    '-',
                    vCompanyReserved4 == 'SKNH01KR' ? this.copyBomtreeModal : '',
                    //vCompanyReserved4 == 'SKNH01KR' ? this.addHosunAction : '',
                    vCompanyReserved4 == 'SKNH01KR' ? this.editDesignAction : '',
...
//                        },
                ...
                cls: 'my-x-toolbar-default2',
                items: [
                    vCompanyReserved4 == 'KWLM01KR' ? this.nodeExpandAction : '',
                    vCompanyReserved4 == 'KWLM01KR' ? this.nodeCollapseAction : '',
                ]
            },
                {
                    dock: 'top',
                    ...
                    items: [/*{
                    ...
                    },
                        {
                            ...
                            listeners: {
                                select: function(combo, record) {
                                    console_logs('combo', combo);
                                   console_logs('콤보 record출력', record);
                                    gm.me().selectHosunCombo(record);
                                    order_com_unique = combo.value;
                                ...
                                change: function(combo, record) {
                                    //console_log('combo', combo);
                                    //console_log('체인지 record출력', record);
...
//                                    	console_logs('o', o);
                ...
                width: (vCompanyReserved4 ==  'SKNH01KR')? 0:0,
                hidden: (vCompanyReserved4 ==  'SKNH01KR')? true: true,
        ...
        this.cloudProjectTreeStore.on('load',function (store, records, successful, eOpts ){
            ...
            if(records!=null) {
               ...
               // 저장한 작업위치 적용
               gm.me().assyGrid.getSelectionModel().select(assyGridSelectedIndex);
            }
            //Block of codes
        ...
        this.assyGrid.getSelectionModel().on({
            select: function(a,b) {
                ...
                // }
                // 작업위치 저장
                assyGridSelectedIndex = gm.me().assyGrid.getStore().indexOf(assySelections[0]);
            },
    ...
    cloudhosunStore: Ext.create('Mplm.store.cloudProjectStore', { hasNull:true}),
    //추가작성
    cloudprojectStore: Ext.create('Mplm.store.cloudProjectStore', { hasNull:true}),
    cloudprojectCopyStore: Ext.create('Mplm.store.cloudProjectStorePrev', { hasNull:true}),
    ...
    selectAssy: function(record) {
        ...
        console_logs('Assemy 탭 셀렉트 어시 이벤트   >>>>>>>>>  ', record);
        ...
        if(vCompanyReserved4 == 'SKNH01KR') {
            ...
            gm.me().vSELECTED_SELECTION_SCANA=gm.me().assyGrid.getSelectionModel().getSelection();
            // console_logs(" >>>>> 어시 셀렉션 pj_code 프로젝트 코드 어레이 출력 >>>>" , assy_selections.get('pj_code' ));

            var pjcodeScana = [];
            for(var i=0; i<assy_selections.length; i++) {
                this.assembly_uids.push(assy_selections[i].get('unique_uid'));
                pjcodeScana = assy_selections[i].get('pj_code' );
        ...
        if (available_depth) {
            ...
            this.addValveListAction.disable();
            this.removeAssyAction.enable();
        } else {
            this.removeAssyAction.enable();
            this.addValveListAction.enable();
        }
        ...
        if(this.depth == 2){
            ...
            this.uploadValveListAction.disable();
        }
        ...
        switch(vCompanyReserved4) {
        case 'SKNH01KR':
        	this.valveNoStore.getProxy().setExtraParam('parent_uid', record.get('id'));
            this.valveNoStore.getProxy().setExtraParam('class_code',gm.me().pClassCode);
            this.valveNoStore.load(function(records) {
                console_logs("Valve   load", records.length);
                var o = gu.getCmp('valve_quan');
                if (o != null) {
                    o.update( '총수량 : ' + records.length);
                }

                gm.me().valveQuan = records.length;
            });
            break;
        }
...
//                        },
    ...
    autoCreateBomVersion: function(){
        Ext.Ajax.request({
            url: CONTEXT_PATH + '/design/bom.do?method=autoCreateBomVersion',
            params: {
                ac_uid: gm.me().selectedPjUid,
                versionCheck: gm.me().versionCheck
            },
            success: function(result, request) {
                alert("새로운 BOM 버전이 생성되었습니다.");
            },
            failure: extjsUtil.failureMessage
        });
    },
...
//                var result = result.responseText;
    ...
    autoHosunCopy: function(){
        ...
        inputItem.push({
            xtype: 'component',
            style: 'margin-bottom:10px;margin-right:5px;width:100%;text-align:center',
            readOnly: true,
            fieldStyle: 'background-color: #EAEAEA; background-image: none;',
            html: '---------------------------------------'
        });
            ...
            items:[
                {
                    ...
                    queryMode: 'remote',
                    //store: this.cloudprojectStore,
                    store: this.cloudhosunStore,
                    displayField: 'reserved_varchar3',
                    valueField: 'reserved_varchar3',
                    minChars: 1,
                    anchor: '-5',
                     listeners : {
                         change : function(fieldObj, e) {                     
                            console_logs("------", e); 
                            gm.me().cloudhosunStore.getProxy().setExtraParam('reserved_varchar3', e);
                         },
                         select: function(combo, record){
                            
                            if(record != null){
                                gu.getCmp('autoHosunCopy_pjname').setValue(record.get('pj_name'));
                                gu.getCmp('autoHosunCopy_waname').setValue(record.get('wa_name'));

                                gm.me().FromAssyUid = record.get('assymap_uid');
                                gm.me().FromAcUid = record.get('id');
                                

                                //project copy  -> session save
            ...
            buttons: [{
                ...
                handler: function(){
                    ...
                    if (gm.me().selectedAssyUid == null || gm.me().selectedAssyUid == '' || gm.me().selectedPjUid == null || gm.me().selectedPjUid == '') {
                        ...
                        Ext.Ajax.request({
                            url: CONTEXT_PATH + '/design/bom.do?method=autoCopyPasteProject',
                            params: {
                                to_pj_uid: gm.me().selectedPjUid,
                                to_assy_uid: gm.me().selectedAssyUid,
                                to_parent:gm.me().selectedChild,
                                FromAcUid: gm.me().FromAcUid,
                                FromAssyUid: gm.me().FromAssyUid
                            },
                            success: function(result, request) {
                                var result = result.responseText;
    ...
    addHosunAction : Ext.create('Ext.Action', {
        iconCls: 'font-awesome_4-7-0_arrow-down_14_0_5395c4_none',
        text: '',
        tooltip: '호선복사',
        handler: function() {
...
//    }),
    ...
    deleteValveConfirm: function(result) {
        if (result == 'yes') {
            var selections = gm.me().gridValveList.getSelectionModel().getSelection();
            if (selections == null || selections.length == 0) {
...
//        console_logs("srchSingleHandler b", b);
    ...
    //추가작성
    srchCopyTreeHandler: function(my_treepanel, cloudProjectTreeStore, widName, parmName, b) {
        ...
        //        console_logs("srchSingleHandler b", b);
                ...
                this.copyGrid.setLoading(true);

                this.resetParam(this.cloudProjectTreeCopyStore, this.searchField);
                var val = gu.getCmp(widName).getValue();
                //console_log('val' + val);
                ...
                this.beforeTreeStore.getProxy().setExtraParam(parmName, val);
                this.beforeTreeStore.load({
                    callback: function(records, operation, success) {
                        console_logs('======> records', records);
                        gm.me().copyGrid.setLoading(false);
                        gm.me().copyGrid.getSelectionModel().select(0);
                        gm.me().copyGrid.expandAll();
                    }
                });
    ...
    selectHosunCombo: function(record) {
        // var pjuid = record.get('unique_id');
        var pj_name = record.get('value');
        // var pj_code = record.get('pj_code');
    ...
    selectBuyerCombo: function(record) {
  
        var wa_name = record.get('wa_name');
        var combst_uid =record.get('unique_id_long');
        console_logs('고객사 테스트',record);
        // Default Set
        Ext.Ajax.request({
            url: CONTEXT_PATH + '/admin/menu.do?method=defaultSet',
            params: {
                paramName: 'CommonBuyer',
                paramValue: wa_name + ';' + combst_uid
            },
    ...
    //추가작성
    selectCopyProjectCombo: function(record) {
        this.srchCopyTreeHandler(this.copyGrid, this.beforeTreeStore, 'projectCopycombo', 'pjuid', true);
    ...
    addNewAction: function(val) {
        ...
        Ext.Ajax.request({
            url: CONTEXT_PATH + '/design/bom.do?method=createNew'+'&versionCheck='+gm.me().versionCheck,
            params: val,
            success: function(result, request) {
                gm.me().selectTreeGrid(null);
...
//    		break;
    ...
    doRequestAction: function(isGoodsin) {
        ...
        var assyGrid = gm.me().assyGrid.getSelectionModel().getSelection()[0];
        var selectPjUid = assyGrid.get('ac_uid');
        var req_date = null;
        var req_time = null;
        ...
        if(selections != null && selections.length > 0) {
            try {
                req_date = selections[0].get('req_date');
                for(var i=0; i<selections.length; i++) {
                    var new_req_date = selections[i].get('req_date');
                    if(new_req_date.length > 0) {
                        new_req_date = Ext.Date.format(new Date(new_req_date), 'Y-m-d');
                        req_date_list.push(new_req_date);
                    }
                }
                if(req_date_list.length>1) {
                    console_logs('>>>>>req_date_list 1', req_date_list);
                    req_date_list = req_date_list.sort(gm.me().date_ascending);
                    console_logs('>>>>>req_date_list 2', req_date_list);
                    req_date = req_date_list[0];
                } else {
                    req_date = req_date_list[0];
                }
            } catch(e) {
...
//                }
                ...
                //new_pr_quans.push(rec.get('reserved_double1'));
                 new_pr_quans.push(rec.get('pr_quan'));
            ...
            mycart_uids[i] = rec.get('mycart_uid');
        ...
        var formItems = [{
                    xtype: 'fieldset',
                    title: '구매 요청(글자 수가 너무 길면 구매요청이 안될 수 있습니다.)',
                    collapsible: false,
                    width: '100%',
                    style: 'padding:10px',
                    ...
                    items: [
                        ...
                        {
                            fieldLabel: '선택된 구매요청 제품 수량 : ',
                            xtype: 'displayfield',
                            anchor: '100%',
                            name: 'count',
                            value : gm.me().gridMycart.getSelectionModel().getSelection().length
                ...
                if(this.useRouting==true) {
                    ...
                    var updown =
                    {
                        ...
                        align: 'center',
                        items: [{
                            icon   : 'http://makeknow.com/web_content75' +  '/resources/follower/demo/resources/images/up.png',
                            tooltip: 'Up',
                        ...
                        {
                            icon   : 'http://makeknow.com/web_content75' +  '/resources/follower/demo/resources/images/down.png',
                            tooltip: 'Down',
                            handler: function(agridV, rowIndex, colIndex) {
...
//        });
    ...
    ongridValveListSelection: function(selections) {
        console_logs('--> ongridValveSelection selections', selections);
        ...
        //수량합계 탭
        onpartSumSearchSelection: function(selections) {
            console_logs('--> onpartSumSearchSelection 출력 >>>> ', selections);
    ...
    req_quan_ten : Ext.create('Ext.Action', {
        itemId: 'reqDateButton',
        iconCls:'font-awesome_4-7-0_refresh_14_0_5395c4_none',
        text: '수량x10',
        disabled: true,
        handler: function(widget, event) {
            gm.me().doReqTen(false);
        ...
        handler: function(widget, event) {
            ...
            Ext.Ajax.request({
                ...
                success: function(result, request) {
                    ...
                    if(vCompanyReserved4 == 'KWLM01KR' || vCompanyReserved4 == 'SKNH01KR')  {
                        bHeight = 750;
                        bWidth = 800;
                    }
                    ...
                    var targetPartStore = Ext.define('targetPartStore', {
                    ...
                    var searchPartGridsknh = null;
                    if(vCompanyReserved4 == 'SKNH01KR') {


                        searchPartGridsknh = Ext.create('Ext.form.Panel', {
                            xtype: 'form-grid',
                            //controller: contlroll,
                            title: '자재검색',
                            width: 880,
                            bodyPadding: 5,
                            layout: {
                            ...
                            items:[
                                ...
                                layout: 'fit',
                                columns: [
                                    {text: "품목코드", width: '20%', dataIndex: 'item_code', sortable: true},
                                    {text: "품명", width: '30%', flex: 1, dataIndex: 'item_name', sortable: true},
                                    {text: "수량", xtype: 'numbercolumn', width: '10%', dataIndex: 'bm_quan'},
                                    {text: "규격", width: '20%', dataIndex: 'specification', sortable: true},
                                    {text: "재질", width: '20%', dataIndex: 'model_no', sortable: true}                             
                                ],
                                
                              
                                
                                //plugin 추가, cellEditing1 을 사용.
                                plugins: [gm.me().cellEditing1],
                                ...
                                tbar: [{
                                    ...
                                    handler: function(rowIndex){
                                        ...
                                        var rec = this.up("grid").getSelectionModel().getSelection();
                                        
                                        var targetGrid = gu.getCmp('targetPartGrid');
                                        var targetRec = targetGrid.getStore().data.items;
                                        var targetRecCnt = targetRec.length;
                                        
                                        for(var i=0; i<targetRecCnt; i++){
                                            var targetId = targetRec[i].get('id');
                                            var sourceId = rec[0].get(id);
                                        ...
                                        if(checkId){
                                            var recName = rec.get[0].get('item_name');
                                            Ext.Msg.alert('안내', recName +' 는 이미 등록된 자재입니다.', function() {});
                                        }else{
                                            targetGrid.getStore().insert(targetRecCnt,rec);
                                            targetUids.push(rec[0].get('id'));
                                            gm.me().targetUid = targetUids;
                                        
    
                                            //추가
                                            bmquanUids.push(rec[0].get('bm_quan'));
                                            gm.me().bm_quan = bmquanUids;
    
                                            //선택한 자재 수량 표시
                                            var o = gu.getCmp('item_quan');
                                            if (o != null) {
                                                o.update( '선택한 자재 : ' + targetRec.length);
                                            }
                                        }
                                        //gm.me().bmquan = rec[0].get('bm_quan');
                                        
                                        
                                        //var bmquan = this.bm_quan;
    
                                        
                                        // var targetGrid = gu.getCmp('targetPartGrid');
                                        // targetGrid.getStore().insert(0,rec);
                                        
                                        // targetUids.push(rec[0].get('bm_quan'));
                                        // gm.me().targetUid = targetUids;
                                ...
                                dockedItems: [
                                    ...
                                    items: [
                                        ...
                                        },
                                        {
                                            ...
                                            listeners : {
                                                change : function(fieldObj, e) {
                                                    //console_log("search_item_name change >>>> ");
                                        ...
                                        },
                                        {
                                            width: partGridWidth,
                                            field_id:  'search_specification',
                                            id: gu.id('search_specification_sk'),
                                            name: 'search_specification',
                                            xtype: 'triggerfield',
                                            emptyText: '규격',
                                            trigger1Cls: Ext.baseCSSPrefix + 'form-clear-trigger',
                                ...
                                //dockedItems
                                    ...
                                    xtype: 'gridpanel',
                                    id: gu.id('targetPartGrid'),
                                    height: 300,
                                    columnWidth: 0.65,
                                    store: targetPartStore,
                                    layout: 'fit',
                                    tbar: [{
                                        text: 'Delete',                                
                                        extend: 'Ext.app.ViewController',
                                        handler: function(rowIndex){
                                            var rec = this.up("grid").getSelectionModel().getSelection();
                                            var delRecId = rec[0].get('id');
                                            
                                            var targetGrid = gu.getCmp('targetPartGrid');
                                            targetGrid.getStore().removeAt(targetGrid.getStore().find('id', delRecId));
    
                                            var index = gm.me().targetUid.indexOf(delRecId);
    
                                            if(index > -1){
                                                gm.me().targetUid.splice(index, 1);
                                            }
    
                                             //선택한 자재 수량 표시
                                             var o = gu.getCmp('item_quan');
                                             if (o != null) {
                                                 o.update( '선택한 자재 : ' + targetGrid.getStore().data.items.length);
                                             }
                                    ...
                                    columns: [
                                        ...
                                        {text: "품목코드", width: '20%', dataIndex: 'item_code', sortable: true},
                                        {text: "품명", width: '30%', dataIndex: 'item_name', sortable: true},
                                        {text: "수량", xtype: 'numbercolumn', width: '10%', dataIndex: 'bm_quan', 
                                            sortable: true, editor: {allowBlank : false}
                                            , renderer: function(val, meta, record){
                                                if(val) return val;
                                                return record.get('bm_quan');
                                            }},
                                        {text: "규격", width: '20%', dataIndex: 'specification', sortable: true},
                                        {text: "재질", width: '20%', dataIndex: 'model_no', sortable: true}
                    ...
                    if(vCompanyReserved4 ==  'SKNH01KR') {
                        partItem = [searchPartGridsknh];
                        //partItem = [gm.me().createPartForm, searchPartGrid];
                    }else{
                        partItem = [gm.me().createPartForm, searchPartGrid];
                    };
                    ...
                    var winPart = Ext.create('ModalWindow', {
                        ...
                        buttons: [{
                            ...
                            handler: function() {
                                ...
                                //console_logs('>>11', gm.me().bmquan);
                                //console_logs('>>112', form.isValid());
                                //var selections = Ext.getCmp(gu.id('targetPartGrid')).getSelectionModel().getSelection();
                                if(vCompanyReserved4 ==  'SKNH01KR') {
                                    var duplicateItem = [];
                                    var selections = Ext.getCmp(gu.id('targetPartGrid')).getStore().data.items;
                                    var checkGrid = gm.me().grid.getStore().data.items;
                                    
                                    var srcItemsCode = [];
                                    for(var j=0; j<checkGrid.length; j++){
                                        srcItemsCode.push(checkGrid[j].get('item_code'));
                                        //console_logs("item_name   :  ", item_name);
                                    }

                                    var bmQuansWithId = new Array();

                                    for(var i=0; i<selections.length;i++){
                                        var unit = new Object();
                                        var uid = selections[i].get('id');
                                        var bm_quan = selections[i].get('bm_quan');
                                        var item_code = selections[i].get('item_code');
                                        
                                        unit.uid = uid;
                                        unit.bm_quan = bm_quan;

                                    //unit.push(uid);
                                    //unit.push(bm_quan);

                                    bmQuansWithId.push(unit);

                                    var isItem = (srcItemsCode.indexOf(item_code)!== -1);

                                    if(isItem){
                                        duplicateItem.push(item_code);
                                    }
                                    ...
                                    for(var i=0; i<selections.length;i++){
                                        ...
                                        var isItem = (srcItemsCode.indexOf(item_name)!== -1);

                                        if(isItem){
                                            duplicateItem.push(item_name);
                                        }
                                    }
                                    var uids = gm.me().targetUid;
                                    

                                    // if(duplicateItem.length!==0) {
                                    //     Ext.MessageBox.alert('등록된 자재입니다','품목코드  :  '+ duplicateItem);
                                    // } else {
                                        ...
                                        Ext.Ajax.request({
                                            ...
                                            params: {
                                                ...
                                                versionCheck: gm.me().versionCheck,
                                                uidList: uids,
                                                bmQuansWithId: JSON.stringify(bmQuansWithId)
                                            },
                                            success: function(result, request) {
                                                box.hide();
                                                winPart.close();
                                ...
                                if (form.isValid()) {
                                   console_logs('>>>error_msg_content', error_msg_content);
                                    var val = form.getValues(false);
...
//                            Ext.MessageBox.alert('경과', result.responseText + '개 자재가 복사되었습니다.');
    ...
    addMyCartAction : Ext.create('Ext.Action', {
        ...
        handler: function(widget, event) {
            ...
            console_logs('mycart  --  class_code  = ', targetId);

            var targetValveRec = gu.getCmp(targetId).getStore().data.items;
            var tValveCnt = targetValveRec.length;
            ...
            for(var i=0; i< selections.length; i++) {
                ...
                var pl_no = rec.get('pl_no');
                var sales_price = rec.get('sales_price');
                var pr_quan = rec.get('new_pr_quan');

                new_pr_quan = rec.get('new_pr_quan');
                ...
                if(bEx == false ) {
                    ...
                    if(vCompanyReserved4 ==  'SKNH01KR' && (gm.me().pClassCode == "ACT" || gm.me().pClassCode == "RAP")) {

                        console_log("vCompanyReserved4 ==  'SKNH01KR' && gm.me().pClassCode == ACT");
                        
                        console_logs("tValveCnt >>>", tValveCnt);


                        //원본, (tValveCnt 0이되서 밸브합산이 안되는 오류증상)

                        // pr_quan = pr_quan * tValveCnt;
                        // my_pr_quan.push(pr_quan);


                        //변경
                      
                            //var valve_count = rec.get('valve_count');
                            var valve_int = rec.get('valve_int');
                            var making_quan = rec.get('making_quan');
                            console_logs('valve_int 출력 >>>>' , valve_int);
                            console_logs('메이킹 출력 >>>>' , rec);
                        if(making_quan > 0){
                           pr_quan = valve_int * making_quan;
                        }else{
                            pr_quan = valve_int
                        }
                        console_logs('로직변경후 밸브 반영된 값 출력 >>>>' , pr_quan);
                        console_logs('로직변경후 밸브 반영된 값 출력 >>>>' , new_pr_quan);
                        my_pr_quan.push(pr_quan);
                        ...
                        if(making_quan > 1){
                            pr_quan = valve_int * making_quan;
                            my_pr_quan.push(pr_quan);
                            console_logs('로직변경후 밸브 반영된 값 출력 >>>>상민 탄다111', my_pr_quan );
                         }else{
                            my_pr_quan.push( rec.get('new_pr_quan'));
                            console_logs('로직변경후 밸브 반영된 값 출력 >>>>상민 탄다222', my_pr_quan );
                         }
                        
                    }
                    console_logs('mycart  --  class_code  = ', pr_quan);
                ...
                Ext.Ajax.request({
                    ...
                    success : function(result, request) {
                        ...
                        gm.me().myCartStore.load(function(records) {
                            ...
                            //         var new_pr_quan = o.get('new_pr_quan');
                            //         o.set('reserved_double1', new_pr_quan);
                            //         console_logs('new_pr_quan1 출력 >>>', new_pr_quan);
                            //     }
                            // }
                            //여기까지
                            ...
                            Ext.MessageBox.alert('알림', '카트담기 성공.', function callBack(id) {
                                ...
                                // gm.me().myCartStore.load(function(records) {
                                //     console_logs("records 출력222 >>>", records);
                                //     if(records!=null) {
                                ...
                                //             //일괄적으로만 들어간다.
                                //             console_logs('newnew_pr_quan 출력3333 >>>', newnew_pr_quan);
                                //             records[i].set('reserved_double1', newnew_pr_quan);
...
//                });
    ...
    getPartListAction : Ext.create('Ext.Action', {
        iconCls: 'af-pdf',
        text: 'P/L 양식',
        tooltip: 'Part',
        disabled: false,
        handler: function(widget, event) {
        	...
        	excel_store.getProxy().setExtraParam('excelSetting', 'ok');
        	excel_store.getProxy().setExtraParam('isLimitZero', true);
        	
        	excel_store.load(
    			{
    	            scope: this,
    	            callback: function(records, operation, success) {
    	                var excelPath = excel_store.getProxy().getReader().rawData.excelPath;
...
//        	excel_store.load(function(result, request) {
//        		console_logs('>>> aaaa', result);
//        		console_logs('>>> bbbb', request);
//        		var jsonData = Ext.decode(result.responseText);
//        		console_logs('>>> jsonData', jsonData);
...
//        	});
    ...
    getPartAllListAction : Ext.create('Ext.Action', {
        iconCls: 'af-pdf',
        text: '전체파트Excel',
        tooltip: 'Part',
        disabled: false,
        handler: function(widget, event) {
            gm.me().printAllPartExcelHandler();
...
//                            },
    ...
    valveNoStore : Ext.create('Mplm.store.PartLineValveStore', {
        hasNull: false
    }),

    //스카나 수량합계 스토어
    partSumSearchStore : Ext.create('Mplm.store.PartSumSearchStore', {
        hasNull: false
    }),
    ...
    redrawSearchStore : function() {
        ...
        if(vCompanyReserved4 ==  'SKNH01KR') {
            ...
            //스카나 설계자재관리 N은 검색되지않도록 처리
            store.getProxy().setExtraParam('notify_flag', 'Y');
    ...
    uploadValveComplete : function(items) {
    	
    	console_logs('uploadValveComplete items', items);
...
//	        autoScroll: true,
    ...
    //전체엑셀
    printAllPartExcelHandler: function () {

        var store = gm.me().getStore();
      
        console_logs('=====>포로젝트', gu.getCmp('projectcombo').getValue());
        store.getProxy().setExtraParam("part_all", 'Y');
        ...
        store.getProxy().setExtraParam("ac_uid", gu.getCmp('projectcombo').getValue());
        ...
        switch (vCompanyReserved4) {
            ...
            default:
                try {
                    ...
                    //			} else {
                    gm.me().excelPrintFc();
                    store.getProxy().setExtraParam("part_all", 'N');
                    store.getProxy().setExtraParam("ac_uid", '-1');
                    //			}
                } catch (e) {
                }
                break;
            ...
            var form = Ext.create('Ext.form.Panel', {
                ...
                items: [
                  
                    {
                        ...
                        id: 'txt_req_quan_ten',
                        name: 'unique_id',
                        emptyText: '선택한 제품의 수량을 입력 값 을 곱합니다',
                        flex: 1,
                        readOnly: true,
                        width: 300,
                        fieldStyle: 'background-color: #EAEAEA; background-image: none;'
            ...
            var prWin = Ext.create('ModalWindow', {
                ...
                buttons: [{
                    ...
                    handler: function(btn){
                        ...
                        // //var txt_req_quan_ten = Ext.getCmp('txt_req_quan_ten').getValue();
                        // //var prquan_arr= [];

                        // //var s =[];
                        // for(var i=0; i<selections.length; i++) {
                        //     //var target_uid = selections[i].get('unique_uid');
                        //     var pr_quan = selections[i].get('pr_quan');
                        //     console_logs("pr quan 구매수량 리스트 >>>", pr_quan);

                        //     var origin_quan = gm.me().gridMycart.getSelectionModel().selected.items[i].get('reserved_double1');
                        //     var multiply = origin_quan * 10
                        //     console_logs("곱하기 값 >>>", multiply);
                        //     console_logs("pr quan multiply 리스트 >>>", multiply);

                        //     gm.me().gridMycart.getSelectionModel().selected.items[i].set('reserved_double1', multiply);

                        //     //gm.editAjax('assymap', 'req_date', a, 'unique_id', target_uid,  {type:''});
                        //     //gm.me().gridMycart.getSelectionModel().selected.items[i].set('reserved_double1', a);
                        ...
                        //valve 수량 곱하기, 스카나,
                        var mupliply_valve = Ext.getCmp('mupliply_valve').getValue();
                        console_logs("mupliply_valve 출력 >>>", mupliply_valve);
                        ...
                        for(var i=0; i<selections.length; i++) {
                            ...
                            //var bm_quan = selections[i].get('bm_quan');
                            var bm_quan = selections[i].get('bm_quan');
                            //console_logs("bm_quan 구매수량 리스트 >>>", bm_quan);

                            //var origin_quan = gm.me().gridMycart.getSelectionModel().selected.items[i].get('bm_quan');
                            //var multiplyBr = bm_quan * mupliply_valve;
                            var multiplyPr = bm_quan * mupliply_valve;
                            //console_logs("곱해진 값 >>>", multiplyBr);
                            console_logs("pr quan multiply 리스트 >>>", multiplyPr);

                            //gm.me().gridMycart.getSelectionModel().selected.items[i].set('bm_quan', multiplyBr);
                            gm.me().gridMycart.getSelectionModel().selected.items[i].set('pr_quan', multiplyPr);

                            Ext.Ajax.request({
                                url: CONTEXT_PATH + '/design/bom.do?method=updateMyCartQty',
                                params: {
                                    assymap_uid: target_uid,
                                    pr_qty: multiplyPr,
                                    //bm_quan : multiplyBr,
                                    child: selections[i].get('unique_id_long')
                                },
                                success: function(result, request) {
    ...
    //추가작성
    cloudProjectTreeCopyStore : Ext.create('Mplm.store.cloudProjectTreeStore', {}),
    beforeTreeStore : Ext.create('Mplm.store.cloudBeforeTreeStore', {autoLoad:false}),
    currentTreeStore : Ext.create('Mplm.store.cloudCurrentTreeStore', {autoLoad:false}),

    //카피봄 버튼 추가작성

    copyBomtreeModal : Ext.create('Ext.Action', {
        itemId: 'copyBomtreeModal',
        iconCls: 'af-edit',
        text : '카피',
        ...
        handler: function(widget, event) {
            ...
            var treeForm = Ext.create('Ext.form.Panel', {
                id: gu.id('formPanel'),
                xtype:'container',
                plain: true,
                frame: false,
                ...
                buttons: [
                    {
                            ...
                            text : '파트 복사',
                            handler : function(a, b){
                                gm.me().copyPartBomAction();
                                gm.me().currentTreeStore.getProxy().setExtraParam('pjuid', gm.me().selectedPjUid);
                                gm.me().currentTreeStore.load();
                                gm.me().originalGrid.expandAll();
                            }
                    ...
                    handler: function() {
                        if (win) {
                            win.close();
                            gm.me().cloudProjectTreeStore.load();
                            //gm.me().assyGrid.expand();
                            // gm.me().assyGrid.getSelectionModel().select(0);
                            gm.me().assyGrid.getSelectionModel().getSelection();
                            // gm.me().copyGrid.expandAll();
                            gm.me().assyGrid.expandAll();
     ...
     copyTreeGrid : function() {        
        
         gm.me().originalGrid = Ext.create('Ext.tree.Panel', {
            title: '현재프로젝트',
            id:'originalGrid',
            collapsible: false,
            useArrows: true,
            rootVisible: false,
            store: this.currentTreeStore,
            multiSelect: false,
            viewConfig: {
                stripeRows: true,
                enableTextSelection: true,
                ...
                listeners: {
                    beforedrop: function(node, data, overModel, dropPosition, dropHandlers, a, b, c, d) {

                        console_logs('>> origin beforedrop node', node.object);
                        console_logs('>> origin beforedrop data.object', data.object);
                        console_logs('>> origin beforedrop data.object.records[0]', data.records[0]);

                        console_logs('>> origin beforedrop dropPosition', dropPosition);
                        console_logs('>> origin beforedrop dropHandlers', dropHandlers);
     ...
     oringTreeGrid : function() {
        this.copyGrid = Ext.create('Ext.tree.Panel', {
            title: '이전프로젝트',
            id:'copyGrid',
            collapsible: false,
            useArrows: true,
            rootVisible: false,
            store: this.beforeTreeStore,
            // multiSelect: false,
            multiSelect: true,
            //selModel: (vCompanyReserved4 == 'SKNH01KR') ? Ext.create("Ext.selection.CheckboxModel", {}) : null,
            selModel: Ext.create('Ext.selection.TreeModel',{expanded :false}),
            // checkPropagation: (vCompanyReserved4 == 'SKNH01KR') ? 'both' : null,
            ...
            viewConfig: {
                ...
                listeners: {
                    ...
                    console_logs('>>>>>>>>>copyTreeGrid beforedrop node', node.object);
                    console_logs('>>>>>>>>>copyTreeGrid beforedrop data.object', data.object);
                    console_logs('>>>>>>>>>copyTreeGrid beforedrop data.object.records[0]', data.records[0]);
                    console_logs('>>>>>>>>>copyTreeGrid beforedrop data.object.records[1]', data.records[1]);


                        console_logs('>>>>>>>>>copyTreeGrid beforedrop dropPosition', dropPosition);
                        console_logs('>>>>>>>>>copyTreeGrid beforedrop dropHandlers', dropHandlers);
                    //handle depth manually here
            ...
            dockedItems: [
                {
                    ...
                    items : [
                        // 호선검색
                        {
                            ...
                            listeners: {
                                select: function(combo, record) {
                                    ...
                                    order_com_unique = combo.value;
                                    gm.me().cloudprojectCopyStore.getProxy().setExtraParam('pj_name', combo.value);
                                    gm.me().cloudprojectCopyStore.load();
                                ...
                                change: function(combo, record) {
                                    ...
                                    order_com_unique = combo.value;
                                    gm.me().cloudprojectCopyStore.getProxy().setExtraParam('order_com_unique', combo.value);
                                    gm.me().cloudprojectCopyStore.getProxy().setExtraParam('pj_name', combo.value);
                                    gm.me().cloudprojectCopyStore.load();
                                }
                            }
                        ...
                        // 이전 프로젝트 포함 검색
                        {
                            ...
                            displayField: 'pj_code',
                            valueField: 'unique_id',
                            store: this.cloudprojectCopyStore,
                            minChars: 1,
                            triggerAction: 'all',
                            listeners: {
                                select: function(combo, record) {
                                    console_logs('===record', record);
                                    gm.me().selectCopyProjectCombo(record);
                                }
                            }
                        },
     ...
     srchCurrentTreeHandler : function(){
       
         this.currentTreeStore.getProxy().setExtraParam('pjuid', gm.me().selectedPjUid);

            this.currentTreeStore.load({
     ...
     copyPartBomAction : function(){
        // 이전프로젝트 from
        var copyData =  gm.me().copyGrid.getSelectionModel().getSelection();
        // 현재프로젝트 to
        //gm.me().originalGrid.getSelectionModel().select(0);
        var currentData = gm.me().originalGrid.getSelectionModel().getSelection();
       // var currentData = gm.me().originalGrid.getSelectionModel().select(0);
        //console_logs("copyData   ", copyData);
        //console_logs("currentData   ", currentData);
        
        var to_pj_uid = currentData[0].get('ac_uid');
        var to_assy_uid = currentData[0].get('id');
        var to_parent = currentData[0].get('child');

        var FromAcUid = copyData[0].get('ac_uid');
        var FromAssyUid = copyData[0].get('id');
        ...
        copyData.forEach(el => {
            fromAcUids.push(el.get('ac_uid'));
            fromAssyUids.push(el.get('id'));
        });

        if (copyData.length < 0 || currentData.length < 0) {
            Ext.MessageBox.alert('오류', '먼저 BOM을 붙여넣을 \r\n대상 호선을 선택하세요.');
        }else if(currentData[0].get('class_code') != copyData[0].get('class_code')){
            Ext.MessageBox.alert('오류', '같은공정의 \r\n어셈블리를 선택하세요.');
        }
            else{
            var box = Ext.MessageBox.wait('자재 추가중...', '알림');
            Ext.Ajax.request({
                ...
                params: {
                    to_pj_uid: to_pj_uid,
                    to_assy_uid: to_assy_uid,
                    to_parent: to_parent,
                    FromAcUid: FromAcUid,
                    FromAssyUid: FromAssyUid,
                    fromAcUids: fromAcUids,
                    fromAssyUids: fromAssyUids,
                },
                success: function(result, request) {
                   
                    box.hide();
                    Ext.MessageBox.alert('알림', '파트 복사가 성공적으로 완료되었습니다.');
                    gm.me().originalGrid.getSelectionModel().select(0);
