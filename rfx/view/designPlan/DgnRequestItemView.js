//수주관리 메뉴
Ext.define('Rfx.view.designPlan.DgnRequestItemView', {
...
//        });
        ...
        var IsNewStore = Ext.create('Mplm.store.IsNewStore',{});
            ...
            listeners: {
                    select: function(combo, record) {
                        var selected = combo.getValue();
                            is_new_combo = selected;
                            is_new = selected;
                        if(selected == null || selected.length < 1 || selected == '') {
                            is_new_combo = 'ALL';
                            is_new = null;
                        }
                        gm.me().store.getProxy().setExtraParam('is_new',selected);
                        DistinctPjName.getProxy().setExtraParam('is_new_combo',selected);
                },
                    // blur:function() {
                    //     if(this.getRawValue() == "" ) {
                    //         this.clearValue();
...
//            	break;
        ...
        this.completeDesign = Ext.create('Ext.Action', {
            iconCls: 'mfglabs-retweet_14_0_5395c4_none',
            text: '설계완료',
            tooltip: '설계완료',
            disabled: true,
            handler: function() {
                ...
                Ext.MessageBox.show({
                    title: '설계완료',
                    msg: '선택된 항목을 완료상태로 변경하시겠습니까?',
                    width: 350,
                    buttons: Ext.MessageBox.OKCANCEL,
                    icon: Ext.MessageBox.QUESTION,
                    fn: function(result) {
                        if(result == 'ok') {
                            gm.me().completeDesignHandler();
                        }
                    }
                })
...
//            buttonToolbar.insert(9, '-');
            ...
            buttonToolbar.insert(4, this.completeDesign);
            buttonToolbar.insert(5, '-');
            buttonToolbar.insert(6, this.delete_requestAction);
            buttonToolbar.insert(7, '-');
            buttonToolbar.insert(8, this.attachFileAction);
            break;
        default:
    }
        ...
        this.grid.getSelectionModel().on({
            selectionchange:function(sm, selections) {
                if(selections!=null && selections.length>0) {
                    ...
                    gm.me().completeDesign.enable();
                } else {
                    gm.me().attachFileAction.disable();
                    gm.me().completeDesign.disable();
                }
            }
...
//    	}
    ...
    completeDesignHandler: function() {
        var selections = gm.me().grid.getSelectionModel().getSelection();
        var assymap_uids = '';
        
        for(var i=0; i<selections.length; i++) {
            var rec = selections[i];
            var is_new = rec.get('is_new');
            var unique_id = rec.get('unique_id_long');

            assymap_uids = assymap_uids == '' ? unique_id : assymap_uids + ',' + unique_id;

            if(is_new == 'F') {
                Ext.MessageBox.alert('알림', '이미 완료된 항목이 있습니다.');
                return;
            }
        }
