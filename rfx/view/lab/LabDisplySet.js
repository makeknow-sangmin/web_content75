 ...
 */
Ext.define('Rfx.view.lab.LabDisplySet', {
    extend: 'Rfx.base.BaseView',
    xtype: 'lab-display-set',
    initComponent: function(){
		...
		//삭제버튼 추가.
		this.labRemoveAction = Ext.create('Ext.Action', {
        	iconCls: 'af-remove',
        	...
        	handler: function() {	
			 	...
			 	Ext.MessageBox.show({
                    ...
                    fn: gm.me().labRemoveHandler,
                    icon: Ext.MessageBox.QUESTION
                });
        	}
		});
		buttonToolbar.insert(3, this.labRemoveAction);
        ...
        this.setGridOnCallback(function(selections) {
        	...
        	if(rec!=undefined && rec!=null) {
            	...
            	gUtil.enable(this.labRemoveAction);
        	} else {
				gUtil.disable(this.labEditAction);
				gUtil.disable(this.labRemoveAction);
			}
...
//    	var lineGap = 30;
	...
	//게시글삭제 labRemoveHandler
	labRemoveHandler : function(btn){
		if(btn == 'yes') {
			...
			for(var i=0; i<selections.length; i++) {
		 	...
		 	Ext.Ajax.request({
				url: CONTEXT_PATH + '/admin/mescode.do?method=destroy',
				params:{
					unique_id : uids
				},
