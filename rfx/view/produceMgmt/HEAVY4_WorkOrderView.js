Ext.define('Rfx.view.produceMgmt.HEAVY4_WorkOrderView', {
...
// 		});
        ...
        this.store.getProxy().setExtraParam('orderBy', 'rtgast.reserved_timestamp1 is null desc');
        this.store.getProxy().setExtraParam('ascDesc', ', rtgast.reserved_timestamp1 desc');
...
//        case "SHNH01KR":
        ...
        this.printPDFAction3 = Ext.create('Ext.Action',{
            ...
            handler: function(widget, event) {
                ...
                if(selectedRecord.get('po_type') == 'ACT' || selectedRecord.get('po_type') == 'UKN' || selectedRecord.get('po_type') == 'RAP') {
        ...
        this.requestGoodsoutIndividual = Ext.create('Ext.Action',{
            ...
            handler: function(widget, event) {
                ...
                // switch(reserved_varchar1) {
                    ...
                    // default:
                        ...
                        prwin = gMain.selPanel.prwinopen3(form);

                // }
            }
        });

        this.deleteValve = Ext.create('Ext.Action',{
            iconCls: 'mfglabs-retweet_14_0_5395c4_none',
            text: '벨브 취소',
            tooltip:'벨브 취소',
            disabled: true,
            ...
            handler: function(widget, event) {
                ...
                Ext.MessageBox.show({
					title: '확인',
					msg: '요청 벨브를 취소 하시겠습니까?',
					buttons: Ext.MessageBox.YESNO,
					fn: function (result) {
						if (result == 'yes') {

                            var rec = gm.me().gridValve.getSelectionModel().getSelection();
                            console_logs('정보 : ',rec);
                            var cartmapUids = [];
                            var assymapUids = [];
                            var pcartmapUids = [];
                            for(var i = 0 ; rec.length > i ; i++  ){
                                cartmapUids.push(rec[i].get('coord_key3'));
                                assymapUids.push(rec[i].get('unique_uid'));
                                pcartmapUids.push(rec[i].get('cart_parent'));
                            }

                            Ext.Ajax.request({
                                url: CONTEXT_PATH + '/index/process.do?method=deleteValve',
                                params: {
                                    cartmapUids: cartmapUids,
                                    assymapUids: assymapUids,
                                    pcartmapUids : pcartmapUids
                                },

                                success: function (result, request) {
                                    gMain.selPanel.store.load();
                                    gm.me().valveNoStore.load();
        ...
        this.addTabworkOrderGridPanel('하위자재', 'EPC5_SUB', {
                ...
                dockedItems: [

                    {
                        ...
                        items: [
                            this.requestGoodsoutIndividual,
                            // '-',
                            // addMinPo
                        ]
                    }
            ...
            function(selections) {
                if (selections.length) {
                    ...
                    gm.me().requestGoodsoutIndividual.enable();
...
//        }
        ...
        buttonToolbar.insert(5, this.printPDFAction);
        if(vCompanyReserved4 == 'DDNG01KR'){
            ...
            if (selections==null || selections.length ==0) {
                ...
                switch(vCompanyReserved4) {
                    ...
                    case 'SKNH01KR':
                    {
                        ...
                        gm.me().vSELECTED_PARENT_UID = rec.get('parent_uid');

                        this.workOrderGrid.getStore().getProxy().setExtraParam('rtgastuid', gm.me().vSELECTED_RTGAST_UID);
                        switch(selectedRecord.get('po_type')){
                            case 'ACT':
                            case 'UKN':
                            case 'RAP':
                                this.workOrderGrid.getStore().getProxy().setExtraParam('parent_uid', gm.me().vSELECTED_PARENT_UID);
                                this.workOrderGrid.getStore().getProxy().setExtraParam('ac_uid', gm.me().vSELECTED_AC_UID);
                                this.workOrderGrid.getStore().getProxy().setExtraParam('po_type', selectedRecord.get('po_type'));
                                this.workOrderGrid.getStore().getProxy().setExtraParam('actuator', true);
                                this.workOrderGrid.getStore().getProxy().setExtraParam('limit', 1000);
                                //this.workOrderGrid.getStore().getProxy().setExtraParam('class_code', gm.me().big_pcs_code);

                                if(vCompanyReserved4 == 'SKNH01KR'){
                                    this.gridValve.getStore().getProxy().setExtraParam('parent_uid', gm.me().vSELECTED_PARENT_UID);
                                    this.gridValve.getStore().getProxy().setExtraParam('menuCode', 'EPJ1');
                                    this.gridValve.getStore().getProxy().setExtraParam('ac_uid', gm.me().vSELECTED_AC_UID);
                                    this.gridValve.getStore().getProxy().setExtraParam('class_code', selectedRecord.get('po_type'));
                                    this.gridValve.getStore().getProxy().setExtraParam('rtgastuid', gm.me().vSELECTED_RTGAST_UID);
                                    this.gridValve.getStore().load(function(records){
                                       console_logs("  ==============>   gridValve  records", records);
                                        if(records!=null) {
                                            var o = gu.getCmp('valve_quan_workorder');
                                            if (o != null) {
                                ...
                                break;
							/*case 'LPT':
							 this.workOrderGrid.getStore().getProxy().setExtraParam('parent_uid', gm.me().vSELECTED_UNIQUE_UID);
							 this.workOrderGrid.getStore().getProxy().setExtraParam('ac_uid', gm.me().vSELECTED_AC_UID);
							 this.workOrderGrid.getStore().getProxy().setExtraParam('po_type', selectedRecord.get('po_type'));
							 this.workOrderGrid.getStore().getProxy().setExtraParam('limit', 1000);
							 break;*/
                            default:
                                ...
                                this.workOrderGrid.getStore().getProxy().setExtraParam('ac_uid', gm.me().vSELECTED_AC_UID);
                                this.workOrderGrid.getStore().getProxy().setExtraParam('po_type', selectedRecord.get('po_type'));
                                this.workOrderGrid.getStore().getProxy().setExtraParam('actuator', false);
                                this.workOrderGrid.getStore().getProxy().setExtraParam('limit', 1000);
                                break;
                        }

                        //this.workOrderGrid.getStore().getProxy().setExtraParam('parent_uid', rec.get('parent_uid'));
                        this.workOrderGrid.getStore().load(function(records){
                ...
                switch(vCompanyReserved4){
                    ...
                    case 'SKNH01KR':
                        if(gm.me().vSELECTED_STATE == 'P' && rec.get('is_closed') == '설계완료'){
                            if(  this.big_pcs_code!=undefined &&
...
//        this.store.load();
    ...
    callBackWorkList: function(title, records, arg, fc, id) {
        ...
        // gm.me().valveNoStore = Ext.create('Mplm.store.PartLineValveStore', {
		// 	hasNull: false
		// }),
...
//
    ...
    denyWorkOrderFc: function() {
        ...
        switch(vCompanyReserved4){
       		case "SKNH01KR":
       			strUrl = '/index/process.do?method=denyWorkOrderHeavySknh';
        		break;
        	default:
        		strUrl = '/index/process.do?method=denyWorkOrderHeavy';
        		break;
        }

        Ext.Ajax.request({
            //url: CONTEXT_PATH + '/index/process.do?method=denyWorkOrderHeavy',
            url: CONTEXT_PATH + strUrl,
            params:{
                rtgastUid: rtgastUid,
                po_type: po_type
            },
    ...
    callBackWorkList: function(title, records, arg, fc, id) {
        ...
        this.gridValve.getSelectionModel().on({
            ...
            selectionchange: function(sm, selections) {
                ...
                if (selections==null || selections.length ==0) {
                    gm.me().deleteValve.disable();
                }else{
                    gm.me().deleteValve.enable();
                }
        ...
        prWin =	Ext.create('Ext.Window', {
            ...
            buttons: [{
                ...
                handler: function(btn){
                    ...
                    var val = form.getValues(false);
                    // var datas = (gm.me().workOrderGrid.store.data)['items'];
                    var record = gm.me().workOrderGrid.getSelectionModel().getSelection();
                    var assymap_uids = [];
                    var vv_cnts=[];
                    var item_names=[];
                    var childs=[];
