Ext.define('Rfx.view.produceMgmt.HEAVY4_ProducePlanScanaView', {
...
//		           direction: 'ASC'
      ...
      //Lot 해체 Action 생성
        ...
        case 'SKNH01KR':
            ...
            buttonToolbar.insert(2, this.restoreValveAction);
       ...
       /* if(vCompanyReserved4 =='DDNG01KR'){
        ...
        this.setGridOnCallback(function(selections) {
            ...
            if (selections.length) {
            	...
            	gm.me().vSELECTED_UNIQUE_UID = rec.get('unique_uid'); //선택된 CARTMAP_UID
				gm.me().vSELECTED_ASSYMAP_UID = rec.get('assymap_uid'); //선택된 ASSYMAP_UID
				gm.me().vSELECTED_PARENT_UID = rec.get('parent_uid'); //선택된 ASSYMAP_UID
			
            	
            	//console_logs('setGridOnCallback     >>>>   ', this.big_pcs_code);
                ...
                default:
					...
					this.bomListGrid.getStore().getProxy().setExtraParam('menuCode', 'EPC5');
					this.bomListGrid.getStore().getProxy().setExtraParam('limit', 1000);
					break;
            	}
            	...
            	if(rec!=null){
            		...
            		try {
						...
						console_logs('processGrid 불러오기',processGrid);
            	...
            	if(class_code=='ACT-V' && assy_is_closed=='Y' ) {
	...
	planFinishByCartmap: function(cartmaparr){
		if(cartmaparr.length == 1){
			var cartuid = cartmaparr[0];

			Ext.Ajax.request({
				url: CONTEXT_PATH + '/index/process.do?method=pcsFinishByCartmap',				
				params:{
					cartuid : cartuid
				},
        		...
        		if(gm.me().checkname == false){
    				Ext.MessageBox.alert('경고', '중복 확인 버튼을 눌러주세요');
                    	...
                    	//var cartmaparr = gm.me().cartmap_uids;
							...
							var pcs_code = gm.me().getProcessType();
							console_logs('pCS코드333',pcs_code);
                    		if(pcs_code == 'ACT-V') {
                    			pcs_code = 'ACT';
							};
							
							if(pcs_code == 'ACT'|| pcs_code == 'RAP') {
...
//				});
    ...
    savePcsstdHandler : function() {
	 ...
	 addTabworkOrderGridPanel: function(title, menuCode, arg, fc, id) {
		...
		//2019sus 9월 6일 valve store 변경
	    // valveNoStore : Ext.create('Mplm.store.PartLineGeneralStore', {
	    //     hasNull: false
		// }),
		valveNoStore : Ext.create('Mplm.store.PartLineValve2Store', {
			class_code : this.big_pcs_code
		}),
	    ...
	    refreshValve : function(ac_uid) {
	        ...
	        gm.me().valveNoStore.getProxy().setExtraParam('ac_uid', gm.me().vSELECTED_AC_UID);
			...
			gm.me().valveNoStore.getProxy().setExtraParam('parent_uid', gm.me().vSELECTED_ASSYMAP_UID);
			gm.me().valveNoStore.getProxy().setExtraParam('menuCode', 'EPC5');
			gm.me().valveNoStore.getProxy().setExtraParam('menuCode', 'EPC5');
			gm.me().valveNoStore.getProxy().setExtraParam('class_code', this.big_pcs_code);
