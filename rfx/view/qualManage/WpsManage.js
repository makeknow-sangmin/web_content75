 ...
 */
Ext.define('Rfx.view.qualManage.WpsManage', {
    extend: 'Rfx.base.BaseView',
    xtype: 'wps-manage-view',
    initComponent: function(){
		...
		switch(vSYSTEM_TYPE) {
		case 'MES':
			console_log('>>>>'+vSYSTEM_TYPE)
			this.setDefComboValue('gubun', 'valueField', 'WP');//ComboBOX의 ValueField 기준으로 디폴트 설정.
			break;
		case 'PLACE':
		...
		//WP
		this.store.getProxy().setExtraParam('gubun', 'WP');
	...
	createWest: function() {
		...
		this.grid.getSelectionModel().on({
			selectionchange: function(sm, selections) {
				...
				gm.me().docuStore.getProxy().setExtraParam('menu_code', 'WPS1');
