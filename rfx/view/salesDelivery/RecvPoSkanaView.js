//수주관리 메뉴
Ext.define('Rfx.view.salesDelivery.RecvPoSkanaView', {
    extend: 'Rfx.base.BaseView',
    xtype: 'received-mgmt-skn-view',
    inputBuyer : null,
...
//							, function() {});
		...
		this.addCallback('GET-SRCAHD-CODE', function(combo, record){
			//영업,출하 > 수주관리 >신규등록 >수주제품선택 >상세정보 (표준제품구성의 서브자재 내용 적용)
			Ext.Ajax.request({
				...
				success : function(result, request) { 
					...
					//console_logs('테스트', result.responseText);
					
					//JSON > object
					var rec = Ext.JSON.decode(result.responseText);
			
					console_logs('테스트', rec);
				
					for(var i=0; i< rec.count; i++){
						gMain.scanagrid.getStore().add({
									pl_no:rec.datas[i].pl_no,
									class_code:rec.datas[i].class_code,
									item_name:rec.datas[i].item_name,
									bm_quan:rec.datas[i].bm_quan
						}
						);
					}
					/*var result = result.responseText;
...
//		this.addReadonlyField('unique_id');
		...
		//명령툴바 생성
        // var buttonToolbar = this.createCommandToolbar({REMOVE_BUTTONS: ['REMOVE']});
		var buttonToolbar = this.createCommandToolbar();

        //console_logs('this.fields', this.fields);

        this.createStore('Rfx.model.RecvPoViewSKNModel', [{
	            property: 'unique_id',
	            direction: 'ASC'
	        }],
	        [100]/*pageSize*/
        ...
        this.tabchangeHandler = function(tabPanel, newTab, oldTab, eOpts)  {
            ...
            switch(newTab.title) {
            case '상세정보':
            	console_logs('tab selected1', newTab.title);
...
//        	var mainmenu = Ext.getCmp( 'recvPoview' + '-mainmenu' );
            ...
            if (selections.length) {
            	...
            	var ac_uid = rec.get('unique_id_long');
            	var parent = rec.get('reserved_numbere');
				var parent_uid = rec.get('parent_uid');
				console_logs("테스트페런트",parent);
            	
    			Ext.Ajax.request({
    				url: CONTEXT_PATH + '/design/bom.do?method=getBomParentList',
    				params:{
    					ac_uid: ac_uid,
    					parent: parent,
    					...
    					if(records!=null) {
    						...
    						if(datas!=null && datas.length>0) {
    							for(var i=0; i<datas.length; i++) {
	    							var o = datas[i];
	    							//console_logs('datas o', o);
	    							gm.scanagrid.getStore().add(o);
    							}
...
/*        	processGrid.getStore().getProxy().setExtraParam('assymap_uid', gMain.selPanel.vSELECTED_UNIQUE_ID);
		...
		this.store.on('load', function (me, records, c, d) {
			...
			records.forEach(el => {
				var columnValue =  el.get('is_complished');
				if(columnValue === 'N'){
					el.set("is_complished", "제작의뢰미완료");
				}else if(columnValue === 'C'){
					el.set("is_complished", "제작의뢰완료");
				}
	        	...
	        	Ext.Ajax.request({
	    			...
	    			success : function(result, request) {	
	    				...
	    				if(result.responseText+''=='0'){
	    					...
	    					failure:Ext.Msg.alert('안내', '입력된 수주번호가 중복되었습니다. </br>수주번호를 다시한번 확인하세요.', function() {})
	    				}
