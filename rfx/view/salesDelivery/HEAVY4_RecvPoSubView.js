...
//수주등록된 전체 자재 메뉴
Ext.define('Rfx.view.salesDelivery.HEAVY4_RecvPoSubView', {
...
//
	...
	editRedord: function(field, rec) {
		...
		console_logs('테스트tkdals1323', field);
		
				var value=rec.get(field);
				var tableName = gm.getTableName(field);
				var whereField = "coord_key3";
				switch(field) {
				
					case 'recv_date':
						rec.set('whereValue', rec.get("coord_key3"));
						//테이블, 컬럼, 입력데이터, where 조건 컬럼, in 값
						gm.editAjax('cartmap', 'recv_date', rec.get(field), 'unique_id', rec.get("coord_key3"), {type: ''});
						break;
					default:
