 ...
 */
Ext.define('Rfx.view.supplyMgmt.ListBoardSupplyView', {
	 extend: 'Rfx.base.BaseView',
	  //  xtype: 'board-view',
	    initComponent: function(){
			...
			switch(vSYSTEM_TYPE) {
			case 'MES':
				this.setDefComboValue('gubun', 'valueField', 'Z');//ComboBOX의 ValueField 기준으로 디폴트 설정.
				//this.setDefComboValue('fields', 'valueField', 'SUP');
				break;
			case 'PLACE':
	        ...
	        //모델 정의
	        this.createStore('Rfx.model.BoardSupplier', [{
	        ...
	        this.setGridOnCallback(function(selections) {
				...
				if(selections!=null && selections.length>0) {
					...
					if(user_id == login_id || login_id=='root') {
			...
			//this.store.getProxy().setExtraParam('gubun', "Z");
			this.store.getProxy().setExtraParam('menuCode', this.link);
			//this.store.getProxy().setExtraParam('fields', "SUP");
	    ...
	    itemdblclick: function (view, record, htmlItem, index, eventObject, opts) {
	    	...
	    	Rfx.model.BoardSupplier.load(record.get('unique_id'), {
