...
Ext.define('Ext.calendar.CalendarPanel', {
    ...
    initComponent: function() {
        ...
        if(this.showMonthTitle) {
            this.tbar.items.push({
                id: this.id + '-tb-select',
                text: Ext.Date.format(new Date(), 'Y/m'),
                // handler: this.onMonthClick,
                scope: this,
                toggleGroup: 'tb-views'
            });
    ...
    // private
    onLayout: function() {
    	//console_logs('CalendarPanel onLayout', this);
        this.callParent();
    ...
    onEventAdd: function(form, rec) {
    	//console_logs('CalendarPanel onEventAdd', rec);
    ...
    onEventUpdate: function(form, rec) {
    	//console_logs('CalendarPanel onEventUpdate', rec);
        this.hideEditForm();
        rec.commit();
        this.eventStore.sync();
    ...
    onEventDelete: function(form, rec) {
    	//console_logs('CalendarPanel onEventDelete', rec);
        this.hideEditForm();
    ...
    onEventCancel: function(form, rec) {
    	//console_logs('CalendarPanel onEventCancel', rec);
        this.hideEditForm();
    ...
    showEditForm: function(rec) {
    	//console_logs('CalendarPanel showEditForm', rec);
    ...
    hideEditForm: function() {
    	//console_logs('CalendarPanel hideEditForm', rec);
        if (this.preEditView) {
    ...
    setActiveView: function(id){
    	//console_logs('CalendarPanel setActiveView', id);
        var l = this.layout,
        ...
        //console_logs('CalendarPanel setActiveView', 'Ext.suspendLayouts();');
        Ext.suspendLayouts();
        ...
        if(id !== this.id+'-edit'){
           if(id !== this.preEditView){
        	   //console_logs('CalendarPanel setActiveView', 'setstartdate');
                l.activeItem.setStartDate(this.startDate, true);
                //console_logs('CalendarPanel setActiveView', 'setstartdate done');
            }
           //console_logs('CalendarPanel setActiveView', 'updateNavState');
           this.updateNavState();
           //console_logs('CalendarPanel setActiveView', 'updateNavState done');
        }
        //console_logs('CalendarPanel setActiveView', 'resumeLayouts');
        Ext.resumeLayouts(true);
        //console_logs('CalendarPanel setActiveView', 'resumeLayouts done');
        
        //console_logs('CalendarPanel setActiveView', 'fireViewChange');
        this.fireViewChange();
        //console_logs('CalendarPanel setActiveView', 'fireViewChange done');
    },

    // private
    fireViewChange: function() {
    	//console_logs('CalendarPanel fireViewChange', this);
        if (this.layout && this.layout.getActiveItem) {
            ...
            if (view && view.getViewBounds) {
                ...
                // var cur_date = Ext.Date.format(vb.start, 'Y/m');
                // Ext.getCmp(this.id + '-tb-select').setText(cur_date);
                var info = {
    ...
    updateNavState: function() {
    	//console_logs('CalendarPanel updateNavState', this);
        if (this.showNavBar !== false) {
    ...
    setStartDate: function(dt) {
    	//console_logs('CalendarPanel setStartDate', dt);
    ...
    showWeek: function(dt) {
    	//console_logs('CalendarPanel showWeek', dt);
    ...
    onPrevClick: function() {
        //console_logs('CalendarPanel onPrevClick', this);
        var cur_date = Ext.getCmp(this.id + '-tb-select').getText();
        if(cur_date == null || cur_date == undefined) {
            cur_date = new Date();
        };
        cur_date = new Date(cur_date);
        cur_date = Ext.Date.format(Ext.Date.add(cur_date, Ext.Date.MONTH, -1), 'Y/m');
        Ext.getCmp(this.id + '-tb-select').setText(cur_date);
    ...
    onNextClick: function() {
    	//console_logs('CalendarPanel onNextClick', this);
        var cur_date = Ext.getCmp(this.id + '-tb-select').getText();
        if(cur_date == null || cur_date == undefined) {
            cur_date = new Date();
        };
    ...
    // private
    onDayClick: function() {
    	//console_logs('CalendarPanel onDayClick', this);
    ...
    onWeekClick: function() {
    	//console_logs('CalendarPanel onWeekClick', this);
    ...
    onMonthClick: function() {
    	//console_logs('CalendarPanel onMonthClick', this);
    ...
    getActiveView: function() {
    	//console_logs('CalendarPanel getActiveView', this.layout.activeItem);
