...
Ext.define('Ext.calendar.view.AbstractCalendar', {
    ...
    isEventVisible: function(evt) {
        console_logs('>>>>????data', evt.data || evt);
        var M = Ext.calendar.data.EventMappings,
            data = evt.data || evt,
    ...
    setStartDate: function(start, refresh) {
        ...
        var cnt = gMain.getAjaxTrack('Ext.calendar.data.EventModel');
        ...
        if(cnt==0) {
	    		var track = gMain.getAjaxTrack('Ext.calendar.data.EventModel');
	    		//console_logs('track', track);
	    		gMain.checkAjaxTrack('Ext.calendar.data.EventModel', true);
		        ...
		        this.store.load({
		            ...
		            callback : function(records, options, success) {
		            	//console_logs('setStartDate callback records', records);
		        		gMain.checkAjaxTrack('Ext.calendar.data.EventModel', false);
		            }
		        });

        } else {
        	//console_logs('duplicated track is abandoned...', cnt);
        }
        if (refresh === true) {
            this.refresh();
        }
