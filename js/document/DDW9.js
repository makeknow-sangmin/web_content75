...
function backupConfirm(btn){
    ...
    if (selections) {
        ...
        if(result=='yes') {
        	...
        	Ext.Ajax.request({
    			url: CONTEXT_PATH +  '/supercom.do?method=rollback&rptype=share',
    			params:{
...
Ext.define('FtpFile', {
	 ...
	 ,
	    proxy: {
			type: 'ajax',
	        api: {
	        	read: CONTEXT_PATH + '/supercom.do?method=projectRepo&repo_type=share', 					/*1recoed, search by cond, search */
