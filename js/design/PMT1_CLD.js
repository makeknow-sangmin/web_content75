               ...
               listeners: {
	                ...
	                afterrender: function(field) {
	            		field.getEl().on('click', function( event, el ) {
	            			...
	            			if(value_image_path=='-'||value_image_path.substring(0,1)=='P'){
...
var contextMenu = Ext.create('Ext.menu.Menu', {
    items: [ editAction, removeAction, /*putMyPartAction,*/ bomCopyAction ]
});
	...
	Ext.define('SrcAhd', {
   	 ...
   	 fields: /*(G)*/vCENTER_FIELDS,
   	    proxy: {
				type: 'ajax',
		        api: {
		            read: CONTEXT_PATH + '/purchase/material.do?method=read'+ '&standard_flag=S&vCUR_MENU_CODE=' + vCUR_MENU_CODE, /*1recoed, search by cond, search */
 		...
 		Ext.each(/*(G)*/vCENTER_COLUMNS, function(columnObj, index) {
			...
			grid = Ext.create('Ext.grid.Panel', {
			        ...
			        dockedItems: [{
			            ...
			            items: [
			                    searchAction
			                    , '-',  addAction,  '-', removeAction, '-',/* putMyPartAction, '-', */addNxExcel,
			                    '-', printBarcode,
	      				        '->',
	      				      {
			                        text: panelSRO1133,
