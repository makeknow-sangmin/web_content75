                ...
                fieldStyle: 'background-color: #FBF8E6; background-image: none;',
    	            ...
    	            listeners: {
    	                 select: function (combo, record,rowIndex) {
    	                 	...
    	                 	//console_logs('Selected unique_id : ' + record[0].get('unique_id'));
    	                 	//console_logs('Selected rowIndex : ' + rowIndex);
//    	                 	var oPj_uid = Ext.getCmp('project_uid');
//    	                 	Ext.getCmp('project_uid').setValue(pjuid);
    	                 	
    	                 	var record1 = myGrid.getStore().getAt(rowIndex);
    	                 	var sel_model = myGrid.getSelectionModel();
    	                 	var record1 = sel_model.getSelection()[0];
//    	                 	var project_uid = record.get('project_uid');
    	                 	record1.set('job_uid',job_uid);
    	                 	record1.set('job_name',job_name);
    	                	//console_logs(record1);
...
//	    store.getProxy().setExtraParam('defaultTaskArray', defaultTaskArray);
	    store.load(function() {
	    	...
	    	var nowm = currentTime.getMonth()+1;
	    	var nowd = currentTime.getDate();
	    	
	    	var lastDay = getLAstDayOfMonth(nowy, nowm);
	    	
	    	//console_logs('Now today :', currentTime);
	    	//console_logs('Now Month :', nowm);
	    	//console_logs('Now Day :', nowd);
	    	...
	    	if(now<10){
	    		todays = nowy+'-'+nowm+'-'+'0'+now;
	    	}else{
	    		todays = nowy+'-'+nowm+'-'+now
	    	}
	    	//console_logs('todays: ', todays);
	    	while(now>=nowd-6){
	    		
	    		if(now<1){
	    			date7s.push([nowy+'-'+(nowm-1)+'-'+lastDay]);
	    			lastDay--;
	    		}else{
	    			if(now<10){
	    				date7s.push([nowy+'-'+nowm+'-'+'0'+now]);
	    			}else{
	    				date7s.push([nowy+'-'+nowm+'-'+now]);	
	    			}
