...
var editHandler = function() {
                			...
                			SrcAhd.load(unique_id ,{
                				 success: function(srcahd) {
                					 var curImage_no = srcahd.get('image_no');
                				        ...
                				        var win = Ext.create('ModalWindow', {
                				            ...
                				            buttons: [{
                				            	...
                				            	handler: function(){
                				                    ...
                				                    if(form.isValid())
                				                    {
                				                    	var newImage_no = Ext.getCmp('image_no').getValue();
                    				                	if(newImage_no!=curImage_no){
                    				                		Ext.getCmp('isVersionUpdate').setValue('true');
                    				                	}else{
                    				                		Ext.getCmp('isVersionUpdate').setValue('false');
                    				                	}
...
function deleteConfirm(btn){
    ...
    if (selections) {
        ...
        if(result=='yes') {
        	...
        	Ext.MessageBox.alert('Check','Delete Success : '+count);
			...
			grid = Ext.create('Ext.grid.Panel', {
			     ...
			     // paging bar on the bottom
			        ...
			        dockedItems: [{
			            ...
			            items: [
			                    searchAction
			                    , '-',  addAction,  '-', editAction, '-', removeAction, '-', putMyPartAction, 
			                    '-', printBarcode, '-' , addNxExcel,
	      				        '->',
	      				      {
			                        text: panelSRO1133,
			                        iconCls: 'save',
