...
//			console_log('setValue id '+id);
			if(values == id) { //IE 호환성 유지.
				...
				var sub_text_pjcode = text_pjcode.substring(0,12);
...
//			    	console.log(record.data);
					if(rec != undefined){
						...
						var not_definedUID= rec.substring(0,2);
						...
						Ext.Ajax.request({
							...
							success : function(result, request) {
								...
								Ext.Ajax.request({
									url: CONTEXT_PATH + '/sales/poreceipt.do?method=read',
									params:{
										id : id
									},
									success : function(result, request) {
										...
										if(obj.datas != null){
											...
											var pm_name = obj.datas[0].pm_name;
											var pj_code = obj.datas[0].pj_code;
											...
											if(pj_code == 'NOTDEFINED'){
												Ext.getCmp('pj_srchDescription').setValue('');
												Ext.getCmp('pj_srchCav_no').setValue('');
												Ext.getCmp('pj_srchRegist_date').setValue('');
												Ext.getCmp('pj_srchDelivery_plan').setValue('');
												Ext.getCmp('pj_srchPm_id').setValue('');
												store.getProxy().setExtraParam('ac_uid', id);
												store.load({});
											}else{
												...
												if(vCUR_MENU_CODE != 'SDL1' && vCUR_MENU_CODE != 'VMF7' && vCUR_MENU_CODE != 'DBM8'
													&& vCUR_MENU_CODE!='DDW2'	
												){
													...
													if(vCUR_MENU_CODE != 'DBM1_ELE'){
														...
														Ext.getCmp('pj_srchPm_id').setValue(pm_name);
...
//													selectedMoldCoord = order_com_unique;
												...
												case 'PPO1':
													//selectedAssyUid = id;
													store.getProxy().setExtraParam('ac_uid', id/*selectedAssyUid*/);
													store.load({});
													break;
												case 'EPC1':
													//selectedAssyUid = id;
												...
												case 'DBM1_ELE':
													selectedMoldUid = id;
													//selectedAssyUid = uid_srcahd;
													storeL.getProxy().setExtraParam('uid_srcahd', selectedAssyUid);
...
//													selectedAssyUid = uid_srcahd;
													...
													var val = hiddenFrm.getValue();
													storeCartLine.getProxy().setExtraParam('is_ordered', val);
													storeCartLine.load({});
													break;
												case 'DBM8':
//													selectedAssyUid = uid_srcahd;
...
var storeMenu = Ext.create('Ext.data.TreeStore', {
    ...
    proxy: {
         type: 'ajax',
         url: CONTEXT_PATH + '/sales/poreceipt.do?method=projectTree&menucode='+vCUR_MENU_CODE,
//         params:{
//        	 callType : 'TOP'
//		 },
         reader: {
...
function callbackToolbarRenderrer(callType) {
	Ext.Ajax.request({
		...
		success : function(result, request) {
			...
			treatCommonProjectAssy2(id, callType);
		}
	}); 
}

function treatCommonProjectAssy2(id, combo) {
	Ext.Ajax.request({
		url: CONTEXT_PATH + '/sales/poreceipt.do?method=pjgroup',			
		params:{
			id : id
		},
		success : function(result, request) {
//			addAction.enable();
			var obj = Ext.decode(result.responseText);
			if(obj.datas[0]!=undefined){
				var Sub_pj_code = obj.datas[0].pj_code;
				var order_com_unique = obj.datas[0].order_com_unique;
				var uid_srcahd = obj.datas[0].uid_srcahd;
				if(callType == 'TOP') {
		    		var obj = Ext.getCmp('mytree');
		    		var treePanel = obj.tree;
//		    		console_log( treePanel.getRootNode() );
		    		var root = treePanel.getRootNode();
		    		var children = root.childNodes;
		    		for(var i=0; i<children.length; i++) {
		    			var parent = children[i];
//		    			console_log('$$$$$$$$$$$$$$$');
//		    			console_log(parent.data);
//		    			console_log(parent.data.text);
		    			var text = parent.data.text;
		    			var key = text.substring(0,9);
		    			if(key == 'NOT-DEFIN'){
		    				key = 'NOTDEFINED' ;
		    			}
//		    			console_log('key=' + key);
//		    			console_log('Sub_pj_code=' + Sub_pj_code);
//		    			console_log('id=' + id);
		    			if(key==Sub_pj_code) {
		    				parent.expand(false, function(){
		    					obj.setValue(id+';'+order_com_unique+';'+uid_srcahd);
		    		        	//console_log('expand');
		    		        	//fillProjectTree();
		    				});//endofexpand
					...
					success : function(result, request) {
						...
						if(pj_code != 'NOTDEFINED'){
							...
							if(id == null || id ==''){
			           			Ext.MessageBox.alert('Please choose a project.','id is null at treatCommonProjectAssy2 success'); 
			           		}
							if(vCUR_MENU_CODE != 'SDL1' && vCUR_MENU_CODE != 'VMF7' && vCUR_MENU_CODE != 'DBM8'
								&& vCUR_MENU_CODE != 'DDW2'
							
							){
								var fildDescription =Ext.getCmp('pj_srchDescription');
								if(fildDescription ==undefined){
									//Ext.MessageBox.alert('Please choose a project.','fildDescription is undefined.'); 
								} else {
									if(vCUR_MENU_CODE != 'DBM1_ELE'){
										Ext.getCmp('pj_srchDescription').setValue(description);
										Ext.getCmp('pj_srchCav_no').setValue(cav_no);
