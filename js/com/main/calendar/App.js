...
Ext.define('Ext.calendar.App', {
    ...
    constructor : function() {
        console_log('Minor workaround for OSX Lion scrollbars');
        this.checkScrollOffset();
        
        console_log('This is an example calendar store that enables event color-coding');
        this.calendarStore = Ext.create('Ext.calendar.data.MemoryCalendarStore', {
            data: Ext.calendar.data.Calendars.getData()
        });

        console_log('eventStore pre');
//        // A sample event store that loads static JSON from a local file. Obviously a real
//        // implementation would likely be loading remote data via an HttpProxy, but the
//        // underlying store functionality is the same.

//        this.eventStore = Ext.create('Ext.calendar.data.MemoryEventStore', {data: Ext.calendar.data.Events.getData()  });
        
        this.eventStore = Ext.create('Ext.calendar.data.EventStore');
        console_log('eventStore post');
                ...
                items: [/*{
                    ...
                    listeners: {
                        ...
                        'eventover': function(vw, rec, el){
                            //console_log('Entered evt rec='+rec.data.Title+', view='+ vw.id +', el='+el.id);
                        },
                        'eventout': function(vw, rec, el){
                            //console_log('Leaving evt rec='+rec.data.Title+', view='+ vw.id +', el='+el.id);
                        },
                        'eventadd': {
                            fn: function(cp, rec){
