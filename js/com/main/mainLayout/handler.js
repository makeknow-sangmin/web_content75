...
function getProjectTreeToolbar() {
	...
	arrPjTreetoolbar.push(combotree);
...
function loadFieldInfoMulti(main_menuKey, sub_menuKey, linkPath) {
		    	    	...
		    	    	if(successSub ==true) {
		    	    		var codeFieldListSub = getCodeFieldListMain(records);
				    	        ...
				    	        LoadJs(linkPath);
					    			...
					    			loadCodeGroup(codeFieldListMain, mainRecords, linkPath);
					    			loadCodeGroupSub(codeFieldListSub, records, linkPath);
					    		}
		    	    	} else {//ifsubSuccess
		    	    		lfn_gotoHome();
		    	    	}
...
function loadCodeGroup(parentList, records, linkPath) {
	Ext.Ajax.request({
...
function loadCodeGroupSub(parentList, records, linkPath) {
	Ext.Ajax.request({
	//url: CONTEXT_PATH + '/code.do?method=codenameJs',
	url: CONTEXT_PATH + '/code.do?method=codelistJs',
	params:{
...
function loadFieldInfo(menu_key, linkPath) {
	(new Ext.data.Store({ model: 'ExtFieldColumn'}) ).load({
	    params: {
	    	menuCode: menu_key
	    },
	    callback: function(records, operation, success) {
	    	...
	    	if(success ==true) {
	    		...
	    		if(codeFieldListMain == undefined || codeFieldListMain==null  || codeFieldListMain.length==0) {
	    			...
	    			loadCodeGroup(codeFieldListMain, records, linkPath);
	    		}
...
function cenerFinishCallback() {
	if(cenerLoadingPln!=null) {
		...
		Ext.getCmp('mainview-property-panel-div').removeAll();
		Ext.getCmp("mainview-property-panel").setTitle(panelSRO1132);		
	
		cenerLoadingPln.setLoading(false);
		
		//메세지 리셋.
		error_msg_content='잘못된 입력입니다. 다시 한번 확인 해보세요.';
	}
}
function menuAction(menu_key, service_name, displayName, linkPath, permType, flag1, flag2, flag3, flag4, flag5) {
	...
	Ext.getDoc().dom.title = displayName + ' [' + service_name + '^' + menu_key + ']';  
	...
	switch(menu_key) {
	case 'AAP2_CLD': //case for 2 store
		loadFieldInfoMulti(menu_key, 'AAP2_SUB', linkPath, flag1, flag2, flag3, flag4, flag5);
		break;
...
//		break;
	case 'QGR4':
		console_info('multitleloadFieldInfo:' + menu_key +',' + linkPath);
		loadFieldInfoMulti(menu_key, 'QGR4_SUB', linkPath, flag1, flag2, flag3, flag4, flag5);
		break;
	case 'PRF1':
		console_info('multitleloadFieldInfo:' + menu_key +',' + linkPath);
...
var searchToolBarTap = function() {
	//console_logs('searchToolBarTap', vCUR_MENU_CODE);
	...
	store.load(function(record) {/*console_logs(record);*/});
};
