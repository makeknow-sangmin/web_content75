...
var rtgAstColumn =  [
                    ...
                    { text     : 'req_date',   width : 80,  sortable : true, dataIndex: 'req_date'  },
...
var viewHandler = function() {
	...
	RtgAst.load(unique_id ,{
		 success: function(rtgAst) {
			 ...
			 var name = rtgAst.get('name');//제목
			 var content = rtgAst.get('content');//내용
			 var state = rtgAst.get('state');//결재상태		
			 var rtg_type = rtgAst.get('rtg_type');//결재type
			 var aprv_date = rtgAst.get('aprv_date');//결재 일자
			 var req_date = rtgAst.get('req_date');//상신일자
...
Ext.define('RtgAst', {
    	 ...
    	 fields: rtgAstFields,
    	    proxy: {
		        ...
		        api: {
		            read: CONTEXT_PATH + '/rtgMgmt/routing.do?method=readRouting',
...
Ext.onReady(function() {  
	  ...
	  Ext.define('Gubun', {
	        extend: 'Ext.data.Model',
	        fields:gubunRouteFields
	    });
	  

	// Simple ComboBox using the data store
	var gubunCombo = Ext.create('Ext.form.field.ComboBox', {	    
	...
	store = new Ext.data.Store({  
	   ...
	   // params: {gubun: 'P'}
		  baseParams: {
			//  gubun: Ext.getCmp('gubunCombo').getValue()
