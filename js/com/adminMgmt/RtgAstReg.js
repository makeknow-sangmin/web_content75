  ...
  var grid = Ext.create('Ext.grid.Panel', {
	...
	var cform = Ext.create('Ext.form.Panel', {
        ...
        items: [
        {
            ...
            fieldLabel: 'user_name',//결재구분---> 데이타가 안 넘어감 우선 배제 
            name: 'user_name',
            value: 'G', //일반결재 G 결재타입 G:일반, R:PR, O:PO, W:GR, Q:FQ
            xtype: 'combo',
            mode: 'local',
                    ...
                    if (selections) {
                    	alert(selections.length);
                        	for(var i=0; i< selections.length; i++) {
                        		...
                        		hid_userlist = rec.get('usrast_unique_id');
                        		hid_userlist_role = rec.get('user_role');
                        	//	alert("unique_id--->"+unique_id);
                        	//	alert("hid_userlist--->"+hid_userlist);
                        	//	alert("hid_userlist_role-->"+hid_userlist_role);
                        		rtgAst = Ext.ModelManager.create({hid_userlist : hid_userlist,hid_userlist_role : hid_userlist_role},
                        				'RtgAst');
