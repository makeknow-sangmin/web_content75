...
var productTeamCubeStore = Ext.create('Mplm.store.ProductTeamCubeStore', {} );
...
var addElecHandler = function() {
	...
	Equip.load(unique_uid ,{
		 success: function(equip) {
			 
			var unique_uid =  equip.get('unique_uid');
			Ext.Ajax.request({
			        ...
			        var win = Ext.create('ModalWindow', {
			            ...
			            buttons: [{
			            	...
			            	handler: function(){
			                	...
			                	console_info(Ext.JSON.encode(val));
			                	var equip = Ext.ModelManager.create(val, 'Equip');
			            		// 저장 수정
			                	equip.save({
			                		success : function() {
			                           	if(win) 
...
Ext.define('Equip', {
	 ...
	 fields: /* (G) */vCENTER_FIELDS,
	    proxy: {
			type: 'ajax',
	        api: {
	            read: CONTEXT_PATH + '/sales/equip.do?method=read', 					/*															 */
	            create: CONTEXT_PATH + '/sales/equip.do?method=create', 																							
	            update: CONTEXT_PATH + '/sales/equip.do?method=create',
	            destroy: CONTEXT_PATH + '/sales/equip.do?method=destroy' 			/* delete */
	        },
			reader: {
				type: 'json',
				root: 'datas',
        ...
        if(result=='yes') {
        	for(var i=0; i< selections.length; i++) {
        		...
        		var unique_id = rec.get('unique_id');
	           	 var equip = Ext.ModelManager.create({
	           		unique_id : unique_id
	        	 }, 'Equip');
...
var repairAction = Ext.create('Ext.Action', {
	itemId: 'repairButton',
    iconCls: 'return',
    text: 'Repair',
    disabled: true,
...
Ext.onReady(function() {  
	...
	var pjTreeGrid =
		...
		// border: false,
		 title: '부품선택',// cloud_product_class,
		 region: 'center',
		 listeners: {
...
//			xtype:  'textfield',
		...
		{
			fieldLabel: getColName('mc_name'),
			 name: 'mc_code',
			 id: 'mc_code',
            xtype: 'combo',
            store: productTeamCubeStore,
            fieldStyle: 'background-color: #FBF8E6; background-image: none;',
	            ...
	            listConfig: {
	                ...
	                getInnerTpl: function() {
	                	return '<div data-qtip="{unique_id}">[{description}] {specification}</div>';
	                }
	            },
	            listeners: {
	                 select: function (combo, record,rowIndex) {
	                 	...
	                 	console_log('Selected Value : ' + combo.getValue());
	                 	var product_uid = record[0].get('unique_id');
	                 	var mc_name = record[0].get('specification');
	                 	var mc_code = record[0].get('description');
	                 	console_log('Selected product_uid : ' + record[0].get('unique_id'));
	                 	console_log('Selected mc_name : ' + mc_name);
	                 	console_log('Selected mc_code : ' + mc_code);
//	                 	var oPj_uid = Ext.getCmp('project_uid');
	                 	Ext.getCmp('product_uid').setValue(product_uid);
	                 	Ext.getCmp('mc_name').setValue(mc_name);
...
//		    				isNewPart = true;
//		    			};
			    			var product_uid = Ext.getCmp('product_uid').getValue();
			    			var mc_name = Ext.getCmp('mc_name').getValue();
			    			var mc_code = Ext.getCmp('mc_code').getValue();
							...
							Ext.Ajax.request({
								url: CONTEXT_PATH + '/sales/equip.do?method=create',				
								params:{
									unique_id: unique_id,
...
//		        store: cartstore,
		        ...
		        dockedItems: [{
		            dock: 'top',
		            xtype: 'toolbar',
		            items: [resetAction,'-',repairAction
      				     ]
		        	}
...
//			    region: 'center',
			...
			var main3 =  Ext.create('Ext.panel.Panel', {
				height: getCenterPanelHeight(),
			    layout:'border',
			    border: false,
			    region: 'center',
...
//		fLAYOUT_CONTENTMulti([main3, main2]);  
