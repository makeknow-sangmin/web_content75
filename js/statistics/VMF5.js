...
function setLink(systemCode, mchn_code, link) {

	   var panelId = 'panel' + systemCode  + mchn_code;
	   setThisLoading(panelId);	
			console_log(link);
		var iframeId = 'iframe' + systemCode  + mchn_code;
	...
	if(arr!=null) {
		 for(var j=0; j<arr.length; j++) {
	     	...
	     	var link = CONTEXT_PATH +   "/statistics/chart.do?method=main&cubeCode=VMF7&group_code=" + tabType + "&work_unit_gubun=" + ROLE_CODE
	     				+ '&work_unit=' + 'M_'+mchn_code + "&dayDiv=" + dayDiv;
	     	
	     	var panelId = 'panel' + tabType + mchn_code;
	     	console_log(panelId);
	         setThisLoading(panelId);
	         setLink(tabType, mchn_code, link);
		 }
	}
}
...
//	console_log('systemCode=' + systemCode);
	...
	for(var i=0; i<arrMember.length; i++) {
		...
		var mchn_code = o['mchn_code'];
		var name_cn = o['name_cn'];
//		console_log('group_code=' + group_code);
		if(systemCode==group_code) {
			 var panelId = 'panel' + systemCode + mchn_code;
			 var o = {
	                 title: mchn_code.substring(1) + ' - ' + name_cn,
                  	 id: panelId,
	                  height:180,
	                   ...
	                   bodyStyle:  'background:#EAEAEA',
	                     items: [{
  	 			        xtype : "component",
	 			            id: "iframe" + systemCode + mchn_code,
  	 	                height:180,
  	 			        autoEl : {
  	 			            tag : "iframe",
  	 			            height: '100%',
