...
var printPDFAction = Ext.create('Ext.Action',{
    ...
    handler: function(widget, event) {
    	...
    	var rtg_type = rec.get('rtg_type');//po_no
    	var url = null;
    	switch(rtg_type) {
    	case 'R':
    		url = '/pdf.do?method=printPr';
    	break;
    	}
    	if(url==null) {
    		Ext.MessageBox.alert('오류', 'PDF 출력할 수 없는 결재유형입니다.');
    	} else {
	    	Ext.Ajax.request({
    ...
    if (selections) {
    	...
    	if(selections.length>0) {
            ...
            if(result=='yes') {
	    		...
	    		if(state!='I') {
	    			Ext.MessageBox.alert('경고','결재중인 건에 대해서만 상신취소할 수 있습니다.');
	    		} else {
	    			var unique_id = rec.get('unique_id');
   	 ...
   	 fields: /*(G)*/vCENTER_FIELDS,
   	    proxy: {
		        ...
		        api: {
		            ...
		            destroy: CONTEXT_PATH + '/rtgMgmt/routing.do?method=cancelRouting'
		        },
				reader: {
					type: 'json',
					root: 'datas',
