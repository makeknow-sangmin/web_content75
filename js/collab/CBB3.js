...
///********************파일첨부시 추가(Only for FileAttachment)**************
                	...
                	var maker_name_filed = Ext.getCmp('maker_name').getValue();
                	console_log(val);
                	if(maker_name_filed == ""){
...
var productfields = [{name: 'unique_id_long', type: 'string'},
...
Ext.onReady(function() {  
	    Ext.define('Product', {
	        ...
	        proxy:{
	            type:'ajax',
	            api:{
	                read: CONTEXT_PATH + '/sales/poreceipt.do?method=readProduct',
	                create: CONTEXT_PATH + '/sales/poreceipt.do?method=createProduct'
	            },
	            reader:{
	                type:'json',
	                root:'datas',
		...
		productStore.load(function() {
			var dataview = Ext.create('Ext.view.View', {
		        ...
		        tpl  : Ext.create('Ext.XTemplate',
		            '<tpl for=".">',
		                '<div class="phone">',
		                    (!Ext.isIE6? '<img width="64" height="64" src="'+ '/media/b2b/{[values.image_path.replace(/ /g, "-")]}" />' :
		                     '<div style="width:74px;height:74px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'' + '/media/b2b/{[values.image_path.replace(/ /g, "-")]}\',sizingMethod=\'scale\')"></div>'),
		                    '<strong>{maker_name}</strong>',
		                    '<li>사양:{description}</li>',
		                    '<li>업체명:{maker_name}</li>',
		                '</div>',
		            '</tpl>'
		        ),
		        plugins : [
			    ...
			    viewConfig: {
			    	listeners: {
					    itemclick: function(view, record, item, index, e, eOpts) {
					    	...
					    	productStore.getProxy().setExtraParam('class_code', class_code);
					    	productStore.load(function() {});
					    }//end itemclick
			    	}//end listeners
				},
