		        ...
		        selectionchange: function(sm, selections) {
		            if (selections.length) {
						...
						cartstore.load(function(records){
							...
							Ext.getCmp('ok_btn_id').setDisabled(false);
							Ext.getCmp('deny_btn_id').setDisabled(false);
							...
							var myRgType = po_no.substring(0,2);
							//console_logs('myRgType', myRgType);
							switch(myRgType) {
							case 'OR':
						...
						},
						{
						    ...
						    handler: function() {
						    	...
						    	if(comment==null || comment =='') {
						    		//Ext.MessageBox.alert('Error','결재 의견이 누락되었습니다.');
						    		Ext.MessageBox.show({
					    	            title:'승인 확인',
					    	            msg: '결재 의견이 누락되었습니다.  결재 승인을 계속 진행하시겠습니까?',
					    	            buttons: Ext.MessageBox.YESNO,
					    	            fn: function(btn) {
					    	                var result = MessageBox.msg('{0}', btn);
					    	                if(result=='yes') {
					    	                	console_logs('coord_key3', coord_key3);
					    	                	doSanction('YES',  gPr_uid, gRtgType, comment, po_user_uid, coord_key3);
					    	                }
					    	            },
						    	...
						    	if(comment==null || comment =='') {
							    	...
							    	Ext.MessageBox.show({
					    	            ...
					    	            fn: function(btn) {
					    	                ...
					    	                if(result=='yes') {
					    	                	console_logs('coord_key3', coord_key3);
					    	                	doSanction('NO',  gPr_uid, gRtgType, comment, po_user_uid, coord_key3);
					    	                }
					    	            },
					    	            icon: Ext.MessageBox.QUESTION
					    	        });
